...
class rpt_cheque_talonario_2 extends reporte {
	const K_TIPO_PAGO_FAPROV_CHEQUE			= 1;
	const K_TIPO_PAGO_FAPROV_TRANSFERENCIA	= 2;

	function rpt_cheque_talonario_2($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false) {
	...
	function print_normnal(&$pdf, $plataforma) {
	    ...
	    case "N":
				...
				//Se imprime este ID en el impreso del comprobante para saber que paso por aqui.
				$pdf->SetFont('Arial','',6);
				$id_paso = 'XY2';
				$pdf->Text($x_ini+412, $y_ini-342, $id_paso);
				$pdf->SetFont('Arial','',11);
	    ...
	    case "S":
				...
				$pdf->SetXY($x_ini+75, $y_ini+66);class rpt_pago_proveedor extends reporte {
				    const K_TIPO_PAGO_FAPROV_CHEQUE			= 1;
				    const K_TIPO_PAGO_FAPROV_TRANSFERENCIA	= 2;
...
class rpt_cheque_talonario extends reporte {
    ...
    function print_normnal(&$pdf, $plataforma) {
		...
		//llenado del cheque
		$pdf->Text($x_ini+323, $y_ini-6, $monto_documento.' .-');
		$pdf->Text($x_ini+245, $y_ini+35, 'Santiago,  '.$fecha_cheque);
		$pdf->SetFont('Arial','',11);
		$pdf->Text($x_ini+83, $y_ini+52, $pagese_a);
		$pdf->SetXY($x_ini+83, $y_ini-720);
		$pdf->MultiCell(300, 12,"$total_palabras_linea");
		...
		// Nominativo
			...
			// PARA CUBRIR FRASE "LA ORDEN DE"
			$pdf->SetFont('Arial','',19);
			$linea_nominativa = '******';	
			$pdf->Text($x_ini+30, $y_ini+62, $linea_nominativa);
			// PARA CUBRIR FRASE "O AL PORTADOR"				
			$linea_nominativa = 'xxxxxx';
			$pdf->Text($x_ini+390, $y_ini+66, $linea_nominativa);	
		}
