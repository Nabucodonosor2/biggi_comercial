...
class rpt_santander extends reporte {
	const K_TIPO_PAGO_FAPROV_CHEQUE			= 1;
	const K_TIPO_PAGO_FAPROV_TRANSFERENCIA	= 2;

	function rpt_santander($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false) {
	...
	function print_normnal(&$pdf, $plataforma) {
	    ...
	    case "N":
	        /**HASTA EL MOMENTO SIEMPRE ES CON FORMATO ASI QUE NO ES NECESARIO EDITAR EL SIGUIENTE CODIGO **/
	        /*
				if($plataforma == 'MAC'){
					$variable_ejemplo = 0;
				}
	...
	function modifica_pdf(&$pdf) {
		...
		if ($COD_TIPO_PAGO_FAPROV == self::K_TIPO_PAGO_FAPROV_CHEQUE){	
			...
			if ($count == 1){
				$nro_faprov = 3812;//$result[0]['NRO_FAPROV'];
				$fecha_faprov = '01/03/2019';//$result[0]['FECHA_FAPROV'];
				$monto_asignado = $result[0]['MONTO_ASIGNADO'];
				$ordenes_compra = 543;//$result[0]['ORDENES_COMPRA'];
				$notas_venta = 541;//$result[0]['NOTAS_DE_VENTA'];
