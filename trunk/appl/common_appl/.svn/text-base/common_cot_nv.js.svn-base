function validate_cot_nv(ve_tabla) {
	...
	if (ve_tabla == 'ITEM_COTIZACION'){
		var sum_total = document.getElementById('SUM_TOTAL_H_0');	
		var porc_dscto_max = document.getElementById('PORC_DSCTO_MAX_0');
		var cod_usuario_ven = document.getElementById('COD_USUARIO_VENDEDOR1_0').value;
		var cod_usu_actual = document.getElementById('COD_USUARIO_ACTUAL_0').value;
		var aut_descto_esp = document.getElementById('AUT_DESCTO_ESP_H_0').value;
		var porc_dscto_max_esp = document.getElementById('PORC_DSCTO_MAX_ESP_0').value;
		...
		if(aut_descto_esp == 'S' && cod_usuario_ven == 11 && cod_usu_actual == 11){
			if(parseFloat(porc_monto_dscto) > parseFloat(porc_dscto_max_esp)){
				alert('La suma de los descuentos es mayor al permitido (máximo '+number_format(porc_dscto_max_esp, 1, ',', '.')+' % entre los dos descuentos, equivalente a '+number_format(monto_permitido, 0, ',', '.')+')');
				document.getElementById('PORC_DSCTO1_0').focus();
				return false;
			}
		}else{ 
