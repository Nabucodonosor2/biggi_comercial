function dlg_find_factura(ve_nom_header, ve_valor_filtro, ve_sql, ve_campo) {
	
   	var id_campo = ve_campo.id;
    var url = "../common_appl/dlg_find_factura.php?nom_header="+URLEncode(ve_nom_header)+"&valor_filtro="+URLEncode(ve_valor_filtro)+"&sql="+URLEncode(ve_sql);
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 200,
