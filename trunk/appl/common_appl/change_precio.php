...
$sql = "SELECT 	PRECIO_VENTA_PUBLICO,
				PRECIO_LIBRE,
				NOM_PRODUCTO,
				dbo.f_get_parametro(69) PORC_DSCTO_MAX_ESP
		FROM 	PRODUCTO
...
$porc_dscto_max_esp = $result[0]['PORC_DSCTO_MAX_ESP'];
...
if($aut_dscto == 'S'){
	$precio_min = round($precio_lista - ($precio_lista * $porc_dscto_max_esp/100), 0);
	$precio_max = round($precio_lista + ($precio_lista * $porc_dscto_max_esp/100), 0);
}else if($precio_libre == 'S'){
	$precio_min = 0;
