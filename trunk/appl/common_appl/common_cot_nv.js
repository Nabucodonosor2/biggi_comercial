function validate_cot_nv(ve_tabla) {
	...
	var vl_porc_max_vendedor1 = document.getElementById('COD_USUARIO_VENDEDOR1_0').options[vl_cod_usuario_vendedor.selectedIndex].dataset.dropdown.split('-');
	var vl_porc = vl_porc_max_vendedor1[1]; 
	var porc_max_vendedor1 = roundNumber(vl_porc, 0);
	...
	if (ve_tabla == 'ITEM_NOTA_VENTA'){
		...
		if (to_num(cod_estado_nota_venta) != 3){	
			if (((parseFloat(monto_dscto1) + parseFloat(monto_dscto2))/parseFloat(sum_total.value))*100 > parseFloat(porc_dscto_max.value)){
				...
				if(vl_cod_nota_venta != 0){
					var ajax = nuevoAjax();
					tabla = 'NOTA_VENTA';
					ajax.open("GET", "../common_appl/ajax_porc_desc_esp.php?cod_nota_venta="+vl_cod_nota_venta+"&porc_dscto_max="+porc_dscto_max.value+"$tabla="+tabla, false);
					ajax.send(null);
	...
	if (ve_tabla == 'ITEM_COTIZACION'){
		var sum_total = document.getElementById('SUM_TOTAL_H_0');		
		//var porc_dscto_max = document.getElementById('PORC_DSCTO_MAX_0');
		aut_desc_esp = get_value('AUT_DESCTO_ESP_H_0');
		desc_1_autorizado = get_value('DESCTO_1_AUTORIZADO_H_0');
		desc_2_autorizado = get_value('DESCTO_2_AUTORIZADO_H_0');
		/**POR DEFECTO SIEMPRE SERA N HASTA NUEVO AVISO**/
		aut_desc_esp = 'N';
		if(aut_desc_esp == 'S'){
			desc_1_autorizado = 0;
			desc_2_autorizado = 0;
			
			set_value('DESCTO_1_AUTORIZADO_H_0',0,0);
			set_value('DESCTO_1_AUTORIZADO_0',0,0);
			set_value('DESCTO_2_AUTORIZADO_H_0',0,0);
			set_value('DESCTO_2_AUTORIZADO_0',0,0);
		}
		
		if(aut_desc_esp == 'S' || (parseFloat(desc_1_autorizado) == 0 && parseFloat(desc_2_autorizado) == 0)){ 
			if(aut_desc_esp == 'S')
				porc_dscto_max = document.getElementById('PORC_DSCTO_MAX_ESP_0').value;
			else	
				porc_dscto_max = document.getElementById('PORC_DESC_PERMITIDO_0').value;
			
			if (sum_total.value=='') sum_total.value = 0;	
				
			var cod_estado_cot = get_value('COD_ESTADO_COTIZACION_0'); 
			if (to_num(cod_estado_cot) != 3){	
				if (((parseFloat(monto_dscto1) + parseFloat(monto_dscto2))/parseFloat(sum_total.value))*100 > parseFloat(porc_dscto_max)){
					
					var vl_cod_nota_venta = get_value('COD_COTIZACION_H_0');
					if(vl_cod_nota_venta != 0){
						var ajax = nuevoAjax();
						tabla = 'COTIZACION';
						ajax.open("GET", "../common_appl/ajax_porc_desc_esp.php?cod_nota_venta="+vl_cod_nota_venta+"&porc_dscto_max="+porc_dscto_max+"$tabla="+tabla, false);
						ajax.send(null);
						var resp = ajax.responseText;
						
						if(resp != 'PERMITIDO'){
							var monto_permitido = (parseFloat(sum_total.value) * parseFloat(porc_dscto_max)) / 100 ;
							alert('La suma de los descuentos es mayor al permitido (máximo '+number_format(porc_dscto_max, 1, ',', '.')+' % entre los dos descuentos, equivalente a '+number_format(monto_permitido, 0, ',', '.')+')');
							document.getElementById('PORC_DSCTO1_0').focus();
							return false;
						}	
					}else{
		...
		}else if(aut_desc_esp == 'N' && (parseFloat(desc_1_autorizado) > 0 || parseFloat(desc_2_autorizado) > 0)){ 
			if (sum_total.value=='') sum_total.value = 0;
			
			porc_desc1 = (parseFloat(monto_dscto1)/parseFloat(sum_total.value))*100;
			porc_desc2 = (parseFloat(monto_dscto2)/parseFloat(sum_total.value))*100;
			
			var cod_estado_cot = get_value('COD_ESTADO_COTIZACION_0'); 
			if (to_num(cod_estado_cot) != 3){	
				if (porc_desc1 > parseFloat(desc_1_autorizado)){
					alert('El % de descuento 1 es mayor al permitido (máximo %'+desc_1_autorizado+'%');
					document.getElementById('PORC_DSCTO1_0').focus();
					return false;
				}
				if (porc_desc2 > parseFloat(desc_2_autorizado)){
					alert('El % de descuento 2 es mayor al permitido (máximo '+desc_2_autorizado+'%');
					document.getElementById('PORC_DSCTO2_0').focus();
					return false;
				}
	...
	if (parseFloat(cod_forma_pago) == 1 && nom_forma_pago_otro == ''){
		alert ('Debe ingresar la Descripciï¿½n de la forma de pago seleccionada.');
	...
	///revisar porque se deberï¿½a validar tambien para NV, pero falla esta validaciï¿½n al hacer el cierre
	if (ve_tabla == 'ITEM_COTIZACION'){
	...
	for (var i = 0; i < aTR.length; i++) {
		...
		if (cod_tipo_doc_pago == ''){
			...
			if (nro_doc == 0){
				alert ("Debe ingresar el Nï¿½mero de Documento.");
		...
		if (valida_motivo == 'S'){
			...
			for (var i = 0; i < aTR.length; i++){
				...
				if (autoriza == true && motivo == ' '){
					alert('ï¿½Debe ingresar el motivo para autorizar el cierre de la Nota de Venta!');
...
function change_vendedor(ve_cod_usuario_vendedor) {
	var vl_label = ve_cod_usuario_vendedor.options[ve_cod_usuario_vendedor.selectedIndex].dataset.dropdown.split('-');
...
function change_porc_vendedor(ve_porc_vendedor) {
	...
	var vl_label = vl_cod_usuario_vendedor.options[vl_cod_usuario_vendedor.selectedIndex].dataset.dropdown.split('-');
