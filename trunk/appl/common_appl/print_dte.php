...
function imprimir($cod_documento,$ES_CEDIBLE,$DTE_ORIGEN,$guardar){
    ...
    switch ($DTE_ORIGEN) {
    	...
    	if(!$guardar){
    	    ...
    	    $pdf->Output(dirname(__FILE__)."/../factura/33_$folio.pdf", 'F');
    	    
    	    return "33_$folio.pdf";
    	}
    	break;
    	...
    	if($ES_CEDIBLE == 'N'){
    		...
    		$NOM_EMPRESA		= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Receptor->RznSocRecep;
    		$DIRECCION			= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Receptor->DirRecep;
    		$NOM_CIUDAD			= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Receptor->CmnaOrigen;
    		$GIRO				= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Receptor->GiroRecep;
    		$MONTO_DSCTO1_V		= $xml_resolucion->SetDTE->DTE->Documento->DscRcgGlobal[0]->ValorDR;
    		$MONTO_DSCTO2_V		= $xml_resolucion->SetDTE->DTE->Documento->DscRcgGlobal[1]->ValorDR;
    		$TOTAL_NETO_V		= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Totales->MntExe;
    		$TOTAL_CON_IVA_V	= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Totales->MntTotal;
    		$MONTO_DSCTO1		= $MONTO_DSCTO1_V;
    		$MONTO_DSCTO2		= $MONTO_DSCTO2_V;
    		$TOTAL_NETO			= $TOTAL_NETO_V;
    	...
    	if(!$guardar){
    	    ...
    	    $pdf->Output(dirname(__FILE__)."/../factura/34_$folio.pdf", 'F');
    	    
    	    return "34_$folio.pdf";
    	}
    	break;
    	...
    	$ES_EXENTA_IVA			= $result[0]['ES_EXENTA_IVA'];
    	...
    	if($ES_EXENTA_IVA == 'S'){
    		$MONTO_EXENTO					= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Totales->MntExe;
    	}
    	...
    	if($ES_EXENTA_IVA == 'S'){
    		$pdf->MultiCell(26, 1,number_format("$MONTO_EXENTO",0,'.','.'),0,"R");
    	}
    	else{
    		$pdf->MultiCell(26, 1,number_format("$TOTAL_NETO",0,'.','.'),0,"R");
    	}
