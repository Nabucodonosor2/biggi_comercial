...
if($priv_cambio_precio){
	$temp->setVar("W_AUT_PRECIO_COT",'<input type="button" value="Usar precios desde Cotización" onclick="setWindowReturnValue(0); closeMe();" class="button">');
	...
	echo '<script type="text/javascript">alert(\'Atención: la Cotización Nº '.$cod_cotizacion.', esta fuera de la validez de oferta, por lo tanto, la Nota de Venta se creará utilizando precios actualizados.\n\nSi desea respetar los precios de la cotización, favor solicitar autorización en Administración BIGGI\');</script>';
	$temp->setVar("W_AUT_PRECIO_ACTUAL",'<input type="button" value="Usar precios actuales." onclick="okMe();" class="button">');
}
	//nueva variable para idicar si esta fuera de la validez de oferta
if ($validez_oferta == false)
	$temp->setVar("W_AUT_LABEL_OFERTA", 'Cotización fuera de validez por '.($result[0]['VALIDEZ'] * -1).' días.');
