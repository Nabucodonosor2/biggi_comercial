       ...
       break;
    	case "pesomedida":
				if($print_cot[2] == 'pdf')
					...
					$this->printcot_pesomedida_excel('' , $print_cot[3]);
      	break;
    	case "tecnica":
	...
	function printcot_ampliada_excel($con_logo) {
		...
		if($print_descto == 'item'){
			if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
				...
				$margen += 1;
				$worksheet->set_column(10, 10, 14);
				$worksheet->write(9, 10, "Desc. %".$PORC_DSCTO1, $titulo_item_border_all);
			}else if($PORC_DSCTO1 == 0 && $PORC_DSCTO2 <> 0){
				$margen += 1;
				$worksheet->set_column(10, 10, 14);
				$worksheet->write(9, 10, "Desc. Adic. %".$PORC_DSCTO2, $titulo_item_border_all);
			...
			$worksheet->write(9, $margen+10, "Total ".$SIMBOLO, $titulo_item_border_all);
		}else{
			$margen = 1;
			$worksheet->write(9, $margen+9, "Total ".$SIMBOLO, $titulo_item_border_all);
		}
		...
		for ($i=0 ; $i <count($result); $i++) {
			...
			if($print_descto == 'item'){
					if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
						// ENTRA ACA TOTAL 133861 | $i=0
						$monto_descuento1 = $PRECIO * ($PORC_DSCTO1/100);
						$worksheet->write(10+$i, 10, $monto_descuento1, $border_item_especial_total);
						$TOTAL = ($PRECIO - $monto_descuento1) * $CANTIDAD;
					}else if($PORC_DSCTO1 == 0 && $PORC_DSCTO2 <> 0){
						$monto_descuento2 = $PRECIO * ($PORC_DSCTO2/100);
						$worksheet->write(10+$i, 10, $monto_descuento2, $border_item_especial_total);
						$TOTAL = ($PRECIO - $monto_descuento2) * $CANTIDAD;
					}else if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 <> 0){
						$monto_descuento1 = $PRECIO * ($PORC_DSCTO1/100);
						$worksheet->write(10+$i, 10, $monto_descuento1, $border_item_especial_total);
						$monto_descuento2 = ($PRECIO - $monto_descuento1) * ($PORC_DSCTO2/100);
						$worksheet->write(10+$i, 11, $monto_descuento2, $border_item_especial_total);
						$TOTAL = ($PRECIO - $monto_descuento1 - $monto_descuento2) * $CANTIDAD;
					}else	
						$TOTAL = $result[$i]['TOTAL'];
					
		
					$worksheet->write(10+$i, 1, $ITEM, $titulo_item_border_all);
					
					if($COD_PRODUCTO == ''){
						$worksheet->write(10+$i, 3, $NOM_PRODUCTO, $border_item_especial_total);
						$worksheet->merge_cells(10, 3, 10, 7);
					}else{
						$worksheet->write(10+$i, 3, $NOM_PRODUCTO, $border_item_especial_total);
						$worksheet->merge_cells(10, 3, 10, 7);
					}
					...
					$worksheet->write(10+$i, 2, $COD_PRODUCTO, $titulo_item_border_all);
					$worksheet->write(10+$i, 8, $CANTIDAD, $border_item_especial_total);
					$worksheet->write(10+$i, 9, $PRECIO, $border_item_especial_total);
					$worksheet->write(10+$i, 10+$margen, $TOTAL, $border_item_especial_total);
			}else{
				if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
						// ENTRA ACA TOTAL 133861 | $i=0
						$monto_descuento1 = $PRECIO * ($PORC_DSCTO1/100);
						...
						$worksheet->write(10+$i, 8, $CANTIDAD, $border_item_especial_total);
						$worksheet->write(10+$i, 9, $PRECIO, $border_item_especial_total);
						$worksheet->write(10+$i, 9+$margen, $TOTAL, $border_item_especial_total);
			}	
			...
			for ($j=0 ; $j <count($result_2); $j++) {
				if($j==0){
					...
					if($print_descto == 'item'){
						$worksheet->write(11+$p+$j, 12,"", $border_item_left_bold);
					}
				}			
		...
		else if($MONTO_DSCTO2 > 0 && $MONTO_DSCTO1 == 0){
			if($print_descto == 'item'){
				$worksheet->write($row_position, 7, "Subtotal ", $border_item_left);
				$worksheet->write($row_position, 9, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position, 10, $SUBTOTAL, $monto_normal);			
