...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        if (!empty($this->dataArray)){
            ...
            $this->Image($imagen_fondo, -7, -11, 612, 792, '', '', '', false, 300, '', false, false, 0);
    		...
    		$this->Text(258, 13,utf8_encode('Comercial Biggi (Chile) S.A.'));
    		$this->Text(258, 27,utf8_encode('Av. Portugal 1726, Santiago - Chile'));
    		$this->Text(258, 41,utf8_encode('Fono: 56 224126200 - info@biggi.cl'));
    		$this->SetTextColor(0,0,127);
    		$this->SetFont('helvetica','',11);	
    		$this->Text(258, 59,utf8_encode('www.biggi.cl'));
    		$this->Line(602,0,602,792);
    		...
    		$this->SetXY(542, 34);
    		$this->Cell(55, 20,utf8_encode('Nº '.$result[0]['COD_COTIZACION']),0,0,'R');  
...
class wi_cotizacion extends wi_cotizacion_base {
	...
	*/
	...
	$sql = "SELECT			C.COD_COTIZACION,
		...
		$pdf->Text(23, 99,utf8_encode('Señores:'));
		...
		$pdf->SetFont('helvetica','B',9);
		$pdf->Text(23, 156,utf8_encode('Rut:'));
		$pdf->SetFont('helvetica','',9);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(55, 156,utf8_encode($RUT));
	
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',9);
		$pdf->Text(130, 156,utf8_encode('Fono:'));
		$pdf->SetFont('helvetica','',9);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(165, 156,utf8_encode($result[0]['TELEFONO_F']));


	    $pdf->SetFont('helvetica','B',10);
	    $pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->Text(423, 99,utf8_encode('Atención:'));
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->SetFont('helvetica','',9);	
		$pdf->SetXY(423, 114);
		$pdf->Cell(340, 20,utf8_encode($result[0]['NOM_PERSONA']),0,0,'L');


		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',9);
		$pdf->Text(423, 156,utf8_encode('Fono:'));
		$pdf->SetFont('helvetica','',9);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(423, 141,utf8_encode($result[0]['EMAIL']));
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(456, 156,utf8_encode($result[0]['TELEFONO']));

		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',10);
		$pdf->Text(23, 177,utf8_encode('Referencia:'));
		$pdf->SetFont('helvetica','',10);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(80, 177,utf8_encode($result[0]['REFERENCIA']));

		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',10);
		$pdf->Text(521, 177,utf8_encode('Vendedor:'));
		$pdf->SetTextColor(0,0,0);//AZUL RE
		$pdf->SetFont('helvetica','',10);
		$pdf->SetXY(571, 175);
		$pdf->Cell(29, 17,utf8_encode($result[0]['INI_USUARIO']),0,0,'R');
		

		$pdf->SetXY(20, 230); // pos para header de items
		$pdf->SetFont('helvetica','B',9);	
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFillColor(225, 224, 224);
		$pdf->Cell($pdf->w_item, 23.4,utf8_encode('Item'),1,0,'C',1);
		$pdf->Cell($pdf->w_producto, 23.4,utf8_encode('Producto'),1,0,'C',1);
		$pdf->Cell($pdf->w_modelo, 23.4,utf8_encode('Modelo'),1,0,'C',1);
		$pdf->Cell($pdf->w_cant, 23.4,utf8_encode('Cant.'),1,0,'C',1);
		$pdf->Cell($pdf->w_precio, 23.4,utf8_encode('Precio'),1,0,'C',1);
		$pdf->MultiCell($pdf->w_desc_10, 23.4,utf8_encode('Descuento 10%'),1,'C',1,0);
		$pdf->MultiCell($pdf->w_precio_desc, 23.4,utf8_encode('Precio c/desc.'),1,'C',1,0);
		$pdf->MultiCell($pdf->w_total, 23.4,utf8_encode('Total'),1,'C',1,1);
    	/** FIN SUB CABERCERA***/	
    	/** INICIO ITEMS **/
		$pdf->setY(253);
		for($i = 0;$i < count($result); $i++){
		    ...
		    if($pdf->GetY() > 590){
		        $pdf->AddPage();
		        $pdf->SetXY(20, 136.5); // pocision para header de items
		    ...
		    $pdf->SetX(20);
		    $pdf->MultiCell($pdf->w_item, $h_item,utf8_encode($result[$i]['ITEM']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_producto, $h_item,utf8_encode($result[$i]['NOM_PRODUCTO']),1,'L',false,0);
		    $pdf->MultiCell($pdf->w_modelo, $h_item,utf8_encode($result[$i]['COD_PRODUCTO']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_cant, $h_item,utf8_encode($result[$i]['CANTIDAD']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_precio, $h_item,utf8_encode($result[$i]['PRECIO']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_desc_10, $h_item,utf8_encode($result[$i]['PRECIO']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_precio_desc, $h_item,utf8_encode($result[$i]['PRECIO']),1,'C',false,0);
		    $pdf->MultiCell($pdf->w_total, $h_item,utf8_encode($result[$i]['TOTAL']),1,'C',false,1);
		    $pdf->SetX(20);
		    $pdf->MultiCell($pdf->w_item, $h_detalle,'',1,'C',false,0);
		    $pdf->MultiCell($pdf->w_producto, $h_detalle,utf8_encode($result[$i]['ATRIBUTO_PRODUCTO']),1,'L',false,0);
		    
		    $pdf->Image($ruta_img,$pdf->GetX()+6,$pdf->GetY()+6,128,148,'','','C',true,300,'',false,false,0,true,false,false,false,false);
		    $pdf->SetLineWidth(0.9);
		    $pdf->Line(285,$pdf->GetY()+150,425,$pdf->GetY()+150);
		    $x = 425;
		    $pdf->SetX($x);
		    $pdf->MultiCell($pdf->w_desc_10, $h_detalle,'',1,'C',false,0);
		    $pdf->MultiCell($pdf->w_precio_desc, $h_detalle,'',1,'C',false,0);
		    $pdf->MultiCell($pdf->w_total, $h_detalle,'',1,'C',false,1);
		}
		/** TERMINO ITEMS **/
		/** PIE PAGINA **/
		/*
		...
		*/
		...
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$factor_y = 550;
		$pdf->Text(30, $factor_y,utf8_encode('Condiciones Generales:'));
		
		$pdf->SetTextColor(0,0,127);
		$pdf->SetFont('helvetica','B',7);
		$pdf->Text(30, $factor_y+20,utf8_encode('Forma de Pago'));
		$pdf->Text(30, $factor_y+32,utf8_encode('Validez Oferta'));
		$pdf->Text(30, $factor_y+44,utf8_encode('Entrega'));
		$pdf->Text(30, $factor_y+56,utf8_encode('Embalaje'));
		$pdf->Text(30, $factor_y+68,utf8_encode('Flete'));
		$pdf->Text(30, $factor_y+80,utf8_encode('Instalación'));
		$pdf->Text(30, $factor_y+92,utf8_encode('Garantía'));
		$pdf->Text(30, $factor_y+116,utf8_encode('Equipos especiales'));

		$pdf->SetTextColor(0,0,0);
		$pdf->SetFont('helvetica','',7);
		$pdf->Text(117, $factor_y+20,utf8_encode(': '.$result[0]['NOM_FORMA_PAGO']));
		$pdf->Text(117, $factor_y+32,utf8_encode(': '.$result[0]['VALIDEZ_OFERTA'].' días'));
		$pdf->Text(117, $factor_y+44,utf8_encode(': '.$result[0]['ENTREGA']));
		$pdf->Text(117, $factor_y+56,utf8_encode(': '.$result[0]['NOM_EMBALAJE_COTIZACION']));
		$pdf->Text(117, $factor_y+68,utf8_encode(': '.$result[0]['NOM_FLETE_COTIZACION']));
		$pdf->Text(117, $factor_y+80,utf8_encode(': '.$result[0]['NOM_INSTALACION_COTIZACION']));

		$pdf->SetXY(117, $factor_y+92);
		$pdf->MultiCell(293,14,utf8_encode(': '.$result[0]['GARANTIA']), 0, 'L');

		$pdf->Text(117, $factor_y+116,utf8_encode(': '.$result[0]['EQUIPO_ESPECIAL']));
	
		$pdf->SetXY(30, $factor_y+142);
		$pdf->MultiCell(380,14,utf8_encode('                                   :'.$result[0]['OBS']), 0, 'L');
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',7);
		$pdf->Text(30, $factor_y+142,utf8_encode('Notas'));	
