...
class wi_cotizacion extends wi_cotizacion_base {
	...
	function wi_cotizacion($cod_item_menu) {	
		...
		$this->dws['dw_cotizacion']->add_control(new edit_text_hidden('PORC_DESC_PERMITIDO'));
		$this->dws['dw_cotizacion']->add_control(new edit_text_hidden('DESCTO_1_AUTORIZADO_H'));
		$this->dws['dw_cotizacion']->add_control(new edit_text_hidden('DESCTO_2_AUTORIZADO_H'));
		...
		if($priv <> 'E'){
			$this->dws['dw_cotizacion']->set_entrable('DESCTO_1_AUTORIZADO', false);
			$this->dws['dw_cotizacion']->set_entrable('DESCTO_2_AUTORIZADO', false);
		}else{
			$this->dws['dw_cotizacion']->set_entrable('DESCTO_1_AUTORIZADO', true);
			$this->dws['dw_cotizacion']->set_entrable('DESCTO_2_AUTORIZADO', true);
		}
	...
	function new_record() {
		...
		$this->dws['dw_cotizacion']->set_item(0, 'COD_FORMA_PAGO', 27);
	...
	function print_record() {
       ...
       break;
    	...
    	case "tecnica":
    		...
    		for ($i = 0; $i < $tope; $i++){
    			switch ($lista_tecnica[$i]) {
    				...
    				case "gas":
    					if($print_cot[2] == 'pdf')
								$this->printcot_tecnica_gas_pdf($print_cot[3] == 'logo', $print_cot[5]);
							else
								$this->printcot_tecnica_gas_excel($print_cot[3] == 'logo', $print_cot[5]);
    					break;
    				case "vapor":
    				 	if($print_cot[2] == 'pdf')
        ...
        break;
        case "test_resumen":
			$this->printcot_resumen_pdf_test('logo');
			
       	break;
       	case "test_ampliada":
			$this->printcot_ampliada_pdf_test('logo');
			
       	break;
		}
		session::un_set('PRINT_DESCUENTO');
		$this->redraw();
	}
	/*
	...
	*/
	...
	function printcot_resumen_pdf_test($con_logo) {
		$cod_cotizacion = $this->get_key();
		...
		$labels = array();
		$labels['strCOD_COTIZACION'] = $cod_cotizacion;
		$rpt= new reporte($sql, $this->root_dir.'appl/cotizacion/cot_resumen_.xml', $labels, "Cotización Resumen ".$cod_cotizacion.".pdf", $con_logo);
		*/
		
	}
	
	function printcot_ampliada_pdf_test($con_logo) {
	$cod_cotizacion = $this->get_key();
	...
	$sql = "SELECT			C.COD_COTIZACION,
    	...
    	// LOS DOS COMANDOS ANTERIORES DEBEN IR SIEMPRE ANTES DE {AddPage}
		$pdf->AddPage();
    	$pdf->SetCreator('Integrasystem');
    	$pdf->SetAuthor('Integrasystem');
    	$pdf->SetTitle('COTIZACION_AMPLIADA');
    	$pdf->SetSubject('AMPLIADA TEST');
    	$pdf->SetKeywords('COT_A');
    	$pdf->SetFooterMargin(0);
    	$pdf->SetAutoPageBreak(false, $margin=0);
    	$imagen_fondo = K_ROOT_DIR."/images_appl/COMERCIAL/logo_reporte_re.png";
		$pdf->Image($imagen_fondo,0, 0,612,792);
		
		$pagina = $pdf->PageNo();
		$pag_total = $pdf->getAliasNbPages();
		$pdf->SetFont('helvetica','',8);
		$pag_total = settype($pag_total,"string"); //PARA LIMPIAR LA VARIABLE $pag_total
		$pdf->SetXY(539, 66);
		$pdf->Cell(55, 20,utf8_encode('Pag.'.$pagina.'/'.$pag_total),0,0,'R');

		$pdf->SetTextColor(0,0,127);//AZUL RE
	    $pdf->SetFont('helvetica','B',15);	
		$pdf->Text(475, 30,utf8_encode('Cotización'));
		$pdf->Text(475, 47,utf8_encode('Nº '.$cod_cotizacion));


		$pdf->SetTextColor(0,0,0);//NEGRO RE
	    $pdf->SetFont('helvetica','',9);	
		$pdf->Text(258, 30,utf8_encode('Comercial Biggi (Chile) S.A.'));
		$pdf->Text(258, 42,utf8_encode('Av. Portugal 1726, Santiago - Chile'));
		$pdf->Text(258, 54,utf8_encode('Fono: 56 224126200 - info@biggi.cl'));
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','',13);	
		$pdf->Text(258, 70,utf8_encode('www.biggi.cl'));

		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','',11);	
		$pdf->SetXY(419, 83);
		$pdf->Cell(175, 20,utf8_encode('Santiago, '.$result[0]['FECHA_COTIZACION']),0,0,'R');
		
		//$pdf->SetTextColor(0,0,0);//negro
	    //$pdf->Text(390, 130,utf8_encode('Vendedor: '.$result[0]['INI_USUARIO']));
	    
	    $pdf->SetFont('helvetica','B',11);
	    $pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->Text(30, 110,utf8_encode('Señores:'));
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->SetFont('helvetica','',11);	
		$pdf->SetXY(30, 125);
		$pdf->Cell(340, 20,utf8_encode(substr($result[0]['NOM_EMPRESA'],0,57)),0,0,'L');  //FALTA TRUNCAR EL LARGO DEL NOM EMPRESA

		$pdf->SetXY(30, 140);
		$pdf->MultiCell(340,14,utf8_encode($result[0]['DIRECCION'].' - '.$result[0]['NOM_COMUNA'].' - '.$result[0]['NOM_CIUDAD'].'.'), 0, 'L');
		...
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',11);
		$pdf->Text(30, 167,utf8_encode('Rut:'));
		$pdf->SetFont('helvetica','',11);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(55, 167,utf8_encode($RUT));
	
		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',11);
		$pdf->Text(130, 167,utf8_encode('Fono:'));
		$pdf->SetFont('helvetica','',11);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(165, 167,utf8_encode($result[0]['TELEFONO_F']));


	    $pdf->SetFont('helvetica','B',11);
	    $pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->Text(410, 110,utf8_encode('Atención:'));
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->SetFont('helvetica','',11);	
		$pdf->SetXY(410, 125);
		$pdf->Cell(340, 20,utf8_encode($result[0]['NOM_PERSONA']),0,0,'L');  //FALTA TRUNCAR EL LARGO DEL NOM EMPRESA


		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',11);
		$pdf->Text(410, 167,utf8_encode('Fono:'));
		$pdf->SetFont('helvetica','',11);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(410, 152,utf8_encode($result[0]['EMAIL']));
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(445, 167,utf8_encode($result[0]['TELEFONO_F']));

		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',11);
		$pdf->Text(30, 195,utf8_encode('Referencia:'));
		$pdf->SetFont('helvetica','',11);
		$pdf->SetTextColor(0,0,0);//NEGRO RE
		$pdf->Text(92, 195,utf8_encode($result[0]['REFERENCIA']));

		/*	
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(20, 170,utf8_encode('Dirección     :'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(100, 170,utf8_encode($result[0]['DIRECCION']));
		
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(20, 190,utf8_encode('Comuna      :'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(100, 190,utf8_encode($result[0]['NOM_COMUNA']));
		
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(190, 190,utf8_encode('Ciudad:'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(230, 190,utf8_encode($result[0]['NOM_CIUDAD']));
		
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(300, 190,utf8_encode('Fono:'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(330, 190,utf8_encode($result[0]['TELEFONO_F']));
		
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(390, 190,utf8_encode('Fax:'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(410, 190,utf8_encode($result[0]['FAX_F']));
		
		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(20, 210,utf8_encode('Referencia    :'));
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->Text(100, 210,utf8_encode($result[0]['REFERENCIA']));
		*/

		$pdf->SetTextColor(0,0,255);//azul
		$pdf->Text(20, 250,utf8_encode('INI ITEMS    '));
		$pdf->Text(20, 500,utf8_encode('FIN ITEMS    '));
		

		$pdf->SetTextColor(0,0,127);//AZUL RE
		$pdf->SetFont('helvetica','B',11);
		$pdf->Text(20, 630,utf8_encode('Condiciones Generales:'));
		$pdf->Line(20,645,150,645);
		
		$pdf->Line(400,645,573,645);
		$pdf->Text(403, 650,utf8_encode('COMERCIAL BIGGI (CHILE) S.A.'));
		$pdf->SetXY(400, 664);
		$pdf->Cell(170, 20,utf8_encode($result[0]['NOM_USUARIO']),0,0,'C');
		$pdf->SetXY(400, 678);
		$pdf->Cell(170, 20,utf8_encode($result[0]['MAIL_U']),0,0,'C');
		$pdf->SetXY(400, 692);
		$pdf->Cell(170, 20,utf8_encode($result[0]['FONO_U'].'-'.$result[0]['FONO_U']),0,0,'C');
		
		$pdf->SetFont('helvetica','B',9);
		$pdf->Text(20, 650,utf8_encode('Forma de Pago:'));
		$pdf->Text(20, 660,utf8_encode('Validez Oferta:'));
		$pdf->Text(20, 670,utf8_encode('Entrega:'));
		
		$pdf->SetTextColor(0,0,0);//negro
		$pdf->SetFont('helvetica','',9);
		
		$pdf->Text(100, 650,utf8_encode($result[0]['NOM_FORMA_PAGO']));
		$pdf->Text(100, 660,utf8_encode($result[0]['VALIDEZ_OFERTA'].' días'));
		$pdf->Text(100, 670,utf8_encode($result[0]['ENTREGA']));
		

$pdf->SetFont('helvetica','',9);

$pdf->Text(10, 10, utf8_encode('10'));
$pdf->Text(30, 10, utf8_encode('30'));
$pdf->Text(50, 10, utf8_encode('50'));
$pdf->Text(70, 10, utf8_encode('70'));
$pdf->Text(90, 10, utf8_encode('90'));
$pdf->Text(110, 10, utf8_encode('110'));
$pdf->Text(130, 10, utf8_encode('130'));
$pdf->Text(150, 10, utf8_encode('150'));
$pdf->Text(170, 10, utf8_encode('170'));
$pdf->Text(190, 10, utf8_encode('190'));
$pdf->Text(210, 10, utf8_encode('210'));
$pdf->Text(230, 10, utf8_encode('230'));
$pdf->Text(250, 10, utf8_encode('250'));
$pdf->Text(270, 10, utf8_encode('270'));
$pdf->Text(290, 10, utf8_encode('290'));
$pdf->Text(310, 10, utf8_encode('310'));
$pdf->Text(330, 10, utf8_encode('330'));
$pdf->Text(350, 10, utf8_encode('350'));
$pdf->Text(370, 10, utf8_encode('370'));
$pdf->Text(390, 10, utf8_encode('390'));
$pdf->Text(410, 10, utf8_encode('410'));
$pdf->Text(430, 10, utf8_encode('430'));
$pdf->Text(450, 10, utf8_encode('450'));
$pdf->Text(470, 10, utf8_encode('470'));
$pdf->Text(490, 10, utf8_encode('490'));
$pdf->Text(510, 10, utf8_encode('510'));
$pdf->Text(530, 10, utf8_encode('530'));
$pdf->Text(550, 10, utf8_encode('550'));
$pdf->Text(570, 10, utf8_encode('570'));

/*
$pdf->Text(50, 10,utf8_encode('10_'));
$pdf->Text(50, 20,utf8_encode('20_'));
$pdf->Text(50, 30,utf8_encode('30_'));
$pdf->Text(50, 40,utf8_encode('40_'));
$pdf->Text(50, 50,utf8_encode('50_'));
$pdf->Text(50, 60,utf8_encode('60_'));
$pdf->Text(50, 70,utf8_encode('70_'));
$pdf->Text(50, 80,utf8_encode('80_'));
$pdf->Text(50, 90,utf8_encode('90_'));
$pdf->Text(50, 100,utf8_encode('100_'));
$pdf->Text(50, 110,utf8_encode('110_'));
$pdf->Text(50, 120,utf8_encode('120_'));
$pdf->Text(50, 130,utf8_encode('130_'));
$pdf->Text(50, 140,utf8_encode('140_'));
$pdf->Text(50, 150,utf8_encode('150_'));
$pdf->Text(50, 160,utf8_encode('160_'));
$pdf->Text(50, 170,utf8_encode('170_'));
$pdf->Text(50, 180,utf8_encode('180_'));
$pdf->Text(50, 190,utf8_encode('190_'));
$pdf->Text(50, 200,utf8_encode('200_'));
$pdf->Text(50, 210,utf8_encode('210_'));
$pdf->Text(50, 220,utf8_encode('220_'));
$pdf->Text(50, 230,utf8_encode('230_'));
$pdf->Text(50, 240,utf8_encode('240_'));
$pdf->Text(50, 250,utf8_encode('250_'));
$pdf->Text(50, 260,utf8_encode('260_'));
$pdf->Text(50, 270,utf8_encode('270_'));
$pdf->Text(50, 280,utf8_encode('280_'));
$pdf->Text(50, 290,utf8_encode('290_'));
$pdf->Text(50, 300,utf8_encode('300_'));
$pdf->Text(50, 310,utf8_encode('310_'));
$pdf->Text(50, 320,utf8_encode('320_'));
$pdf->Text(50, 330,utf8_encode('330_'));
$pdf->Text(50, 340,utf8_encode('340_'));
$pdf->Text(50, 350,utf8_encode('350_'));
$pdf->Text(50, 360,utf8_encode('360_'));
$pdf->Text(50, 370,utf8_encode('370_'));
$pdf->Text(50, 380,utf8_encode('380_'));
$pdf->Text(50, 390,utf8_encode('390_'));
$pdf->Text(50, 400,utf8_encode('400_'));
$pdf->Text(50, 410,utf8_encode('410_'));
$pdf->Text(50, 420,utf8_encode('420_'));
$pdf->Text(50, 430,utf8_encode('430_'));
$pdf->Text(50, 440,utf8_encode('440_'));
$pdf->Text(50, 450,utf8_encode('450_'));
$pdf->Text(50, 460,utf8_encode('460_'));
$pdf->Text(50, 470,utf8_encode('470_'));
$pdf->Text(50, 480,utf8_encode('480_'));
$pdf->Text(50, 490,utf8_encode('490_'));
$pdf->Text(50, 500,utf8_encode('500_'));
$pdf->Text(50, 510,utf8_encode('510_'));
$pdf->Text(50, 520,utf8_encode('520_'));
$pdf->Text(50, 530,utf8_encode('530_'));
$pdf->Text(50, 540,utf8_encode('540_'));
$pdf->Text(50, 550,utf8_encode('550_'));
$pdf->Text(50, 560,utf8_encode('560_'));
$pdf->Text(50, 570,utf8_encode('570_'));
$pdf->Text(50, 580,utf8_encode('580_'));
$pdf->Text(50, 590,utf8_encode('590_'));
$pdf->Text(50, 600,utf8_encode('600_'));
$pdf->Text(50, 610,utf8_encode('610_'));
$pdf->Text(50, 620,utf8_encode('620_'));
$pdf->Text(50, 630,utf8_encode('630_'));
$pdf->Text(50, 640,utf8_encode('640_'));
$pdf->Text(50, 650,utf8_encode('650_'));
$pdf->Text(50, 660,utf8_encode('660_'));
$pdf->Text(50, 670,utf8_encode('670_'));
$pdf->Text(50, 680,utf8_encode('680_'));
$pdf->Text(50, 690,utf8_encode('690_'));
$pdf->Text(50, 700,utf8_encode('700_'));
$pdf->Text(50, 710,utf8_encode('710_'));
$pdf->Text(50, 720,utf8_encode('720_'));
$pdf->Text(50, 730,utf8_encode('730_'));
$pdf->Text(50, 740,utf8_encode('740_'));
$pdf->Text(50, 750,utf8_encode('750_'));
$pdf->Text(50, 760,utf8_encode('760_'));
$pdf->Text(50, 770,utf8_encode('770_'));
$pdf->Text(50, 780,utf8_encode('780_'));
$pdf->Text(50, 790,utf8_encode('790_'));
$pdf->Text(50, 800,utf8_encode('800_'));
$pdf->Text(50, 810,utf8_encode('810_'));
$pdf->Text(50, 820,utf8_encode('820_'));
$pdf->Text(50, 830,utf8_encode('830_'));
$pdf->Text(50, 840,utf8_encode('840_'));
$pdf->Text(50, 850,utf8_encode('850_'));
$pdf->Text(50, 860,utf8_encode('860_'));
$pdf->Text(50, 870,utf8_encode('870_'));
$pdf->Text(50, 880,utf8_encode('880_'));
$pdf->Text(50, 890,utf8_encode('890_'));
$pdf->Text(50, 900,utf8_encode('900_'));
$pdf->Text(50, 910,utf8_encode('910_'));
$pdf->Text(50, 920,utf8_encode('920_'));
$pdf->Text(50, 930,utf8_encode('930_'));
$pdf->Text(50, 940,utf8_encode('940_'));
$pdf->Text(50, 950,utf8_encode('950_'));
$pdf->Text(50, 960,utf8_encode('960_'));
$pdf->Text(50, 970,utf8_encode('970_'));
$pdf->Text(50, 980,utf8_encode('980_'));
$pdf->Text(50, 990,utf8_encode('990_'));
$pdf->Text(50, 1000,utf8_encode('1000_'));
*/		
$pdf->Output("cotizacion.pdf", 'D');
	...
	function printcot_cad(){
		...
		$text_border_top->set_top(1);
		
		$text_border_all_center =& $workbook->addformat();
		$text_border_all_center->copy($text_border_all);
		$text_border_all_center->set_align('center');
		
		$text_border_all_left =& $workbook->addformat();
		$text_border_all_left->copy($text_border_all);
		$text_border_all_left->set_align('left');
		
		$text_border_all_right =& $workbook->addformat();
		$text_border_all_right->copy($text_border_all);
		$text_border_all_right->set_align('right');
		
		$text_border_all_title =& $workbook->addformat();
		$text_border_all_title->copy($text_border_all);
		$text_border_all_title->set_align('center');
		
		$text_border_all_title_bold =& $workbook->addformat();
		$text_border_all_title_bold->copy($text_border_all_title);
		$text_border_all_title_bold->set_bold(1);
		
		$text_border_all_title_bold3 =& $workbook->addformat();
		$text_border_all_title_bold3->copy($text_border_all_title_bold);
		$text_border_all_title_bold3->set_top(1);
		
		$text_border_tlr_title_bold  =& $workbook->addformat();
		$text_border_tlr_title_bold->copy($text_border_all_title_bold);
		$text_border_tlr_title_bold->set_bottom(1);
		
		$text_border_all_title2 =& $workbook->addformat();
		$text_border_all_title2->copy($text_border_all);
		$text_border_all_title2->set_bold(1);
		
		$worksheet->write(2, 6, "AGUA", $text_border_all_title_bold3);
		$worksheet->write(2, 7, "", $text_border_all_title_bold3);
		$worksheet->write(2, 8, "ELECTRICIDAD", $text_border_all_title_bold3);
		$worksheet->write(2, 9, "", $text_border_all_title_bold3);
		$worksheet->write(2, 10, "GAS", $text_border_all_title_bold3);
		
		$worksheet->merge_cells(2, 6, 2, 7);
		$worksheet->merge_cells(2, 8, 2, 9);
		
		$worksheet->write(3, 1, "ITEM", $text_border_all_title_bold3);
		$worksheet->write(3, 2, "DESCRIPCION", $text_border_all_title_bold3);
		$worksheet->write(3, 3, "MODELO", $text_border_all_title_bold3);
		$worksheet->write(3, 4, "FRENTE x PROF x ALTO", $text_border_all_title_bold3);
		$worksheet->write(3, 5, "CANT.", $text_border_all_title_bold3);
		$worksheet->write(3, 6, "GRIFERIA", $text_border_all_title_bold3);
		$worksheet->write(3, 7, "DESAGUE", $text_border_all_title_bold3);
		$worksheet->write(3, 8, "CONSUMO", $text_border_all_title_bold3);
		$worksheet->write(3, 9, "VOLTAJE", $text_border_all_title_bold3);
		$worksheet->write(3, 10, "CONSUMO", $text_border_all_title_bold3);
		
		$worksheet->write(4, 1, "", $text_border_tlr_title_bold);
		$worksheet->write(4, 2, "", $text_border_tlr_title_bold);
		$worksheet->write(4, 3, "", $text_border_tlr_title_bold);
		$worksheet->write(4, 4, "CMS", $text_border_tlr_title_bold);
		$worksheet->write(4, 5, "", $text_border_tlr_title_bold);
		$worksheet->write(4, 6, "Ø", $text_border_tlr_title_bold);
		$worksheet->write(4, 7, "Ø", $text_border_tlr_title_bold);
		$worksheet->write(4, 8, "Kw/hr", $text_border_tlr_title_bold);
		$worksheet->write(4, 9, "V", $text_border_tlr_title_bold);
		$worksheet->write(4, 10, "Kw/hr", $text_border_tlr_title_bold);
		
		$l = 1;
		while($my_row = $db->get_row()){
			if($my_row['COD_PRODUCTO'] == 'T'){
				$worksheet->write(4+$l, 1, $my_row['ITEM'], $text_border_all_title_bold);
				$worksheet->write(4+$l, 2, $my_row['NOM_PRODUCTO'], $text_border_all_title2);
				$worksheet->write(4+$l, 3, "", $text_border_all_left);
				$worksheet->write(4+$l, 4, "", $text_border_all_center);
				$worksheet->write(4+$l, 5, "", $text_border_all_right);
				$worksheet->write(4+$l, 6, "", $text_border_all_center);
				$worksheet->write(4+$l, 7, "", $text_border_all_center);
				$worksheet->write(4+$l, 8, "", $text_border_all_center);
				$worksheet->write(4+$l, 9, "", $text_border_all_center);
				$worksheet->write(4+$l, 10, "", $text_border_all_center);
				$l++;
				continue;
			}else{
				$worksheet->write(4+$l, 1, $my_row['ITEM'], $text_border_all_center);
				$worksheet->write(4+$l, 2, $my_row['NOM_PRODUCTO'], $text_border_all_left);
			}
			
			$worksheet->write(4+$l, 3, $my_row['COD_PRODUCTO'], $text_border_all_left);
			...
			$worksheet->write(4+$l, 6, $GRIFERIA, $text_border_all_center);
			$worksheet->write(4+$l, 7, $DESAGUE, $text_border_all_center);
			$worksheet->write(4+$l, 8, $POTENCIA, $text_border_all_center);
			$worksheet->write(4+$l, 9, $VOLTAJE, $text_border_all_center);
			$worksheet->write(4+$l, 10, $POTENCIA_KW, $text_border_all_center);
			$l++;
		}
		
		$worksheet->write(4+$l, 1, "", $text_border_top);
		$worksheet->write(4+$l, 2, "", $text_border_top);
		$worksheet->write(4+$l, 3, "", $text_border_top);
		$worksheet->write(4+$l, 4, "", $text_border_top);
		$worksheet->write(4+$l, 5, "", $text_border_top);
		$worksheet->write(4+$l, 6, "", $text_border_top);
		$worksheet->write(4+$l, 7, "", $text_border_top);
		$worksheet->write(4+$l, 8, "", $text_border_top);
		$worksheet->write(4+$l, 9, "", $text_border_top);
		$worksheet->write(4+$l, 10, "", $text_border_top);
		
		$workbook->close();
		
		header("Content-Type: application/x-msexcel; name=\"COTIZACION $cod_cotizacion CAD.xls\"");
		header("Content-Disposition: inline; filename=\"COTIZACION $cod_cotizacion CAD.xls\"");
		$fh=fopen($fname, "rb");
		fpassthru($fh);
		unlink($fname);
	}
	
	function print_resumen_interno(){
		require_once(dirname(__FILE__)."/../../../../../commonlib/trunk/php/php_writeexcel-0.3.0/class.writeexcel_workbook.inc.php");
		require_once(dirname(__FILE__)."/../../../../../commonlib/trunk/php/php_writeexcel-0.3.0/class.writeexcel_worksheet.inc.php");
		...
		$text_cantidad =& $workbook->addformat();
		$text_cantidad->copy($text);
		$text_cantidad->set_num_format('#.0');
		$text_cantidad->set_right(2);
		$text_cantidad->set_left(2);
		
		$text_cantidad2 =& $workbook->addformat();
		$text_cantidad2->copy($text);
		$text_cantidad2->set_num_format('#,##0');
		$text_cantidad2->set_right(2);
		$text_cantidad2->set_left(2);
		
		$text_black=& $workbook->addformat();
		$text_black->copy($text);
		$text_black->set_bold(1);
		
		$text_title =& $workbook->addformat();
		$text_title->copy($text_black);
		$text_title->set_border_color('black');
		$text_title->set_color('blue_0x20');
		
		$text_title_footer =& $workbook->addformat();
		$text_title_footer->copy($text_title);
		$text_title_footer->set_size(8);
		
		$text_title_center =& $workbook->addformat();
		$text_title_center->copy($text_title);
		$text_title_center->set_align('center');
		$text_title_center->set_top(2);
		$text_title_center->set_bottom(2);
		$text_title_center->set_right(2);
		$text_title_center->set_left(2);
		
		$text_title_center_no_b =& $workbook->addformat();
		$text_title_center_no_b->copy($text_title);
		$text_title_center_no_b->set_align('center');
		
		$text_title_center_pro =& $workbook->addformat();
		$text_title_center_pro->copy($text_title);
		$text_title_center_pro->set_align('center');
		$text_title_center_pro->set_top(2);
		$text_title_center_pro->set_bottom(2);
		
		$text_title_right =& $workbook->addformat();
		$text_title_right->copy($text_title);
		$text_title_right->set_align('right');
		$text_title_right->set_size(13);
		
		$text_black_cell_rl =& $workbook->addformat();
		$text_black_cell_rl->copy($text_black);
		$text_black_cell_rl->set_right(2);
		$text_black_cell_rl->set_left(2);
		
		$text_cell_rl =& $workbook->addformat();
		$text_cell_rl->copy($text);
		$text_cell_rl->set_right(2);
		$text_cell_rl->set_left(2);
		
		$border_top =& $workbook->addformat();
		$border_top->copy($text);
		$border_top->set_top(2);
		
		$border_bottom =& $workbook->addformat();
		$border_bottom->copy($text);
		$border_bottom->set_bottom(2);
		
		$worksheet->write(1, 1, "Santiago,".$FECHA_ACTUAL, $text_title);
		$worksheet->write(1, 9, "COTIZACION Nº".$this->get_key(), $text_title_right);
		
		$worksheet->write(3, 1, "Referencia", $text_title);
		$worksheet->write(3, 3, $REFERENCIA, $text);
		
		$worksheet->write(5, 1, "Ítem", $text_title_center);
		$worksheet->write(5, 2, "", $text_title_center_pro);
		$worksheet->write(5, 3, "Producto", $text_title_center_pro);
		$worksheet->write(5, 4, "", $text_title_center_pro);
		$worksheet->write(5, 5, "", $text_title_center_pro);
		$worksheet->write(5, 6, "Modelo", $text_title_center);
		$worksheet->write(5, 7, "Cantidad", $text_title_center);
		$worksheet->write(5, 8, "Precio $", $text_title_center);
		$worksheet->write(5, 9, "Total $", $text_title_center);
		
		$l = 0;
		while($my_row = $db->get_row()){
			if($my_row['COD_PRODUCTO'] == 'T'){
				$worksheet->write(6+$l, 1, $my_row['ITEM'], $text_black_cell_rl);
				$worksheet->write(6+$l, 2, $my_row['NOM_PRODUCTO'], $text_black_cell_rl);
				$worksheet->write(6+$l, 6, "", $text_black_cell_rl);
				$worksheet->write(6+$l, 7, "", $text_black_cell_rl);
				$worksheet->write(6+$l, 8, "", $text_black_cell_rl);
				$worksheet->write(6+$l, 9, "", $text_black_cell_rl);
				$l++;
				continue;
			}else{
				$worksheet->write(6+$l, 1, $my_row['ITEM'], $text_cell_rl);
				$worksheet->write(6+$l, 2, $my_row['NOM_PRODUCTO'], $text_cell_rl);
				$worksheet->write(6+$l, 7, $my_row['CANTIDAD'], $text_cantidad);
				$worksheet->write(6+$l, 8, 0, $text_cantidad2);
				$worksheet->write(6+$l, 9, 0, $text_cantidad2);
			}
			
			$worksheet->write(6+$l, 6, $my_row['COD_PRODUCTO'], $text_cell_rl);
			$l++;
		}
		
		$worksheet->write(6+$l, 1, "", $border_top);
		$worksheet->write(6+$l, 2, "", $border_top);
		$worksheet->write(6+$l, 3, "", $border_top);
		$worksheet->write(6+$l, 4, "", $border_top);
		$worksheet->write(6+$l, 5, "", $border_top);
		$worksheet->write(6+$l, 6, "", $border_top);
		$worksheet->write(6+$l, 7, "", $border_top);
		$worksheet->write(6+$l, 8, "", $border_top);
		$worksheet->write(6+$l, 9, "", $border_top);
		
		$worksheet->write(11+$l, 1, "Notas", $text_title);
		
		if($OBS<>'' && $OBS<>'NULL'){
			$worksheet->write($l+12, 1, $OBS, $text_normal_justify);
			$worksheet->merge_cells($l+12, 1, $l+18, 5);
			
			$worksheet->write($l+20, 6, "", $border_bottom);
			$worksheet->write($l+20, 7, "", $border_bottom);
			$worksheet->write($l+20, 8, "", $border_bottom);
			$worksheet->write($l+20, 9, "", $border_bottom);
			$worksheet->write($l+20, 10, "", $border_bottom);
			
			$worksheet->write($l+21, 8, $NOM_EMPRESA_EMISOR, $text_title_center_no_b);
			$worksheet->write($l+22, 8, $NOM_USUARIO, $text_title_center_no_b);
			$worksheet->write($l+23, 8, $MAIL_U, $text_title_center_no_b);
			$worksheet->write($l+24, 8, $FONO_U."-".$CEL_U, $text_title_center_no_b);
		
			$worksheet->write($l+27, 1, $NOM_EMPRESA_EMISOR." - RUT: ".$RUT_EMPRESA." - ".$DIR_EMPRESA." - ".$CIUDAD_EMPRESA." - ".$PAIS_EMPRESA." - ".$TEL_EMPRESA." - ".$FAX_EMPRESA, $text_title_footer);
			$worksheet->write($l+28, 5, $MAIL_EMPRESA." - ".$SITIO_WEB_EMPRESA, $text_title_footer);
		}else{
			$worksheet->write($l+12, 1, $OBS, $text);
			
			$worksheet->write($l+13, 6, "", $border_bottom);
			$worksheet->write($l+13, 7, "", $border_bottom);
			$worksheet->write($l+13, 8, "", $border_bottom);
			$worksheet->write($l+13, 9, "", $border_bottom);
			$worksheet->write($l+13, 10, "", $border_bottom);
			
			$worksheet->write($l+14, 8, $NOM_EMPRESA_EMISOR, $text_title_center_no_b);
			$worksheet->write($l+15, 8, $NOM_USUARIO, $text_title_center_no_b);
			$worksheet->write($l+16, 8, $MAIL_U, $text_title_center_no_b);
			$worksheet->write($l+17, 8, $FONO_U."-".$CEL_U, $text_title_center_no_b);
	
			$worksheet->write($l+20, 1, $NOM_EMPRESA_EMISOR." - RUT: ".$RUT_EMPRESA." - ".$DIR_EMPRESA." - ".$CIUDAD_EMPRESA." - ".$PAIS_EMPRESA." - ".$TEL_EMPRESA." - ".$FAX_EMPRESA, $text_title_footer);
			$worksheet->write($l+21, 5, $MAIL_EMPRESA." - ".$SITIO_WEB_EMPRESA, $text_title_footer);
		}
		
		
		$workbook->close();
		
		header("Content-Type: application/x-msexcel; name=\"COTIZACION $cod_cotizacion.xls\"");
		header("Content-Disposition: inline; filename=\"COTIZACION $cod_cotizacion.xls\"");
		$fh=fopen($fname, "rb");
		fpassthru($fh);
		unlink($fname);
	}
	...
	*/
	...
	function printcot_tecnica_gas_pdf($con_logo, $con_total_consumo) {
		$cod_cotizacion = $this->get_key();
		...
		$labels = array();
		$labels['strCOD_COTIZACION'] = $cod_cotizacion;
		$labels['strTOTAL_CONSUMO_GAS'] = $result[0]['TOTAL_CONSUMO_GAS'];
		$labels['strCON_TOTAL_CONSUMO'] = $con_total_consumo;
	...
	function printcot_resumen_excel() {
		...
		$EQUIPO_ESPECIAL = $result[0]['EQUIPO_ESPECIAL'];
		$OBS = $result[0]['OBS'];		
		$NOM_USUARIO = $result[0]['NOM_USUARIO'];
		$MAIL_U = $result[0]['MAIL_U'];
		$FONO_U = $result[0]['FONO_U'];
		...
		if($print_descto == 'item'){
			if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
				...
				$worksheet->write(9, 10, "Precio c/Desc.", $titulo_item_border_all);
				...
				$worksheet->write(9, 11, "Precio c/Desc.", $titulo_item_border_all);
			}
			...
			$worksheet->write(10, $margen+8, " ", $border_item_top);
		}
		...
		for ($i=0 ; $i <count($result); $i++) {
			...
			if($print_descto == 'item'){
				if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
					...
					if($monto_descuento1 <> 0){
					   $worksheet->write(10+$i, 9, $monto_descuento1, $monto_normal);
					   $precio_con_desc = $PRECIO - $monto_descuento1;
					   $worksheet->write(10+$i, 10, $precio_con_desc, $monto_normal);
					   $TOTAL = ($PRECIO - $monto_descuento1) * $CANTIDAD;
					}else{
					     $worksheet->write(10+$i, 9, '', $monto_normal);
					     $worksheet->write(10+$i, 10, '', $monto_normal);
					     $TOTAL = '';
					}
					...
					if($monto_descuento2 <> 0){
					    $worksheet->write(10+$i, 9, $monto_descuento2, $monto_normal);
					
    					$precio_con_desc = $PRECIO - $monto_descuento2;
    					$worksheet->write(10+$i, 10, $precio_con_desc, $monto_normal);
					...
					if($monto_descuento1 <> 0){
    					...
    					$precio_con_desc = $PRECIO - $monto_descuento1 - $monto_descuento2;
    					$worksheet->write(10+$i, 11, $precio_con_desc, $monto_normal);
    					
    					$TOTAL = ($PRECIO - $monto_descuento1 - $monto_descuento2) * $CANTIDAD;
					}else{
					    $worksheet->write(10+$i, 9, '', $monto_normal);
					    $worksheet->write(10+$i, 10, '', $monto_normal);
					    $worksheet->write(10+$i, 11, '', $monto_normal);
					    $TOTAL = '';
					}
		...
		if($print_descto == 'item'){
			...
			if($PORC_DSCTO1 <> 0 && $PORC_DSCTO2 == 0){
    			...
    			$worksheet->write(10+$i, 9, " ", $border_item_top);	
    			$worksheet->write(10+$i, 10, " ", $border_item_top);
    			$worksheet->write(10+$i, 11, " ", $border_item_top);	
    		}
		}else
		    $worksheet->write(10+$i, 9, " ", $border_item_top);
		...
		if($MONTO_DSCTO1 > 0 && $MONTO_DSCTO2 > 0){
			...
			if($print_descto == 'total'){
			    ...
			    $worksheet->write($row_position+1, 6+$margen_dscto, "Descuento ", $border_item_left);
				$worksheet->write($row_position+1, 8+$margen_dscto, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position+1, 9+$margen_dscto, $FINAL, $monto_normal);
				$row_position+=1;
		...
		$worksheet->write($row_position+8, 1, "Forma de Pago", $text_blue_bold_left);
		...
		$worksheet->write($row_position+15, 1, "Equipos especiales", $text_blue_bold_left);
		$worksheet->write($row_position+15, 3, $EQUIPO_ESPECIAL, $text_normal_left);
		
		$worksheet->write($row_position+16, 1, "Notas", $text_blue_bold_left);
		
		if($OBS<>'' && $OBS<>'NULL'){
			$worksheet->write($row_position+17, 1, $OBS, $text_normal_justify);
			$worksheet->merge_cells($row_position+17, 1, $row_position+23, 5);
		...
		{
			$worksheet->write($row_position+17, 1, $OBS, $text_normal_left);
	...
	function printcot_ampliada_excel($con_logo){
		...
		if($print_descto=='item'){
			if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0)){
				...
				$worksheet->write(9, 11, "Precio c/Desc.", $titulo_item_border_all);
				$worksheet->write(9, 12, "Total", $titulo_item_border_all);
				...
				$worksheet->write(9, 12, "Precio c/Desc.", $titulo_item_border_all);
				$worksheet->write(9, 13, "Total".$SIMBOLO, $titulo_item_border_all);
			}else{
		...
		for ($i=0 ; $i <count($result); $i++) {
			...
			if($print_descto=='item'){
				if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0) &($PRECIO>0)){
					...
					$worksheet->write(10+$p, 11,$PRECIO - ($PRECIO * ($PORC_DSCTO1/100)), $border_item_especial_total);
					$worksheet->write(10+$p, 12,($PRECIO - ($PRECIO * ($PORC_DSCTO1/100))) * $CANTIDAD, $border_item_especial_total);		
				}else if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0) &($PRECIO==0)){
				    $worksheet->write(10+$p, 8,'', $border_item_especial_total);
					$worksheet->write(10+$p, 9,'', $border_item_especial_total);
					$worksheet->write(10+$p, 10,'', $border_item_especial_total);
					$worksheet->write(10+$p, 11,'', $border_item_especial_total);
					$worksheet->write(10+$p, 12,'', $border_item_especial_total);
				}else if(($result[0]['MONTO_DSCTO2']>0)&($result[0]['MONTO_DSCTO1']==0) &($PRECIO>0)){
					$worksheet->write(10+$p, 8,$CANTIDAD, $border_item_especial_total);
					$worksheet->write(10+$p, 9,$PRECIO, $border_item_especial_total);
					$worksheet->write(10+$p, 10,$PRECIO * ($PORC_DSCTO2/100), $border_item_especial_total);
					$worksheet->write(10+$p, 11,$PRECIO - ($PRECIO * ($PORC_DSCTO2/100)), $border_item_especial_total);
					$worksheet->write(10+$p, 12,($PRECIO - ($PRECIO * ($PORC_DSCTO2/100))) * $CANTIDAD, $border_item_especial_total);		
				}else if(($result[0]['MONTO_DSCTO2']>0)&($result[0]['MONTO_DSCTO1']==0) &($PRECIO==0)){
				    $worksheet->write(10+$p, 8,'', $border_item_especial_total);
					$worksheet->write(10+$p, 9,'', $border_item_especial_total);
					$worksheet->write(10+$p, 10,'', $border_item_especial_total);
					...
					$worksheet->write(10+$p, 12,$PRECIO - ($PRECIO * ($PORC_DSCTO2/100)) - ($PRECIO * ($PORC_DSCTO1/100)), $border_item_especial_total);
					$worksheet->write(10+$p, 13,($PRECIO - ($PRECIO * ($PORC_DSCTO1/100)) - ($PRECIO * ($PORC_DSCTO2/100))) * $CANTIDAD, $border_item_especial_total);		
				}else if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']>0)&($PRECIO==0)){
				    $worksheet->write(10+$p, 8,'', $border_item_especial_total);
					$worksheet->write(10+$p, 9,'', $border_item_especial_total);
					$worksheet->write(10+$p, 10,'', $border_item_especial_total);
					$worksheet->write(10+$p, 11,'', $border_item_especial_total);
					$worksheet->write(10+$p, 12,'', $border_item_especial_total);
					$worksheet->write(10+$p, 13,'', $border_item_especial_total);
				}
			}else{
				$worksheet->write(10+$p, 8,$CANTIDAD, $border_item_especial_total);
				$worksheet->write(10+$p, 9,$PRECIO, $border_item_especial_total);
				...
				if($j==0){
					...
					if($print_descto=='item'){
						if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0)){
							...
							$worksheet->write(11+$p+$j, 11,'', $border_item_right);
							$worksheet->write(11+$p+$j, 12,"", $border_item_right);
							$worksheet->write(11+$p+$j, 13,"", $border_item_right);
						}
					}else{
						$worksheet->write(11+$p+$j, 10, '', $border_item_right);
					}
					...
					if($print_descto=='item'){
						if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0)){
							...
							$worksheet->write(11+$p+$j, 12,"", $border_item_right_bold);
						}else if(($result[0]['MONTO_DSCTO2']>0)&($result[0]['MONTO_DSCTO1']==0)){
						 	$worksheet->write(11+$p+$j, 10, "",$border_item_left_bold);
							$worksheet->write(11+$p+$j, 11,"", $border_item_right_bold);
							$worksheet->write(11+$p+$j, 12,"", $border_item_right_bold);
							...
							$worksheet->write(11+$p+$j, 12,"", $border_item_right_bold);
							$worksheet->write(11+$p+$j, 13,"", $border_item_right_bold_2);
						}
					}else{
		...
		if($print_descto=='item'){
		    if(($result[0]['MONTO_DSCTO1']>0)&($result[0]['MONTO_DSCTO2']==0)){
				...
				$worksheet->write(10+$p, 11, " ", $border_item_top);
				$worksheet->write(10+$p, 12, " ", $border_item_top);
				$worksheet->write(10+$p, 13, " ", $border_item_top);
			}
		}
		...
		*/
		...
		if($print_descto=='item'){
			if((($MONTO_DSCTO1 > 0)&($MONTO_DSCTO2 == 0)) ||
				...
				$worksheet->write($row_position+1, 7+$margen_dscto, "Descuento ", $border_item_left);
				$worksheet->write($row_position+1, 9+$margen_dscto, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position+1, 10+$margen_dscto, $FINAL, $monto_normal);
				$row_position+=1;
				
				$worksheet->write($row_position+1, 7+$margen_dscto, "Total Neto ", $border_item_left);
				$worksheet->write($row_position+1, 9+$margen_dscto, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position+1, 10+$margen_dscto, $TOTAL_NETO, $monto_normal);
				$worksheet->write($row_position+2, 7+$margen_dscto, "IVA ".$PORC_IVA."% ", $border_item_left);
				$worksheet->write($row_position+2, 9+$margen_dscto, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position+2, 10+$margen_dscto, $MONTO_IVA, $monto_normal);
				$worksheet->write($row_position+3, 7+$margen_dscto, "Total con IVA ", $border_item_left);
				$worksheet->write($row_position+3, 9+$margen_dscto, $SIMBOLO, $text_normal_right);
				$worksheet->write($row_position+3, 10+$margen_dscto, $TOTAL_CON_IVA, $monto_normal);
				$worksheet->write($row_position+4, 7+$margen_dscto, " ", $border_item_top);
				$worksheet->write($row_position+4, 8+$margen_dscto, " ", $border_item_top);
				$worksheet->write($row_position+4, 9+$margen_dscto, " ", $border_item_top);
				$worksheet->write($row_position+4, 10+$margen_dscto, " ", $border_item_top);
				
				$worksheet->write($row_position-2, 7+$margen_dscto, " ", $border_item_bottom);
        		$worksheet->write($row_position-2, 8+$margen_dscto, " ", $border_item_bottom);
        		$worksheet->write($row_position-2, 9+$margen_dscto, " ", $border_item_bottom);
        		$worksheet->write($row_position-2, 10+$margen_dscto, " ", $border_item_bottom);
			}else if(($MONTO_DSCTO1 > 0)&($MONTO_DSCTO2 > 0)) {
			     $margen_dscto = 3;
		...
		$worksheet->write($row_position+14, 1, "Garantía", $text_blue_bold_left);
		$worksheet->write($row_position+14, 3, $GARANTIA, $text_normal_left);
		$worksheet->write($row_position+15, 1, "Equipo Especial", $text_blue_bold_left);
		$worksheet->write($row_position+15, 3, $EQUIPO_ESPECIAL, $text_normal_left);
		$worksheet->write($row_position+16, 1, "Notas", $text_blue_bold_left);
		$worksheet->write($row_position+17, 1, $OBS, $text_normal_left);
	...
	function printcot_tecnica_gas_excel($con_logo, $con_total_consumo) {
		$print_descto = session::get('PRINT_DESCUENTO');
		error_reporting(E_ALL & ~E_NOTICE);
		...
		$border_item_top_2->set_top(2);
		$border_item_top_2->set_bold(1);
		$border_item_top_2->set_align('right');
		...
		$worksheet->write(12, 9, "Consumo[Kw/hr]", $titulo_item_border_all);
		$worksheet->write(12, 10, "Total [Kw/hr]", $titulo_item_border_all);
		...
		for ($i=0 ; $i <count($result); $i++) {
			...
			$COD_PRODUCTO = $result[$i]['COD_PRODUCTO'];
			$CANTIDAD = $result[$i]['CANTIDAD'];
			$CONSUMO_GAS = $result[$i]['CONSUMO_GAS'];
			...
			$worksheet->write(13+$i, 9,$CONSUMO_GAS, $cant_normal);
		...
		$worksheet->write(13+$i, 6, " ", $border_item_top);
		$worksheet->write(13+$i, 7, " ", $border_item_top);
		$worksheet->write(13+$i, 8, "Total Consumo[Kw/hr]", $border_item_top_2);
		$worksheet->write(13+$i, 9, "", $border_item_top);
		...
		if($con_total_consumo == 'S'){
			...
			$worksheet->write(15+$i, 9, "", $text_blue_bold_right);
			$worksheet->write(15+$i, 10, $result_b[0]['TOTAL_CONSUMO_GAS'], $cant_normal_total);
			$worksheet->merge_cells(15+$i, 8, 15+$i, 9);
		}
