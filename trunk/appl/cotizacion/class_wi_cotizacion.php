...
class wi_cotizacion_base extends w_cot_nv {
	...
	function wi_cotizacion_base($cod_item_menu) {		
		...
		$this->add_auditoria('VALIDEZ_OFERTA');
		//auditoria
	...
	*/
	...
	function printcot_resumen_excel($con_logo) {
		...
		header("Content-Type: application/x-msexcel; name=\"COTIZACION RESUMEN $COD_PRODUCTO.xls\"");
		header("Content-Disposition: inline; filename=\"COTIZACION RESUMEN $COD_PRODUCTO.xls\"");
		$fh=fopen($fname, "rb");
		fpassthru($fh);
		unlink($fname);
