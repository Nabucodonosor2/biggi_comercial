		...
		$.showModalDialog({
			 ...
			 onClose: function(){ 
			 	...
			 	if (returnVal == null){		
					...
					var arrayRespuesta = returnVal.split('&');
					var datos = arrayRespuesta[0];
					var logoWp = arrayRespuesta[1];
					...
					document.getElementById('wi_hidden').value = datos;
					document.getElementById('wi_hidden_logo_wp').value = logoWp;
...
function request_crear_desde() {
	var url = "request_crear_desde.php";
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 200,
		 ...
		 onClose: function(){ 
		 	...
		 	if (returnVal == null){		
				...
				var tipo_seleccion = resp[1];
				var cambio_precio = resp[2]; 
				
				if(cambio_precio == 'SI'){
					if(tipo_seleccion == 'COTIZACION'){
					...
					}else{//ENTRA CUANDO ES SOLICITUD
						var url = "../common_appl/que_precio_usa_solicitud.php?cod_solicitud"+cod_cotizacion;
						$.showModalDialog({
							 url: url,
							 dialogArguments: '',
							 height: 330,
