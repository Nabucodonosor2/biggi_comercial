...
class wi_empresa_base extends w_input {
	function wi_empresa_base($cod_item_menu) {
		...
		$sql = "select COD_EMPRESA,
						...
						ALIAS_CONTABLE,
						MONTO_COMPRA_MAX_SIN_AUTORIZAR,
						REFERENCIA_HEM,
						REFERENCIA_HES,
						MAIL_NOTIFICACION_PAGO,
						OMITE_FAPROV
					from EMPRESA 
					where COD_EMPRESA = {KEY1}";				
		...
		$this->dws['dw_empresa']->add_control(new edit_text_hidden('COD_EMPRESA_H'));	
		...
		$this->dws['dw_empresa']->add_control(new edit_num('MONTO_COMPRA_MAX_SIN_AUTORIZAR', 10, 10));	
		$this->dws['dw_empresa']->add_control(new edit_mail('MAIL_NOTIFICACION_PAGO', 30, 100));
		...
		/////
		$priv = $this->get_privilegio_opcion_usuario('996510', $this->cod_usuario);
		if($priv == 'E')
			$this->dws['dw_empresa']->set_entrable('PORC_DSCTO_CORPORATIVO', true);	
		else
			$this->dws['dw_empresa']->set_entrable('PORC_DSCTO_CORPORATIVO', false);
		...
		$this->dws['dw_empresa']->add_control($control = new edit_check_box('OMITE_FAPROV', 'S', 'N', ''));
		...
		$this->dws['dw_empresa']->add_control($control = new edit_check_box('REFERENCIA_HEM', 'S', 'N', 'HEM'));
		$control->set_onChange("valida_check(this);");
		$this->dws['dw_empresa']->add_control($control = new edit_check_box('REFERENCIA_HES', 'S', 'N', 'HES'));
		$control->set_onChange("valida_check(this);");
		...
		*/		
		...
		$priv = $this->get_privilegio_opcion_usuario('996515', $this->cod_usuario);
		if ($priv == 'E')
			$this->dws['dw_empresa']->set_entrable('OMITE_FAPROV', true);	
		else
			$this->dws['dw_empresa']->set_entrable('OMITE_FAPROV', false);
	}
	function new_record() {
        ...
        unset($this->dws['dw_empresa']->controls['COD_USUARIO']);
		...
		$this->dws['dw_empresa']->add_control($control = new drop_down_dw('COD_USUARIO',$sql,150));

		unset($this->dws['dw_valor_defecto_compra']->controls['COD_FORMA_PAGO']);
		$sql_forma_pago			= "	select 			COD_FORMA_PAGO
													,NOM_FORMA_PAGO													
						   			from			FORMA_PAGO
						   			where ES_VIGENTE = 'S'
						   			order by  		ORDEN";
		$this->dws['dw_valor_defecto_compra']->add_control(new drop_down_dw('COD_FORMA_PAGO',$sql_forma_pago,150));	
		
		unset($this->dws['dw_empresa']->controls['COD_FORMA_PAGO_CLIENTE']);
		...
		$priv_d = $this->get_privilegio_opcion_usuario('996520', $this->cod_usuario);
		if($priv_d == 'E')
			$this->dws['dw_empresa']->set_entrable('SUJETO_A_APROBACION', true);	
		else
			$this->dws['dw_empresa']->set_entrable('SUJETO_A_APROBACION', false);
	}
	function load_record() {
		...
		$cod_forma_pago_cc = $this->dws['dw_empresa']->get_item(0, 'COD_FORMA_PAGO_CLIENTE');
		unset($this->dws['dw_empresa']->controls['COD_FORMA_PAGO_CLIENTE']);
		$sql_fp_cliente = "select COD_FORMA_PAGO
								 ,NOM_FORMA_PAGO													
						   from FORMA_PAGO
						   where ES_VIGENTE = 'S' OR COD_FORMA_PAGO = $cod_forma_pago_cc
						   order by ORDEN";
		$this->dws['dw_empresa']->add_control(new drop_down_dw('COD_FORMA_PAGO_CLIENTE',$sql_fp_cliente,150));
		...
		*/
        ...
        // para que muestre solo usuarios viegente y el asignado
		$cod_usuario = $this->dws['dw_empresa']->get_item(0, 'COD_USUARIO');
		if ($cod_usuario=='') 
			$cod_usuario = 0;
        $sql="SELECT COD_USUARIO, 
					 NOM_USUARIO 
			 ...
			 order by NOM_USUARIO";
        $this->dws['dw_empresa']->controls['COD_USUARIO']->set_sql($sql);
		$this->dws['dw_empresa']->controls['COD_USUARIO']->retrieve();
		
		$priv = $this->get_privilegio_opcion_usuario('996505', $this->cod_usuario);
		if($priv == 'E')
			$this->dws['dw_empresa']->set_entrable('MONTO_COMPRA_MAX_SIN_AUTORIZAR', true);	
		else
			$this->dws['dw_empresa']->set_entrable('MONTO_COMPRA_MAX_SIN_AUTORIZAR', false);
	...
	function save_record($db) {
		...
		$REFERENCIA_HEM				= $this->dws['dw_empresa']->get_item(0, 'REFERENCIA_HEM');
		$REFERENCIA_HES				= $this->dws['dw_empresa']->get_item(0, 'REFERENCIA_HES');
		$MAIL_NOTIFICACION_PAGO		= $this->dws['dw_empresa']->get_item(0, 'MAIL_NOTIFICACION_PAGO');
		$OMITE_FAPROV				= $this->dws['dw_empresa']->get_item(0, 'OMITE_FAPROV');
		// EL CAMPO PORC_DSCTO_CORPORATIVO FUE ELIMINADO DE LA BD	
		...
		$MONTO_COMPRA_MAX_SIN_AUTORIZAR = $this->dws['dw_empresa']->get_item(0, 'MONTO_COMPRA_MAX_SIN_AUTORIZAR');
		...
		$MAIL_NOTIFICACION_PAGO = ($MAIL_NOTIFICACION_PAGO=='') ? "null" : "'$MAIL_NOTIFICACION_PAGO'";
		...
		$MONTO_COMPRA_MAX_SIN_AUTORIZAR	= ($MONTO_COMPRA_MAX_SIN_AUTORIZAR =='') ? 0 : $MONTO_COMPRA_MAX_SIN_AUTORIZAR;	
