...
for($i=0; $i < count($result); $i++) {
	...
	$db->EXECUTE_SP('spu_envio_mail', "'COMP_REN_COMER', $COD_ENVIO_MAIL");
...
echo $sql;
		...
		// Si ya esta rechazado, debe saltarlo
		$cod_estado_libre_dte = $result_dte[0]['COD_ESTADO_LIBRE_DTE'];
		if($cod_estado_libre_dte== 4 || $cod_estado_libre_dte== 5 || $cod_estado_libre_dte== 6 || $cod_estado_libre_dte== 7)
			continue;
...
if($body_diff <> ""){
	...
	$param = "'INSERT'
			 ...
			 ,null
		 	 ...
		 	 ,null
		 	 ,null
		 	 ,'mherrera@biggi.cl'	--mail_destinatario
		 	 ,'Marcelo Herrera'										--nom_destinatario
		 	 ,'COMERCIAL ERROR xml <> bd'						--Subject
		 	 ,'$body_diff'
		 	 ,''
		 	 ,'ERROR_XML'
		 	 ,0";
