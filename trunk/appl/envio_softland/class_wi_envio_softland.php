...
class dw_envio_transbank extends datawindow {
	function dw_envio_transbank() {
		$sql = "SELECT	ET.COD_ENVIO_TRANSBANK
									,ET.NRO_AUTORIZA_TB
									,ET.MONTO_ABONO
									,ET.COD_INGRESO_PAGO
									,ET.COD_INGRESO_PAGO COD_INGRESO_PAGO_I
									,INP.NOM_TIPO_ORIGEN_PAGO
									,convert(varchar(20), INP.FECHA_INGRESO_PAGO, 103) FECHA_INGRESO_PAGO
									,ET.COD_NOTA_VENTA
									,ET.COD_NOTA_VENTA COD_NOTA_VENTA_I
									,SUBSTRING(ET.RAZON_SOCIAL, 1, 41) RAZON_SOCIAL
									,ET.RAZON_SOCIAL RAZON_SOCIAL_I
									,SUBSTRING(TDP.NOM_TIPO_DOC_PAGO,8,14) NOM_TIPO_DOC_PAGO
									,TDP.COD_TIPO_DOC_PAGO
									,ET.CUOTAS
									,ET.CUOTAS CUOTAS_I
									,ET.CUOTAS_N
									,ES.COD_ENVIO_SOFTLAND COD_ENVIO_SOFTLAND_TB
                  ,ET.COD_EMPRESA  
                  ,DIP.MONTO_DOC                                                                                
							from  ENVIO_TRANSBANK ET, ENVIO_SOFTLAND ES, INGRESO_PAGO INP,TIPO_DOC_PAGO TDP, DOC_INGRESO_PAGO DIP
							where  ES.COD_ENVIO_SOFTLAND = {KEY1}
							and ET.COD_ENVIO_SOFTLAND = ES.COD_ENVIO_SOFTLAND
							and ET.COD_INGRESO_PAGO = INP.COD_INGRESO_PAGO
							and ET.COD_TIPO_DOC_PAGO = TDP.COD_TIPO_DOC_PAGO
              and DIP.COD_INGRESO_PAGO = INP.COD_INGRESO_PAGO
              and ET.NRO_AUTORIZA_TB = DIP.NRO_DOC
							order by COD_ENVIO_TRANSBANK ASC";
		parent::datawindow($sql, 'ENVIO_TRANSBANK', true, true, 'COD_PRODUCTO');

		$this->add_control(new edit_text('COD_ENVIO_TRANSBANK',10, 10, 'hidden'));
		$this->add_control(new edit_text('COD_ENVIO_SOFTLAND_TB',10, 10, 'hidden'));
		$this->add_control(new edit_text('COD_TIPO_DOC_PAGO',10, 10, 'hidden'));
		$this->add_control(new edit_text('COD_INGRESO_PAGO_I',10, 10, 'hidden'));
		$this->add_control(new edit_text('COD_NOTA_VENTA_I',10, 10, 'hidden'));
		$this->add_control(new edit_text('RAZON_SOCIAL_I',10, 10, 'hidden'));
		$this->add_control(new edit_text('CUOTAS_I',10, 30, 'hidden'));
		$this->add_control(new edit_text('CUOTAS_N',8, 30));
		$this->add_control($control = new edit_text('NRO_AUTORIZA_TB',5, 10));
		$control->set_onChange("request_autorizatb(this);");
		$this->add_control(new edit_num('MONTO_ABONO',8, 10));
		$this->add_control(new static_text('COD_INGRESO_PAGO'));
		$this->add_control(new static_text('NOM_TIPO_ORIGEN_PAGO'));
		$this->add_control(new static_text('FECHA_INGRESO_PAGO'));
		$this->add_control(new static_text('COD_NOTA_VENTA'));
		$this->add_control(new static_text('RAZON_SOCIAL'));
		...
		//$this->set_entrable('COD_INGRESO_PAGO', false);
	...
	function update($db){
		...
		for ($i = 0; $i < $this->row_count('delete'); $i++) {
			...
			$COD_ITEM_COTIZACION = $this->get_item($i, 'COD_ENVIO_TRANSBANK', 'delete');
			...
			$sql = "select 'S' EM_SELECCION
					...
					from 	(select distinct COD_EMPRESA
							...
							where COD_ESTADO_DOC_SII in (2,3,4)
                             ...
                             select distinct COD_EMPRESA
							from NOTA_VENTA
							where COD_ESTADO_NOTA_VENTA in (1,2,4)
					) A, EMPRESA E
...
class dw_envio_softland extends datawindow {
	...
	function dw_envio_softland() {
		$sql = "SELECT E.COD_ENVIO_SOFTLAND
            ...
            ,E.TOTAL_BANCO
            ,E.COMISION_TBK
            ,convert(varchar(20), E.FECHA_ABONO, 103) FECHA_ABONO
						,case E.COD_ESTADO_ENVIO
							when ".self::K_ENVIO_EGRESOS." then 'ANULADA'
							else ''
						...
						,case E.COD_TIPO_ENVIO when 5 then '' else 'none' end DISPLAY_TB
		...
		$this->add_control(new edit_num('MONTO_TOT_TD'));
		$this->add_control(new edit_num('MONTO_TOT_TC'));
    $this->add_control(new edit_num('TOTAL_BANCO'));
		$this->add_control(new edit_num('COMISION_TBK'));
    $this->add_control(new edit_date('FECHA_ABONO'));
	}
	function new_envio($cod_tipo_envio) {
		$this->insert_row();
		...
		$this->set_item(0, 'MONTO_TOT_TD', 0);	// Emitido
		$this->set_item(0, 'MONTO_TOT_TC', 0);	// Emitido
    $this->set_item(0, 'TOTAL_BANCO', 0);	// Emitido
		$this->set_item(0, 'COMISION_TBK', 0);	// Emitido
		...
		$this->set_item(0, 'DISPLAY_TB', 'none');
		...
		else if ($cod_tipo_envio==self::K_ENVIO_TRANSBANK) {	//facturas de compras
			$this->set_item(0, 'DISPLAY_TB', '');
			$this->set_item(0, 'DISPLAY_1_2_TIPO', 'none');
			$this->set_item(0, 'DISPLAY_3_4_TIPO', 'none');
		}
	}
}
class wi_envio_softland_base extends w_input {
	...
	function wi_envio_softland_base($cod_item_menu) {
		...
		$this->dws['dw_envio_transbank'] = new dw_envio_transbank();
	...
	function print_anexo() {
	    ...
	    print " <script>window.open('print_especial.php?cod_envio_softland=$cod_envio_softland')</script>";
	    $this->_load_record();
	}
	function habilita_boton(&$temp, $boton, $habilita) {
	    if($boton == 'b_print_anexo'){
	        if($habilita){
	            ...
	            $control = '<img src="../../images_appl/b_print_anexo_d.jpg">';
	        }
  ...
  function navegacion(&$temp){
    ...
    //$this->dws['dw_envio_transbank']->new_envio($cod_tipo_envio);
		...
		if ($cod_tipo_envio == 1) {
			...
			$this->dws['dw_envio_transbank']->new_envio();
		}
	...
	function load_record() {
		...
		$this->dws['dw_envio_transbank']->retrieve($cod_envio_softland);
	...
	function save_record($db) {
		...
		$monto_tot_td = $this->dws['dw_envio_softland']->get_item(0, 'MONTO_TOT_TD');
		$monto_tot_tc = $this->dws['dw_envio_softland']->get_item(0, 'MONTO_TOT_TC');
    $total_banco = $this->dws['dw_envio_softland']->get_item(0, 'TOTAL_BANCO');
		$comision_tbk = $this->dws['dw_envio_softland']->get_item(0, 'COMISION_TBK');
    $fecha_abono = $this->dws['dw_envio_softland']->get_item(0, 'FECHA_ABONO');
		...
		$monto_tot_td = ($monto_tot_td=='') ? 'null' : $monto_tot_td;
		$monto_tot_tc = ($monto_tot_tc=='') ? 'null' : $monto_tot_tc;
    $total_banco = ($total_banco=='') ? 'null' : $total_banco;
    $comision_tbk = ($comision_tbk=='') ? 'null' : $comision_tbk;
    $fecha_abono = $this->str2date($fecha_abono);
		...
		$param	= "'$operacion', $cod_envio_softland, $cod_usuario, $cod_tipo_envio, $nro_comprobante, $cod_estado_envio, $nro_correlativo_interno, $monto_tot_td, $monto_tot_tc,$total_banco,$comision_tbk,$fecha_abono";
      ...
      if (!$this->dws['dw_lista_factura']->update($db))
        ...
        $this->dws['dw_lista_ingreso_pago']->set_item($i, 'IN_COD_ENVIO_SOFTLAND', $cod_envio_softland);
			...
			for ($i=0; $i<$this->dws['dw_envio_transbank']->row_count(); $i++){
			  $this->dws['dw_envio_transbank']->set_item($i, 'COD_ENVIO_SOFTLAND_TB', $cod_envio_softland);
        }
			if (!$this->dws['dw_envio_transbank']->update($db))
				return false;
  ...
  function agregar_envio(){
	...
	function send_nula($handle, $tipo_doc, $i, $cod_cuenta_contable) {
		...
		$COD_TIPO_ENVIO_SOFT_AUX = $this->dws['dw_envio_softland']->get_item(0, 'COD_TIPO_ENVIO');
		IF (($COD_TIPO_ENVIO_SOFT_AUX == 3) || ($COD_TIPO_ENVIO_SOFT_AUX == 4)){
			// DESPUES CAMBIO DESARROLLO SOFTLAND 14032022
			fwrite($handle, $this->nro_comprobante.$this->separador);				// nro comprobante
			fwrite($handle, '""'.$this->separador);									// Graba el detalle de libro (S/N)(Opcional, por defecto 'S')
			fwrite($handle, '""'.$this->separador);									// Documento Nulo (S/N)(Opcional, por defecto 'N')
			fwrite($handle, '"'.$glosa.'"'."\r\n");									// Descripcion del comprobante (Solo en primer movimiento de cada grupo)
		}ELSE{
			// ANTES CAMBIO DESARROLLO SOFTLAND 14032022
	...
	function send_por_cobrar($handle, $tipo_doc, $i, $cuenta_por_cobrar) {
		...
		//MH 06062022 JC+SP SOLICITAN QUE EN EL TRASPASO DE VENTAS ENVIE VACIO EN VEZ DE 001 COMENZO A DAR ERROR EN SOFTLAND SIN EXPLICACION
		//fwrite($handle, '"001"'.$this->separador);								// codigo ubicacion
	...
	function send_por_pagar($handle, $tipo_doc, $i, $cuenta_por_pagar, $nro_correlativo_interno) {
		...
		if ($tipo_doc=='FC' || $tipo_doc=='FE' || $tipo_doc=='FX' || $tipo_doc=='BE') {
			...
			$sql = "select E.ALIAS_CONTABLE
							,E.NOM_EMPRESA
							,E.RUT
							,convert(varchar, dateadd(m, 1, FECHA_NCPROV), 103) FECHA_VENCTO
							,F.COD_TIPO_NCPROV
			...
			$rut = $result[0]['RUT'];
			$fecha_vencto = $result[0]['FECHA_VENCTO'];
			$cod_tipo_ncprov = $result[0]['COD_TIPO_NCPROV'];
			...
			$monto_debe = $total_con_iva;
			$monto_haber = 0;
			if ($cod_tipo_ncprov==2) {	// NC Electronica
				$tipo_doc_ref = 'NE';
				$tipo_doc = 'NE';
			}
			else {
	...
	function send_por_pagar_egreso($handle, $cod_pago_faprov, $nro_correlativo_interno, $nro_documento, $CH_o_NC) {
		...
		for ($j=0; $j < count($result_faprov); $j++) {
			...
			if ($tipo_doc=='FC' || $tipo_doc=='FE' || $tipo_doc=='BE' || $tipo_doc=='FX') {
				...
				if ($CH_o_NC=='CHEQUE') {
					// "CH/nro_cheque"
					if ($cod_tipo_pago_faprov == 2) //transferencia
						//$dato_documento = 'TF/'.$cod_pago_faprov;
						$dato_documento = 'TF/'.$nro_documento.' PP '.$cod_pago_faprov;
					else
			...
			fwrite($handle, '"'.$rut.'"'.$this->separador);							// codigo auxiliar
			if ($cod_tipo_pago_faprov == 2) { //transferencia
				fwrite($handle, '"TF"'.$this->separador);								// tipo documento
				fwrite($handle, $cod_pago_faprov.$this->separador);							// nro documento
			}
			else {
	...
	function send_banco_egreso($handle, $cuenta_banco, $monto, $centro_costo, $nro_documento, $cod_pago_faprov) {
		...
		if ($cod_tipo_pago_faprov == 2) //transferencia
			//$glosa = 'TF/'.$cod_pago_faprov.' '.$alias.' '.$nro_fas;
			$glosa = 'TF/'.$nro_documento.' PP '.$cod_pago_faprov.' '.$alias.' '.$nro_fas;
		else
		...
		fwrite($handle, '"'.$centro_costo.'"'.$this->separador);				// codigo centgro costo
		if ($cod_tipo_pago_faprov == 2) { //transferencia
			fwrite($handle, '"TF"'.$this->separador);								// tipo docto conciliacion 
	...
	function send_pago_ingreso($handle, $cod_ingreso_pago, $es_deposito_inmediato, $cod_doc_ingreso_pago=0) {
		...
		for ($i=0; $i < count($result); $i++) {
			...
			if ($tipo_doc=='DP') {
				...
				//////////////////
				// 03-11-2015 solocitado por RE, ver mail del 03-11-2015 asunto "Re: Corrección en traspaso de ingresos (Caso José Catalán)"
				$tipo_docto_conciliacion = "";
				$nro_docto_conciliacion = "0";
				$tipo_docto_referencia = $tipo_doc;
				$nro_docto_referencia = $nro_doc;
