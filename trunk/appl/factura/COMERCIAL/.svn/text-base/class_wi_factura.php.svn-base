...
class wi_factura extends wi_factura_base {
	...
	function enviar_dte(){
		...
		for($i=0 ; $i < $this->dws['dw_referencias']->row_count() ; $i++){
			$COD_TIPO_REFERENCIA = $this->dws['dw_referencias']->set_item($i, 'COD_TIPO_REFERENCIA', $COD_TIPO_REFERENCIA);
		...
		$a['Encabezado']['IdDoc']['TipoDTE']		= $cod_tipo_dte; //Factura
		$a['Encabezado']['IdDoc']['Folio']			= $nro_factura;
		$a['Encabezado']['IdDoc']['TermPagoGlosa']	= utf8_encode($TermPagoGlosa);///////////
		...
		if ($FolioRef <> ''){
			...
			$c['Referencia'][$i]['RazonRef']	= substr($RazonRef,0,90);
			$tiene_Folio = 'S';
			$i++;
		}
