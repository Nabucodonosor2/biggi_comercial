...
class wi_factura extends wi_factura_base {
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		...
		if($boton == 'print_dte_xml'){
			if($habilita){
				...
				$ruta_out = "'../../images_appl/b_reimprime_dte.jpg'";
				$ruta_click = "'../../images_appl/b_reimprime_dte_click.jpg'";
				$control =  '<input name="b_print_dte_xml" id="b_print_dte_xml" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
				   			'style="cursor:pointer;height:68px;width:66px;border: 0;background-image:url(../../images_appl/b_reimprime_dte.jpg);background-repeat:no-repeat;background-position:center;border-radius: 15px;"'.
				   			'onClick="return dlg_print_dte2();" />';
	...
	function procesa_event() {		
		...
		/*else if(isset($_POST['b_print_dte_x']))
			...
			$this->imprimir_dte_xml($_POST['wi_hidden']);
		}else
	...
	function enviar_dte(){
		...
		$FolioRef		= substr($contenido[0]['FolioRef'], 0, 18);
		$FchRef			= substr($contenido[0]['FchRef'], 0, 10);
		$CodRef			= substr($contenido[0]['CodRef'], 0, 1);
	...
	function imprimir_dte_xml($es_cedible){
		$cod_factura = $this->get_key();
		$PORC_IVA = $this->dws['dw_factura']->get_item(0, 'PORC_IVA');
		...
		print " <script>window.open('../common_appl/print_dte_xml.php?cod_documento=$cod_factura&DTE_ORIGEN=$cod_tipo_dte&ES_CEDIBLE=$es_cedible')</script>";
		$this->_load_record();
	}
