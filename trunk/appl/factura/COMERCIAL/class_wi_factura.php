...
class wi_factura extends wi_factura_base {
	...
	function envia_FA_electronica(){
			...
			if ($db->EXECUTE_SP($sp, $param)){
				...
				if ($GIRO=='') {
				    $this->_load_record();
				    $this->alert('No se puede enviar Fatura Electronica sin el Giro de la empresa');
				}
		    ...
		    if($habilita){
		        $control = '<input name="b_envio_mail" id="b_envio_mail" src="../../images_appl/b_envio_mail.jpg" type="image" '.
		  		        ...
		  		        'onMouseOver="MM_swapImage(\'b_envio_mail\',\'\',\'../../images_appl/b_envio_mail_over.jpg\',1)"
							 onClick="return confirma_mail();"/>';
		    }else{
		        $control = '<img src="../../images_appl/b_envio_mail_d.jpg">';
		    }
		    
		    $temp->setVar("ENVIAR_MAIL", $control);
		}
	}
	...
	function navegacion(&$temp){
		...
		if($COD_ESTADO_DOC_SII == self::K_ESTADO_SII_EMITIDA){
			...
			$this->habilita_boton($temp, 'enviar_mail', false);
			...
			if($TRACK_ID_DTE <> ''){
				...
				else $this->habilita_boton($temp, 'reenviar_dte', false);
				
				if($COD_DOC != '' && $TIPO_DOC == 'NOTA_VENTA'){
				    $this->habilita_boton($temp, 'enviar_mail', true);
				    $Sql= "select N.CORREO_FA 
                            FROM NOTA_VENTA N,FACTURA F 
                            WHERE F.COD_FACTURA = $cod_factura
                            and N.COD_NOTA_VENTA  = F.COD_DOC";
				    $result = $db->build_results($Sql);
				    $CORREO_FA = $result[0]['CORREO_FA'];
				    $temp->setVar("CORREOS_NV", $CORREO_FA);
				}
			}
	...
	function enviar_dte($reenviar = false){
		...
		for($i = 0; $i < count($Detalles); $i++) {
			...
			if($contenido[0]['CODIGO_QBLI'] == 'S'){
				$ad['Detalle'][$i]["CdgItem"]["TpoCodigo"]= 'QBLI';
				$ad['Detalle'][$i]["CdgItem"]["VlrCodigo"]= $VlrCodigo;
			}
			else{
				$ad['Detalle'][$i]["CdgItem"]["TpoCodigo"]= 'INT1';
				$ad['Detalle'][$i]["CdgItem"]["VlrCodigo"]= $VlrCodigo;
			}
		}
		...
		if($count_ref_receptor > 0){
			$sql = "SELECT DOC_REFERENCIA
					FROM REFERENCIA
					WHERE COD_FACTURA = $cod_factura
					AND COD_TIPO_REFERENCIA = 9";
			$result = $db->build_results($sql);
			
			if(count($result) > 0 ){
				$cdg_int_recep	= $result[0]['DOC_REFERENCIA'];
			}
		}
		...
		if($count_ref_receptor > 0){
			$a['Encabezado']['Receptor']['CdgIntRecep']	= utf8_encode($cdg_int_recep);
		}
      ...
      $response = $dte->actualizar_estado($tipodte,$FolioRef,'91462001-5');
      $actualizar_estado	= $dte->respuesta_actualizar_estado($response);
      $revision_estado	= substr($actualizar_estado[6], 0, 3);
			*/
			$tiene_Folio = 'S';
