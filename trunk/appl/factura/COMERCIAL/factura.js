...
function validate(){
	...
	if (((parseFloat(monto_dscto1) + parseFloat(monto_dscto2))/parseFloat(sum_total.value))*100 > parseFloat(porc_dscto_max.value)) {
		...
		alert('La suma de los descuentos es mayor al permitido (m�ximo '+number_format(porc_dscto_max.value, 0, ',', '.')+' % entre los dos descuentos, equivalente a '+number_format(monto_permitido, 0, ',', '.')+')');
	...
	if (document.getElementById('COD_FORMA_PAGO_0')){
		...
		if (parseFloat(cod_forma_pago) == 1 && nom_forma_pago_otro == ''){
			alert ('Debe ingresar la Descripci�n de la forma de pago seleccionada.');
...
function change_item_factura(ve_valor, ve_campo) {
	...
	if(ve_campo == 'COD_PRODUCTO' | ve_campo == 'NOM_PRODUCTO'){
		...
		if(cod_producto.value == 'T'){
			alert('No se pueden agregar T�tulos.');
...
function confirma_mail(){
	var correos = $("#correos_nv").val(); 
	
	if(correos.length == 0){
		alert('La nota de venta no tiene correos ingresados');
		return false;
	}
	
	return confirm('Se enviar\u00E1 un mail con la factura a: '+ correos);
}
