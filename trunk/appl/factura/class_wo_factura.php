...
class wo_factura_base extends w_output_biggi {
	...
	function crear_fa_from_nv($valor_devuelto,$nv_valida){
		...
		$pos = strpos($valor_devuelto, 'desde_gd');
	  	if($pos!==false){
			...
			*/
			...
			if($array2[0] == 'desde_nv'){
				$cod_nota_venta	= $array2[1];
				
				$sp = 'sp_fa_crear_desde_nv_new';
				$param = "$cod_nota_venta, $cod_usuario, '$datos','$nv_valida'";
				$reg = explode("|", $datos);
				...
				if ($db->EXECUTE_SP($sp, $param)) { 
					...
					$db->ROLLBACK_TRANSACTION();
					$this->_redraw();
					$this->alert("No se pudo crear la factura. Error en 'sp_fa_crear_desde_nv_new', favor contacte a Soporte BIGGI. ".$sp." ".$param);
				}
				
			}else{
				$array = explode("|", $array[0]);
				$cod_nota_venta = $array[1];
	...
	function procesa_event() {		
		if(isset($_POST['b_create_x'])) {
			$this->crear_fa_from_nv($_POST['wo_hidden'],$_POST['wo_valida']);
