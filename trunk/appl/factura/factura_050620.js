...
{
	var valida_nv = '';
	var url = "../../../trunk/appl/factura/request_factura.php?prompt="+URLEncode(ve_prompt)+"&valor="+URLEncode(ve_valor);
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 280,
		 ...
		 onClose: function(){ 
		 	...
		 	if (returnVal == null){		
				return false;
	 			...
	 			if(opcion == 'desde_nv_anticipo'){
	 				var ajax = nuevoAjax();
					ajax.open("GET", "ajax_valida_factura_parcial.php?cod_nota_venta="+cod_nota_venta_value+"&tipo_doc=NV_ANTICIPO", false);
					ajax.send(null);	
					...
					document.output.submit();
			   		return true;
	 			}else if(opcion == 'desde_nv'){
	 			
	 				var ajax = nuevoAjax();
					ajax.open("GET", "ajax_valida_factura_parcial.php?cod_nota_venta="+cod_nota_venta_value+"&tipo_doc=NV", false);
					ajax.send(null);	
					var resp = ajax.responseText.split('|');
					...
					if(resp[0] == 'ERROR'){
	 					alert(resp[1]);
	 					return false;
	 				}else if(resp[0] == 'NO_VALIDA'){
	 					valida_nv = 'N';
	 				}else{
	 					valida_nv = 'S';
	 				}
	 				
	 				var url2 = "../../../trunk/appl/factura/request_factura_parcial.php?cod_doc="+cod_nota_venta_value+"&tipo_doc=NV";
					$.showModalDialog({
						 url: url2,
						 dialogArguments: '',
						 height: 530,
						 ...
						 onClose: function(){ 
						 	...
						 	if (returnVal2 == null){		
								...
								document.getElementById('wo_valida').value = valida_nv;
								document.output.submit();
						   		return true;
							}
						}
	 			...
	 			}else if(opcion == 'desde_gd'){
	 				
	 				var ajax = nuevoAjax();
					ajax.open("GET", "ajax_valida_factura_parcial.php?cod_nota_venta="+cod_nota_venta_value+"&tipo_doc=GD", false);
					ajax.send(null);	
					var resp = ajax.responseText.split('|');
					...
					var url3 = "../../../trunk/appl/factura/request_factura_desdeGd.php?cod_nota_venta="+ cod_nota_venta_value;
					$.showModalDialog({
						 url: url3,
						 dialogArguments: '',
						 height: 400,
						 ...
						 scrollable: false,
						 onClose: function(){ 
						 	var returnVal3 = this.returnValue;
						 	if (returnVal3 == null){		
								return false;
							}			
							else {
								...
								document.getElementById('wo_hidden').value = returnVal3;
								document.output.submit();
						   		return true;
							}
						}
...
/******NO SE OCUPA select_printer_dte****/
