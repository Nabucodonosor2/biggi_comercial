...
BEGIN  
		...
		declare @vl_cod_max_cant_it_fa numeric,
			...
			@vl_rut	numeric,
			@vl_count	numeric,
			@vl_fecha_ref	datetime,
			@vl_cc_cliente	varchar(20)
		...
		------------------------REFERENCIA SODEXO-----------------------------------
		SELECT @vl_rut = RUT
			 ,@vl_fecha_ref = GETDATE()
			 ,@vl_cc_cliente = CENTRO_COSTO_CLIENTE
		FROM FACTURA
		...
		if(@vl_count > 0)
			execute spu_referencia 'INSERT', NULL, @vl_fecha_ref, @vl_cc_cliente, 3, @vl_cod_fa
		----------------------------------------------------------------------------
