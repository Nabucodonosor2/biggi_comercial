...
class wi_faprov extends w_input {
	...
	function new_record() {
		...
		//CUANDO CREAN DESDE LA NADA SETEA EL CAMPO DE TIPO FA ELECTRONICA MH 06012020
		...
		if($boton == 'create'){
			if($habilita){
				...
				$control = '<input name="b_'.$boton.'" id="b_'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
							...
							'onClick="request_tipo_faprov2(\'Origen de Recepción de Factura\',\'\');" />';
			}else{
	...
	function print_record() {	
		...
		$rpt = new rpt_print_faprov($sql, $this->root_dir.'appl/faprov/faprov.xml', $labels, "Recepción de Factura ".$cod_faprov.".pdf", 0);
		$this->_load_record();
		return true;
	}
