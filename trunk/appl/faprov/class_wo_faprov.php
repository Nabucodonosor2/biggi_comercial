...
class wo_faprov extends w_output_biggi {
	...
	const K_PARAM_DIRECTORIO = 31;
   	...
   	function wo_faprov() {
		...
		$this->add_header(new header_date('FECHA_FAPROV', 'FECHA_FAPROV', 'Fecha Doc'));
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		if ($boton=='asignar') {
  			if ($this->get_privilegio_opcion_usuario(self::K_ASIGNA_PROY_COMPRA, $this->cod_usuario)=='E') {
				if ($habilita){
					$ruta_over = "'../../images_appl/b_asignar_over.jpg'";
					$ruta_out = "'../../images_appl/b_asignar.jpg'";
					$ruta_click = "'../../images_appl/b_asignar_click.jpg'";
					$temp->setVar("WO_ASIGNAR", '<input name="b_'.$boton.'" id="b_'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
									'style="cursor:pointer;height:68px;width:66px;border: 0;background-image:url(../../images_appl/b_asignar.jpg);background-repeat:no-repeat;background-position:center;border-radius: 15px;"'.
									'onClick="request_cuenta();" style="display:{DISPLAY_MARCAR}"/>');
				}else
			...
			$temp->setVar("WO_CREATE", '<input name="b_'.$boton.'" id="b_'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
							...
							'onClick="request_tipo_faprov(\'Origen de Recepción de Factura\',\'\');" />');
		}
		else 
