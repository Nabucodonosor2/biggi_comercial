...
class wo_gasto_fijo_base extends w_output_biggi {
	...
	function wo_gasto_fijo_base() {
		...
		if($this->get_privilegio_opcion_usuario('2550', $this->cod_usuario)=='L'){
			...
			if($this->cod_usuario == 81){ //NICOLE RODRIGUEZ
				$sql .= " UNION
						select COD_ORDEN_COMPRA 
								,convert(varchar(20), FECHA_ORDEN_COMPRA, 103) FECHA_ORDEN_COMPRA 
								,FECHA_ORDEN_COMPRA DATE_GASTO_FIJO 
								,E.NOM_EMPRESA 
								,REFERENCIA 
						...
						from ORDEN_COMPRA O
							,EMPRESA E 
							,ESTADO_ORDEN_COMPRA EOC 
						where O.COD_EMPRESA = E.COD_EMPRESA 
						and O.COD_ESTADO_ORDEN_COMPRA = EOC.COD_ESTADO_ORDEN_COMPRA 
						and TIPO_ORDEN_COMPRA = 'GASTO_FIJO' 
						and O.COD_PERSONA in (17458, 17923) -- JAVIER QUINTEROS, NICOLE RODRIGUEZ
						and O.COD_EMPRESA = 1337"; //COMERCIAL BIGGI CHILE S.A.
			}
