					...
					if (@vl_nro_guia_despacho is null)
						begin
							...
							if(@vl_count > 0)begin
								...
								select   @vl_nro_gd_comercial=  max(nro_guia_despacho) + 1
								from    RENTAL_dbo_guia_despacho
								where    cod_estado_doc_sii = @vl_cod_estado_gd_enviada
								
								if (@vl_nro_gd_comercial > @vl_nro_guia_despacho)
									set @vl_nro_guia_despacho  = @vl_nro_gd_comercial
							end
							else begin
