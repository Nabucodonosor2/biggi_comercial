...
/*
		...
		$sql = "select 	
				...
				,CASE
					WHEN G.COD_DOC IS NULL THEN
						(SELECT NOM_USUARIO FROM USUARIO  WHERE COD_USUARIO = G.COD_USUARIO) 
					ELSE	
				...
				,CASE
					WHEN G.COD_DOC IS NULL THEN
						(SELECT MAIL FROM USUARIO WHERE COD_USUARIO = G.COD_USUARIO) 
					ELSE	
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		...
		if($boton == 'print_dte_xml'){
			if($habilita){
				$control = '<input name="b_print_dte_xml" id="b_print_dte_xml" src="../../images_appl/b_reimprime_dte.jpg" type="image" '.
							 'onMouseDown="MM_swapImage(\'b_print_dte_xml\',\'\',\'../../images_appl/b_reimprime_dte_click.jpg\',1)" '.
							 'onMouseUp="MM_swapImgRestore()" onMouseOut="MM_swapImgRestore()" '.
							 'onMouseOver="MM_swapImage(\'b_print_dte_xml\',\'\',\'../../images_appl/b_reimprime_dte_over.jpg\',1)"
							 onClick="return true;"/>';
			}
			
			$temp->setVar("WSWAP_IMPRIMIR_DTE_XML", $control);
		}
	}
	...
	function navegacion(&$temp){
		...
		if($this->cod_usuario == 1 && $this->modify == false)
			$this->habilita_boton($temp, 'print_dte_xml', true);
	...
	function procesa_event() {		
		...
		/*else if(isset($_POST['b_print_dte_x'])) {
			...
			$this->xml_dte();	
		}else if(isset($_POST['b_print_dte_xml_x'])){
			$this->imprimir_dte_xml();
		}else
	...
	function enviar_dte(){
		...
		$sql = "select (CAST(GD.RUT AS NVARCHAR(8)))+'-'+(CAST (GD.DIG_VERIF AS NVARCHAR(1))) as RUT_COMPLETO 
						...
						,C.NOM_COMUNA
						,PATENTE
						,(CAST(GD.RUT_RETIRADO_POR AS NVARCHAR(10)))+'-'+(CAST (GD.DIG_VERIF_RETIRADO_POR AS NVARCHAR(1))) as RUTTrans 
						,(CAST(GD.RUT_RETIRADO_POR AS NVARCHAR(10)))+'-'+(CAST (GD.DIG_VERIF_RETIRADO_POR AS NVARCHAR(1))) as RUTChofer 
						,RETIRADO_POR
						,COD_TIPO_GUIA_DESPACHO as IndTraslado
	...
	function imprimir_dte_xml(){
		$guia_despacho = $this->get_key();
		print "<script>window.open('../common_appl/print_dte_xml.php?cod_documento=$guia_despacho&DTE_ORIGEN=52')</script>";
		$this->_load_record();
	}
		...
		if($rep_response[1] == '' && $TRACK_ID_DTE == '' && $COD_ESTADO_DOC_SII == self::K_ESTADO_SII_EMITIDA){
			...
			if (($nro_gd_dte <> '') && ($EnvioDTExml <> '')&& ($track_id <> '')){
				...
				if ($db->EXECUTE_SP($sp, $param)) {
					$db->COMMIT_TRANSACTION();
					$guia_despacho = $this->get_key();
					print " <script>window.open('print_dte.php?cod_guia_despacho=$guia_despacho')</script>";
					$this->_load_record();
				}else{
					$db->ROLLBACK_TRANSACTION();
				}
	...
	function xml_dte(){
		$cod_guia_despacho = $this->get_key();
		$name_archivo = "XML_DTE_GUIA_DESPACHO_".$this->get_key_para_ruta_menu().".xml";
