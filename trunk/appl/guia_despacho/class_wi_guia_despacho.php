...
class dw_guia_despacho extends dw_help_empresa {
	...
	function dw_guia_despacho() {
		...
		$sql = "SELECT GD.COD_GUIA_DESPACHO,
					...
					,COD_USU_VENDEDOR_RESP
					,CASE
						WHEN GD.COD_DOC IS NULL THEN ''
						ELSE 'none'
					END DISPLAY_USU_VENDEDOR
					,AUTORIZA_GD_FACTURADA
					,MOTIVO_GD_FACTURADA
					,CASE
		...
		$sql = "SELECT COD_USUARIO COD_USU_VENDEDOR_RESP
					   ,NOM_USUARIO
				FROM USUARIO
				WHERE ES_VENDEDOR = 'S'
				AND AUTORIZA_INGRESO = 'S'
				ORDER BY NOM_USUARIO ASC";
		$this->add_control($control = new drop_down_dw('COD_USU_VENDEDOR_RESP',$sql,150));
		
		$this->add_control($control = new edit_check_box('AUTORIZA_GD_FACTURADA', 'S', 'N'));
		$control->set_onChange("display_motivo_gd_facturada();");
		$this->add_control(new edit_text('MOTIVO_GD_FACTURADA', 50, 50));
		...
		$control->set_onChange("ajax_tipo_gd_interno(this);valida_demo(this);");
		...
		$this->set_mandatory('COD_TIPO_GUIA_DESPACHO', 'Tipo Guia Despacho');
	}
	function fill_record(&$temp, $record) {	
		parent::fill_record($temp, $record);
		$COD_DOC = $this->get_item(0, 'COD_DOC');
...
class wi_guia_despacho extends w_input {
	...
	function wi_guia_despacho($cod_item_menu) {		
		...
		$this->add_auditoria('AUTORIZA_GD_FACTURADA');
		
		$priv = $this->get_privilegio_opcion_usuario('994540', $this->cod_usuario);
		if($priv == 'E')
			$this->dws['dw_guia_despacho']->set_entrable('AUTORIZA_GD_FACTURADA', true);	
		else
			$this->dws['dw_guia_despacho']->set_entrable('AUTORIZA_GD_FACTURADA', false);
	}
	function new_record() {
		...
		$this->dws['dw_guia_despacho']->set_item(0, 'DISPLAY_USU_VENDEDOR', '');
		$this->dws['dw_guia_despacho']->set_item(0, 'DESDE_BNT_NEW', 'S');
		...
		$this->dws['dw_guia_despacho']->set_item(0, 'DISPLAY_GD_FACTURADA', 'none');
	}
	function load_record() {
		...
		if($AUTORIZA_GD_FACTURADA == 'S'){
			$this->dws['dw_guia_despacho']->set_entrable('AUTORIZA_GD_FACTURADA', false);
			$this->dws['dw_guia_despacho']->set_entrable('MOTIVO_GD_FACTURADA', false);
		}else{
			$this->dws['dw_guia_despacho']->set_entrable('AUTORIZA_GD_FACTURADA', true);
			$this->dws['dw_guia_despacho']->set_entrable('MOTIVO_GD_FACTURADA', true);
		}
		...
		$this->dws['dw_guia_despacho']->set_entrable('COD_USU_VENDEDOR_RESP', true);
		...
		if ($COD_ESTADO_DOC_SII == self::K_ESTADO_SII_EMITIDA) {
			...
			if ($COD_DOC  != '') {	
				...
				$this->dws['dw_guia_despacho']->set_entrable('COD_USU_VENDEDOR_RESP', false);
	...
	function save_record($db) {
		...
		if ($this->is_new_record()){
	    	$operacion = 'INSERT';
	    	if($COD_TIPO_GUIA_DESPACHO == 2){
	    	    $COD_DOC = $this->dws['dw_guia_despacho']->get_item(0, 'COD_DOC_H');
	    	}
		}else
	    	$operacion = 'UPDATE';					
...
/*
				...
				if (!$upload) {
					...
					$sql = "SELECT GENERA_MAIL FROM GUIA_DESPACHO WHERE COD_GUIA_DESPACHO =$cod_guia_despacho";
					$result = $db->build_results($sql);
					if($result[0]['GENERA_MAIL'] == 'S')
						$this->envia_mail($db, $cod_guia_despacho); 
					$this->_load_record();
	...
	function envia_mail($db, $COD_GUIA_DESPACHO){
	
		$temp = new Template_appl('plantilla_envio_mail.htm');
		
		$sql = "select 	
				G.NRO_GUIA_DESPACHO
				,CONVERT(VARCHAR, G.FECHA_GUIA_DESPACHO, 103)+' a las '+(SUBSTRING( convert(varchar, getdate(),108),1,5)) FECHA_STRING
				,CONVERT(VARCHAR, G.FECHA_GUIA_DESPACHO, 103) FECHA_GUIA_DESPACHO
				,(dbo.number_format(CONVERT(VARCHAR, G.RUT), 0, ',', '.')  +'-'+ G.DIG_VERIF) RUT
				,G.NOM_EMPRESA
				,CASE 
					when convert(varchar,G.COD_DOC) IS null  THEN 'NO ESPECIFICA'
					ELSE convert(varchar,G.COD_DOC) 
			 	 END COD_NOTA_VENTA	
				,U.NOM_USUARIO EMISOR
				,G.REFERENCIA
				,G.OBS
				,CASE
					WHEN G.COD_DOC IS NULL THEN
						(SELECT NOM_USUARIO FROM USUARIO  WHERE COD_USUARIO = G.COD_USU_VENDEDOR_RESP) 
					ELSE	
						(SELECT UL.NOM_USUARIO FROM USUARIO UL,NOTA_VENTA WHERE COD_NOTA_VENTA = G.COD_DOC
						AND UL.COD_USUARIO = COD_USUARIO_VENDEDOR1   ) 
				END	REMITENTE
				,CASE
					WHEN G.COD_DOC IS NULL THEN
						(SELECT MAIL FROM USUARIO WHERE COD_USUARIO = G.COD_USU_VENDEDOR_RESP) 
					ELSE	
						(SELECT UL.MAIL FROM USUARIO UL,NOTA_VENTA WHERE COD_NOTA_VENTA = G.COD_DOC
				AND UL.COD_USUARIO = COD_USUARIO_VENDEDOR1) 
				END	MAIL
				,U.MAIL MAIL_EMISOR
				,(select NOM_INDICADOR_TIPO_TRASLADO from INDICADOR_TIPO_TRASLADO where COD_INDICADOR_TIPO_TRASLADO = G.COD_INDICADOR_TIPO_TRASLADO)NOM_INDICADOR_TIPO_TRASLADO
				from GUIA_DESPACHO G, USUARIO U 
				WHERE G.COD_GUIA_DESPACHO = $COD_GUIA_DESPACHO
				AND U.COD_USUARIO = G.COD_USUARIO";
		...
		$dw_item = new datawindow($sql, 'ITEM_GUIA_DESPACHO');
		$dw_item->retrieve();
		$dw_item->habilitar($temp, false);
		
		$subject = "Aviso Guía Despacho Nº ".$result[0]['NRO_GUIA_DESPACHO']." desde Nota de Venta N° ".$result[0]['COD_NOTA_VENTA']." ,Vendedor: ".$result[0]['REMITENTE'];
		$html = $temp->toString();
		
		$mailto = $result[0]['MAIL'];
		$mailtoname = $result[0]['REMITENTE'];
		if($result[0]['COD_NOTA_VENTA']== 'NO ESPECIFICA'){
			$mailcc = "";
			$mailccname = "";
			$str_mailcc		= $mailcc;
			$str_mailccname	= $mailccname;
		}else{
			$mailcc = $result[0]['MAIL_EMISOR'];
			$mailccname = $result[0]['EMISOR'];
			$str_mailcc		.= $mailcc.";";
			$str_mailccname .= $mailccname.";";
		...
		$param = "'INSERT'							--@ve_operacion
				...
				,null								--@ve_fecha_envio
			 	...
			 	,'GUIA_DESPACHO'					--@ve_tipo_doc
			 	,$COD_GUIA_DESPACHO";				//@ve_cod_doc
	...
	function enviar_dte($reenviar = false){
		...
		if($reenviar){
			...
			$sql_folio = "SELECT dbo.f_get_nro_dte('GD') NRO_GUIA_DESPACHO";
		...
		$es_exenta_iva = $contenido[0]['ES_EXENTA_IVA'];
		...
		$a['Encabezado']['IdDoc']['TipoDTE']		= self::K_TIPO_DOC;
		$a['Encabezado']['IdDoc']['Folio']			= 0;
		$a['Encabezado']['IdDoc']['TipoDespacho']	= 2;
	...
	function imprimir_dte($desde_output=false){
		...
		if($guia_despacho > 26706)
			...
			if(file_exists("../../../../PDF/PDFCOMERCIALBIGGI/$year/52_$nro_guia_despacho.pdf"))
				print " <script>window.open('../../../../PDF/PDFCOMERCIALBIGGI/$year/52_$nro_guia_despacho.pdf')</script>";
			else
