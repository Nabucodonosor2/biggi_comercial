...
class wi_guia_recepcion extends wi_guia_recepcion_base {
	...
	function save_record($db) {
		...
		$COD_ESTADO_GUIA_RECEPCION_ANT	= $result[0]['COD_ESTADO_GUIA_RECEPCION'];
	...
	function envia_mail($db, $COD_GUIA_RECEPCION){
		...
		$sql = "SELECT COD_GUIA_RECEPCION
					  ...
					  ,COD_DOC
					  ,CASE
						WHEN GR.TIPO_DOC = 'FACTURA' THEN (SELECT COD_DOC
														   FROM FACTURA
														   WHERE COD_FACTURA = GR.COD_DOC)
						WHEN GR.TIPO_DOC = 'GUIA_DESPACHO' THEN (SELECT COD_DOC
																 FROM GUIA_DESPACHO
		...
		$mailcc = array('ascianca@biggi.cl', 'sergio.pechoante@biggi.cl', 'rescudero@biggi.cl', 'kverdugo@biggi.cl', 'jcatalan@biggi.cl', 'psilva@biggi.cl');
		$mailccname = array('ANGEL SCIANCA','SERGIO PECHOANTE','RAFAEL ESCUDERO','KARINA VERDUGO','JOSE CATALAN', 'PIERO SILVA');
