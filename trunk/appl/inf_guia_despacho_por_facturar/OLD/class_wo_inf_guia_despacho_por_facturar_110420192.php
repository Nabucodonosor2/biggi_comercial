...
class wo_inf_guia_despacho_por_facturar extends w_informe_pantalla {
   function wo_inf_guia_despacho_por_facturar() {
		...
		$sql_u = "select COD_USUARIO ,NOM_USUARIO from USUARIO order by	NOM_USUARIO ASC";
		$this->add_header(new header_drop_down('NOM_USUARIO', 'N.COD_USUARIO_VENDEDOR1', 'Vendedor', $sql_u));
	...
	function print_informe() {
		...
		if($filtro[0] == 'Vendedor'){
		    $db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		    $cod_usuario = $this->headers['NOM_USUARIO']->valor_filtro;
		    $sql_u = "SELECT INI_USUARIO from USUARIO WHERE COD_USUARIO = $cod_usuario";
