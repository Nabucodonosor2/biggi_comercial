...
$dw_resultado->add_control(new static_text('ANO'));
...
if (isset($_POST['b_print_x'])) {
	...
	$sql_centro_costo = "SELECT	DISTINCT(CC.NOM_CENTRO_COSTO) CENTRO_COSTO
	...
	$labels['str_mes'] = $centro_costo;//$result[0]['NOM_MES'];
   $labels['str_ano'] = $ano2;
	...
	$sql = "SELECT	(SELECT NOM_MES FROM MES ME WHERE MONTH(RE.FECHA_NOTA_VENTA) = ME.COD_MES) NOM_MES
				...
				,SUM(RE.PAGO_VENDEDOR)   PAGO_VENDEDOR
				,CC.NOM_CENTRO_COSTO
		FROM INF_RESULTADO RE, CENTRO_COSTO CC
		where COD_USUARIO = $cod_usuario
		AND RE.CENTRO_COSTO = CC.COD_CENTRO_COSTO
		GROUP BY MONTH(RE.FECHA_NOTA_VENTA), CC.NOM_CENTRO_COSTO
		ORDER BY MONTH(RE.FECHA_NOTA_VENTA) ASC";
	...
	$worksheet->write(1, 0, "MES", $border_item_left_bold_2);
	$worksheet->merge_cells(1,0,2,0);
	$worksheet->write(2, 0, " ", $space_item_bold_bottom_1);
	$worksheet->write(1, 1, "RESULTADOS", $text_blue_bold_center_top);
	$worksheet->merge_cells(1,1,1,2);
	$worksheet->write(1, 2, " ", $space_item_bold_top);
	...
	$worksheet->write(1, 10, " ", $space_item_bold_top_2);
	...
	$worksheet->write(1, 13, " ", $space_item_bold_top_2);
	$worksheet->write(1, 14, " ", $space_item_right_bold);
	...
	$sql_centro_costo = "SELECT	DISTINCT(CC.NOM_CENTRO_COSTO) CENTRO_COSTO, '$ano2' ANO
					FROM	INF_RESULTADO INF, CENTRO_COSTO CC
					WHERE CC.COD_CENTRO_COSTO = INF.CENTRO_COSTO 
					AND INF.COD_USUARIO =$cod_usuario";
	$result_centro_costo = $db_user->build_results($sql_centro_costo);
