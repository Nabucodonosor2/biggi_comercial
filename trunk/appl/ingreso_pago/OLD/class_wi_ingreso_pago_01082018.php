...
class dw_ingreso_pago_factura extends datawindow {
	...
	function dw_ingreso_pago_factura() {		
		$sql = "SELECT 'S' SELECCION
						...
						,0 MONTO_ASIGNADO_C
						,U.INI_USUARIO
				FROM	INGRESO_PAGO_FACTURA IPF, INGRESO_PAGO IP, FACTURA F, NOTA_VENTA NV, USUARIO U
				WHERE	IPF.COD_INGRESO_PAGO = {KEY1} AND
						...
						NV.COD_NOTA_VENTA = F.COD_DOC AND
...
class wi_ingreso_pago_base extends w_input {
	...
	function envio_mail(){
		...
		$lista_mail_cc = "'sergio.pechoante@biggi.cl;ascianca@biggi.cl;rescudero@biggi.cl;jcatalan@biggi.cl;'";
		$lista_mail_cc_name = "'SERGIO PECHOANTE;ANGEL SCIANCA;RAFAEL ESCUDERO;JOSE CATALAN;'";
...
class print_ingreso_pago extends reporte {	
	...
	function modifica_pdf(&$pdf) {	
			...
			$sql_factura = "SELECT	F.NRO_FACTURA
							...
							AND		U.COD_USUARIO = NV.COD_USUARIO_VENDEDOR1
							AND		NV.COD_NOTA_VENTA = F.COD_DOC
