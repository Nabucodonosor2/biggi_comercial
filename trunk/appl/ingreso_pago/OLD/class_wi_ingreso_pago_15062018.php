...
class wi_ingreso_pago_base extends w_input {
	...
	function envio_mail(){
		...
		$sql = "SELECT U.NOM_USUARIO
					  ...
					  ,dbo.number_format(NV.TOTAL_CON_IVA, 0, ',', '.') TOTAL_CON_IVA
					  ,E1.NOM_EMPRESA NOM_EMPRESA_NV
					  ,NV.COD_NOTA_VENTA
				...
				AND E1.COD_EMPRESA = NV.COD_EMPRESA";
		...
		if(count($result) > 1){
			...
			$asunto = "'Pago F/$nro_factura $".$sum_monto_asignado." CL. $nom_empresa'";
		...
		$temp->setVar("NOM_VENDEDOR1_NV", $nom_vendedor);
		$temp->setVar("COD_INGRESO_PAGO", $COD_INGRESO_PAGO);
		$temp->setVar("FECHA_INGRESO_PAGO", $result[0]['FECHA_INGRESO_PAGO']);
		$temp->setVar("HORA_INGRESO_PAGO", $result[0]['HORA_INGRESO_PAGO']);
		$temp->setVar("RUT", $rut);
		$temp->setVar("NOM_EMPRESA", $nom_empresa);
		$temp->setVar("TOTAL_INGRESO_PAGO", $sum_monto_asignado);
		$temp->setVar("NOM_USUARIO", $this->nom_usuario);
		...
		/*$lista_mail_cc = "'sergio.pechoante@biggi.cl;rescudero@biggi.cl;jcatalan@biggi.cl;ascianca@biggi.cl'";
		$lista_mail_cc_name = "'SERGIO PECHOANTE;RAFAEL ESCUDERO;JOSE CATALAN;ANGEL SCIANCA'";*/
		...
		if($db->EXECUTE_SP($sp, $param))
			$this->alert("Se ha enviado correo alertando sobre Ingreso de Pago.");
		else
			$this->alert("Se ha producido un error al enviar el correo.");
