...
class dw_ingreso_pago_nota_venta extends datawindow {
	...
	function dw_ingreso_pago_nota_venta() {
		$sql = "SELECT 'S' SELECCION_NV
						...
						,U.INI_USUARIO
				FROM	INGRESO_PAGO_FACTURA IPF, INGRESO_PAGO IP, NOTA_VENTA NV, USUARIO U
				WHERE	IPF.COD_INGRESO_PAGO = {KEY1} AND
...
class dw_doc_ingreso_pago extends datawindow {
	...
	const K_TIPO_DOC_TARJETA_CREDITO	= 6;
	...
	function dw_doc_ingreso_pago() {
		$sql = "SELECT DIP.COD_DOC_INGRESO_PAGO
				...
				WHERE 	DIP.COD_INGRESO_PAGO = {KEY1}
		...
		$this->add_control(new static_text('CUOTAS_WEBPAY'));
		$this->add_control($control = new edit_num('NRO_CUOTAS_TBK',10, 10, 0, true, false, false));
		$control->set_onChange("change_cuotas_tbk(this);");
	}
	function insert_row($row=-1) {
		$row = parent::insert_row($row);
		return $row;
	...
	function fill_record(&$temp, $record) {
		...
		if ($COD_TIPO_DOC_PAGO==self::K_TIPO_DOC_PAGO_EFECTIVO) {
			...
			$this->controls['NRO_CUOTAS_TBK']->type = 'hidden';
		}else if($COD_TIPO_DOC_PAGO==self::K_TIPO_DOC_TARJETA_CREDITO){
			$this->controls['NRO_CUOTAS_TBK']->type = 'text';
		...
		$CUOTAS_WEBPAY = $this->get_item($record, 'CUOTAS_WEBPAY');
		if($CUOTAS_WEBPAY == ''){
			$this->set_item($record, 'DISPLAY_CB', '');
			$this->set_item($record, 'DISPLAY_CW', 'none');
		}else{
			$this->set_item($record, 'DISPLAY_CB', 'none');
			$this->set_item($record, 'DISPLAY_CW', '');
		}
	...
	function update($db)	{
		...
		for ($i = 0; $i < $this->row_count(); $i++){
				...
				if ($statuts == K_ROW_NOT_MODIFIED || $statuts == K_ROW_NEW)
					...
					$NRO_CUOTAS_TBK 		= $this->get_item($i, 'NRO_CUOTAS_TBK');
					...
					$NRO_CUOTAS_TBK			= ($NRO_CUOTAS_TBK =='') ? "null" : $NRO_CUOTAS_TBK;
...
class wi_ingreso_pago_base extends w_input {
	...
	const K_AUTORIZA_ESTADO_APROBADA		= '992510';
	const K_PERMISO_DEUDA_CASTIGADA			= '992520';
	const K_MODIFICA_INGRESO_PAGO_TBK			= '992525';
	...
	function wi_ingreso_pago_base($cod_item_menu) {
		...
		$sql="SELECT 	IP.COD_INGRESO_PAGO
						...
						,COD_DOC_ORIGEN
						,case IP.COD_ESTADO_INGRESO_PAGO
							when ".self::K_ESTADO_INGRESO_PAGO_ANULADA." then 'ANULADA'
							else ''
						end TITULO_ESTADO_NOTA_VENTA
						,CASE
							WHEN NOM_TIPO_ORIGEN_PAGO = 'WEBPAY PLUS' THEN 'Cuotas'
							ELSE 'Banco'
						END LBL_BANCO
						,CASE
							WHEN NOM_TIPO_ORIGEN_PAGO = 'WEBPAY PLUS'
	...
	function new_record() {
		...
		$this->dws['dw_ingreso_pago']->set_item(0, 'LBL_BANCO', 'Banco');
		$this->dws['dw_ingreso_pago']->set_item(0, 'NOM_LABEL_DOC', 'Nº Documento');

		$priv = $this->get_privilegio_opcion_usuario(self::K_PERMISO_DEUDA_CASTIGADA, $this->cod_usuario);
		if ($priv == 'E')
	...
	function load_record() {
		...
		if ($COD_ESTADO_INGRESO_PAGO == self::K_ESTADO_INGRESO_PAGO_EMITIDA) {
			...
			if($priv_m == 'E'){
				...
				$vl_cod_usuario = $result_sum[0]['COD_USUARIO'];
				if($vl_cod_usuario == $this->cod_usuario){
					//MH 06072022 AQUI EVALUA LOS TIPOS DE DOCUMENTOS QUE LOS VENDEDORES CON PRIVILEGIO AUTORIZADO MODIFICA_INGRESO_PAGO_TBK
					//PUEDEN AUTORIZAR, HOY SP SOLICITA SACAR DEL LISTADO LOS COD_TIPO_DOC_PAGO = EFECTIVO SOLO QUEDAN TC Y TB
					$sql2 = " SELECT TOP 1 COD_DOC_INGRESO_PAGO FROM DOC_INGRESO_PAGO 
								WHERE COD_INGRESO_PAGO = $cod_ingreso_pago AND COD_TIPO_DOC_PAGO IN (5,6)";
					$result_sql2	= $db->build_results($sql2);
					$vl_COD_DOC_INGRESO_PAGO = $result_sql2[0]['COD_DOC_INGRESO_PAGO'];
					if($vl_COD_DOC_INGRESO_PAGO <> ''){
							$INGRESO_PAGO_TBK = 'S';
					}
				}
			}

			$priv = $this->get_privilegio_opcion_usuario(self::K_AUTORIZA_ESTADO_APROBADA, $this->cod_usuario);
			if($priv == 'E'){
				$sql = "select COD_ESTADO_INGRESO_PAGO
							  ,NOM_ESTADO_INGRESO_PAGO
						from ESTADO_INGRESO_PAGO
						order by COD_ESTADO_INGRESO_PAGO";
			}else if($INGRESO_PAGO_TBK == 'S'){
				$sql = "select COD_ESTADO_INGRESO_PAGO
							  ,NOM_ESTADO_INGRESO_PAGO
						from ESTADO_INGRESO_PAGO
						order by COD_ESTADO_INGRESO_PAGO";
				...
				$sql = "select 	COD_ESTADO_INGRESO_PAGO
							  ,NOM_ESTADO_INGRESO_PAGO
						from ESTADO_INGRESO_PAGO
						where COD_ESTADO_INGRESO_PAGO <> 2 --confirmada
						order by COD_ESTADO_INGRESO_PAGO";
			}
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		...
		if($boton == 'envio_mail'){
			if($habilita){
				$control = '<input name="b_envio_mail" id="b_envio_mail" src="../../images_appl/b_envio_mail.jpg" type="image" '.
							 'onMouseDown="MM_swapImage(\'b_envio_mail\',\'\',\'../../images_appl/b_envio_mail_click.jpg\',1)" '.
							 'onMouseUp="MM_swapImgRestore()" onMouseOut="MM_swapImgRestore()" '.
							 'onMouseOver="MM_swapImage(\'b_envio_mail\',\'\',\'../../images_appl/b_envio_mail_over.jpg\',1)"
							 onClick="return true;"/>';
			}else{
				$control = '<img src="../../images_appl/b_envio_mail_d.jpg">';
			}

			$temp->setVar("WI_ENVIO_MAIL", $control);
		}
	}
	...
	function navegacion(&$temp){
		...
		if($cod_estado_ingreso_pago == 2)
			$this->habilita_boton($temp, 'envio_mail', true);
		else
			$this->habilita_boton($temp, 'envio_mail', false);
	...
	function procesa_event(){
		if(isset($_POST['b_envio_mail_x']))
			$this->envio_mail();
		else if(isset($_POST['b_add_x'])){
			$this->agregar_ingreso_pago();
		}
		else
			parent::procesa_event();
	}
	function agregar_ingreso_pago(){
		session::set('DESDE_wo_'.$this->nom_tabla, 'desde output');	// para indicar que viene del output
		$url = $this->get_url_mantenedor();
		header ('Location:'.$url.'/wi_'.$this->nom_tabla.'.php?rec_no='.K_NEW_RECORD.'&cod_item_menu='.$this->cod_item_menu);

	}
	function envio_mail(){
		$temp = new Template_appl('envio_mail_ip.htm');

		$COD_INGRESO_PAGO	= $this->get_key();
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);

		$sql =     "SELECT 'NV ' + CONVERT(VARCHAR, COD_DOC) COD_DOCUMENTO
						,CONVERT(VARCHAR, FECHA_NOTA_VENTA, 103) FECHA_DOCUMENTO
						,NOM_EMPRESA
						,dbo.number_format(IPF.MONTO_ASIGNADO, 0, ',', '.') MONTO_ASIGNADO
						,NOM_USUARIO
						,U.MAIL
					FROM INGRESO_PAGO_FACTURA IPF
						,NOTA_VENTA NV
						,EMPRESA E
						,USUARIO U
					WHERE IPF.COD_INGRESO_PAGO = $COD_INGRESO_PAGO
					AND IPF.TIPO_DOC = 'NOTA_VENTA'
					AND IPF.MONTO_ASIGNADO > 0 --MH 28072020 SE CREAN REGISTRO EN INGRESO_PAGO_FACTURA EN CERO Y ENSUCIAN MAIL
					AND NV.COD_NOTA_VENTA = IPF.COD_DOC
					AND NV.COD_EMPRESA = E.COD_EMPRESA
					AND U.COD_USUARIO = NV.COD_USUARIO_VENDEDOR1
					UNION
					SELECT CONVERT(VARCHAR, NRO_FACTURA) COD_DOCUMENTO
						,CONVERT(VARCHAR, FECHA_FACTURA, 103) FECHA_DOCUMENTO
						,F.NOM_EMPRESA
						,dbo.number_format(IPF.MONTO_ASIGNADO, 0, ',', '.') MONTO_ASIGNADO
						,NOM_USUARIO
						,U.MAIL
		...
		$result	= $db->build_results($sql);
		$dw_ingreso_pago = new datawindow($sql, "INGRESO_PAGO");
		$dw_ingreso_pago->retrieve();
		$dw_ingreso_pago->habilitar($temp, false);

		$nom_empresa		= $result[0]['NOM_EMPRESA'];
		$nom_vendedor		= $result[0]['NOM_USUARIO'];

		$sql = "SELECT dbo.number_format(SUM(MONTO_ASIGNADO), 0, ',', '.') MONTO_ASIGNADO
				FROM INGRESO_PAGO_FACTURA
				WHERE COD_INGRESO_PAGO = $COD_INGRESO_PAGO";
		$result_sum	= $db->build_results($sql);
		$sum_monto_asignado = $result_sum[0]['MONTO_ASIGNADO'];
		...
		if(count($result) > 1){
			$old_nom_usuario = array();
			for($i = 0 ; $i < count($result) ; $i++){
				if(!in_array($result[$i]['MAIL'], $old_nom_usuario)){
					$old_nom_usuario[] = $result[$i]['MAIL'];

					$lista_mail_to		.= $result[$i]['MAIL'].";";
					$lista_mail_to_name	.= $result[$i]['NOM_USUARIO'].";";
				}
			}

			$lista_mail_to		= trim($lista_mail_to, ";");
			$lista_mail_to_name	= trim($lista_mail_to_name, ";");

			if($result_fa[0]['COUNT'] > 0)
				$asunto	= "'Pago F/VARIAS $".$sum_monto_asignado." CL. $nom_empresa'";
			else
				$asunto	= "'Pago NV/VARIAS $".$sum_monto_asignado." CL. $nom_empresa'";

		}else if(count($result) == 1){
			$cod_documento = $result[0]['COD_DOCUMENTO'];

			if($result_fa[0]['COUNT'] == 1)
				$asunto = "'Pago F/$cod_documento $".$sum_monto_asignado." CL. $nom_empresa'";
			else
				$asunto = "'Pago $cod_documento $".$sum_monto_asignado." CL. $nom_empresa'";

			$lista_mail_to		= $result[0]['MAIL'];
			$lista_mail_to_name	= $result[0]['NOM_USUARIO'];
		}

		$sql = "SELECT CONVERT(VARCHAR, FECHA_INGRESO_PAGO, 103) FECHA_INGRESO_PAGO
					  ,SUBSTRING(CONVERT(VARCHAR, FECHA_INGRESO_PAGO, 108), 0, 6) HORA_INGRESO_PAGO
				FROM INGRESO_PAGO
				WHERE COD_INGRESO_PAGO = $COD_INGRESO_PAGO";
		$result	= $db->build_results($sql);

		$temp->setVar("NOM_VENDEDOR1", $nom_vendedor);
		$temp->setVar("COD_INGRESO_PAGO", $COD_INGRESO_PAGO);

		$html = $temp->toString();

		//ENVIO OFICIAL
		$lista_mail_to		= "'$lista_mail_to'";
		$lista_mail_to_name	= "'$lista_mail_to_name'";

		$lista_mail_cc = "'sergio.pechoante@biggi.cl;ascianca@biggi.cl;jcatalan@biggi.cl;'";
		$lista_mail_cc_name = "'SERGIO PECHOANTE;ANGEL SCIANCA;JOSE CATALAN;'";

		$lista_mail_bcc = "'mherrera@biggi.cl'";
		$lista_mail_bcc_name = "'MARCELO HERRERA'";
		...
		if($db->EXECUTE_SP($sp, $param))
			$this->alert("Se ha enviado correo alertando sobre Ingreso de Pago Nº $COD_INGRESO_PAGO.");
		else
	...
	function print_record() {
		...
		$sql= "SELECT	--datos ingreso pago
            			...
            			,DIP.NRO_CUOTAS_TBK
						,IP.OTRO_ANTICIPO
						,dbo.f_status_anticipo(IP.COD_INGRESO_PAGO) ESTADO_ANTICIPO
		...
		$rpt = new print_ingreso_pago($sql, $this->root_dir.'appl/ingreso_pago/ingreso_pago.xml', $labels, "Ingreso Pago ".$cod_ingreso_pago.".pdf", 'logo');
		$this->_load_record();
		return true;
	}
}
class print_ingreso_pago extends reporte {
	...
	function modifica_pdf(&$pdf) {
			...
			$sql_factura = "SELECT	F.NRO_FACTURA
                  ...
                  ,case when IP.OTRO_ANTICIPO > 999 	then 'SI' else 'NO' end VER_OTRO_ANTICIPO
                  ,IP.OTRO_ANTICIPO
							FROM	INGRESO_PAGO_FACTURA IPF, INGRESO_PAGO IP, FACTURA F
							WHERE	IPF.COD_INGRESO_PAGO = $cod_ingreso_pago
							AND		IPF.TIPO_DOC = 'FACTURA'
							...
							AND		IPF.MONTO_ASIGNADO > 0 --MH 28072020 SE CREAN REGISTRO EN INGRESO_PAGO_FACTURA EN CERO Y ENSUCIAN PRINT
							ORDER BY	F.NRO_FACTURA ASC";
      ...
      $VERF_OTRO_ANTICIPO		=	$result_factura[0]['VER_OTRO_ANTICIPO'];
      
      $VERN_OTRO_ANTICIPO		=	$result_nota_venta[0]['VER_OTRO_ANTICIPO'];
			...
			$pdf->SetXY(30,$y_fa + 5);
			$pdf->Cell(450,17,'DOCUMENTOS INGRESO PAGO', 'LTR', '','C');

			/*LABEL Nº DOC*/
			$nom_label_doc	=	$result[0]['NOM_LABEL_DOC'];
			...
			$pdf->Cell(60,15,'V1', 'LTB', '','C');
			$pdf->SetXY(215,$y_fa + 20);
			$pdf->Cell(70,15,'Fecha', 'LTRB', '','C');
			$pdf->SetXY(285,$y_fa + 20);
			$pdf->Cell(65,15,'Monto Docto.', 'TRB', '','C');
			$pdf->SetXY(350,$y_fa + 20);
			$pdf->Cell(65, 15,'Saldo Docto.', 'TRB', '','C');
			$pdf->SetXY(415,$y_fa + 20);
			$pdf->Cell(65, 15,'Monto Pago', 'TRB', '','C');
      if(($VERF_OTRO_ANTICIPO=='SI')||($VERN_OTRO_ANTICIPO=='SI')) {
      $pdf->SetXY(480,$y_fa + 20);
			$pdf->Cell(65, 15,'ANTICIPOS', 'TRB', '','C');
      }
			...
			if($count_factura != 0){
				...
				$pdf->SetFont('Arial','',9);
					...
					for($i=0, $ii = 0; $i<$count_factura; $i++, $ii++){
						...
						$nom_usu_vendedor	=	$result_factura[$i]['INI_USUARIO'];
            ...
            $otro_anticipo	=	$result_factura[$i]['OTRO_ANTICIPO'];
						$otro_anticipo	=	number_format($otro_anticipo, 0, ',', '.');                      
						...
						$pdf->Cell(60,15,$nom_usu_vendedor, 'LTRB', '','C');

						$pdf->SetXY(215,$y_fa + 35+(15*$ii));
						$pdf->Cell(70,15,$fecha_factura, 'LTB', '','R');

						$pdf->SetXY(285,$y_fa + 35+(15*$ii));
						$pdf->Cell(65,15,$monto_factura, 'LTRB', '','R');

						$pdf->SetXY(350,$y_fa + 35+(15*$ii));
						$pdf->Cell(65,15,$saldo_factura, 'LTRB', '','R');

						$pdf->SetXY(415,$y_fa + 35+(15*$ii));
						$pdf->Cell(65,15,$monto_pago_factura, 'TRB', '','R');
            if($VERF_OTRO_ANTICIPO=='SI') {
              $pdf->SetXY(480,$y_fa + 35+(15*$ii));
					  	$pdf->Cell(65,15,$otro_anticipo, 'TRB', '','R');
            }
					}
			...
			if($count_nota_venta != 0){
				...
				for($i=0, $ii = 0; $i<$count_nota_venta; $i++, $ii++){
					...
					$nom_usu_vendedor	=	$result_nota_venta[$i]['INI_USUARIO'];
          ...
          $otro_anticipo	=	$result_nota_venta[$i]['OTRO_ANTICIPO'];
						$otro_anticipo	=	number_format($otro_anticipo, 0, ',', '.'); 
					...
					$pdf->Cell(60,15,$nom_usu_vendedor, 'LTRB', '','C');
					$pdf->SetXY(215,$y_fa + 35+(15*$ii));
					$pdf->Cell(70,15,$fecha_nv, 'LTB', '','R');
					$pdf->SetXY(285,$y_fa + 35+(15*$ii));
					$pdf->Cell(65,15,$monto_nv, 'LTRB', '','R');
					$pdf->SetXY(350,$y_fa + 35+(15*$ii));
					$pdf->Cell(65,15,$saldo_nv, 'TRB', '','R');
					$pdf->SetXY(415,$y_fa + 35+(15*$ii));
					$pdf->Cell(65,15,$monto_pago_nv, 'TRB', '','R');
          if($VERN_OTRO_ANTICIPO=='SI') {                                
            $pdf->SetXY(480,$y_fa + 35+(15*$ii));
					  $pdf->Cell(65,15,$otro_anticipo, 'TRB', '','R');
          }
				}
