function validate() {
	...
	for (var i = 0; i < aTR.length; i++) {
		...
		else if (cod_tipo_doc_pago == tipo_doc_pago_tarjeta_credito){

			nro_cuotas_tbk = document.getElementById('NRO_CUOTAS_TBK_'+ record).value;

			if(nro_cuotas_tbk == ''){
				alert ("Debe ingresar las cuotas.");
				document.getElementById('NRO_CUOTAS_TBK_' + record).focus();
				return false;
			}else if(to_num(nro_cuotas_tbk) > 24){
				alert ("no puede ingresar un mumero mayor a 24 cuotas.");
				document.getElementById('NRO_CUOTAS_TBK_' + record).focus();
				return false;
			}
      if (nro_doc == 0){
	...
	// MH 11052022 SP+JC SOLICITAN QUE EL MAXIMO MONTO DE LOS CAMPOS "OTROS INGRESOS" Y "GASTOS VARIOS" NO SEAN SUPERIOR A 1000
	if (otro_gasto > 1000){
		alert ('El monto máximo del campo "Gastos Varios" no puede ser superior a $1.000.-');
		return false;
	}
	if (otro_ingreso > 1000){
		alert ('El monto máximo del campo "Otros Ingresos" no puede ser superior a $1.000.-');
		return false;
	}
	
	if (sum_ingreso != sum_gasto){
		alert('¡No se puede registrar el ingreso de pago!. El "Total de Documentos" es ...es distinto al "Total Asignado".\n \n - Total de Documentos: $'+number_format(sum_ingreso, 0, ',', '.')+'\n - Total Asignado: $'+number_format(sum_gasto, 0, ',', '.'));
		return false;
	}
	return true;
}
...
function valida_tipo_doc_pago(ve_tipo_doc_pago) {
	...
	var nro_cuotas_tbk = document.getElementById('NRO_CUOTAS_TBK_'+ record);
	...
	var kl_tipo_doc_deuda_castigada = 13;

	if (ve_tipo_doc_pago.value == kl_tipo_doc_efectivo || ve_tipo_doc_pago.value == kl_tipo_doc_deuda_castigada){
		nro_doc.value = '';
		...
		cod_banco.setAttribute('disabled', "");
		nro_cuotas_tbk.removeAttribute('disabled');
		cod_banco.value = '';
...
function usar_anticipo() {
	...
	var url = "usar_anticipo.php?cod_empresa="+cod_empresa;
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 330,
...
function usar_nota_credito() {
	...
	var url = "usar_nota_credito.php?cod_empresa="+cod_empresa;
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 330,
...
function usar_factura_compra() {
	...
	var url = "usar_factura_compra.php?cod_empresa="+cod_empresa;
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 330,
...
function change_cuotas_tbk(ve_campo){
	var record = get_num_rec_field(ve_campo.id);

	nro_cuotas_tbk = document.getElementById('NRO_CUOTAS_TBK_'+ record).value;

	if(to_num(nro_cuotas_tbk) > 24){
		alert ("no puede ingresar un mumero mayor a 24 cuotas.");
		document.getElementById('NRO_CUOTAS_TBK_' + record).value = 24;
		document.getElementById('NRO_CUOTAS_TBK_' + record).focus();
		return false;
	}
}
