...
class dw_llamado extends datawindow {
	function dw_llamado() {		
		...
		$this->add_control(new static_text('COD_LLAMADO'));
		...
		$this->add_control(new static_text('ORIGEN'));
		
		$sql_usuario_realizado = "select COD_USUARIO COD_USUARIO_REALIZADO, 
							  NOM_USUARIO
						 from usuario";
		$this->add_control(new drop_down_dw('COD_USUARIO_REALIZADO', $sql_usuario_realizado, 100));
		//$this->add_control(new drop_down_dw('COD_USUARIO_REALIZADO'));
		
		$this->add_control(new edit_text('COD_CONTACTO',10,10, 'hidden'));
		$this->add_control($control = new edit_text_upper('NOM_CONTACTO',65, 100));
		...
		$this->add_control($control = new edit_text_upper('NOM_PERSONA',65, 100));
		$control->set_onChange("help_contacto(this);");
		...
		$this->add_control(new drop_down_dw('DD_TELEFONO_CONTACTO', $sql, 135));
		$this->add_control(new drop_down_dw('DD_TELEFONO_PERSONA', $sql, 135));
		...
		$this->add_control(new edit_text_multiline('MENSAJE', 75, 6));
		$this->add_control(new edit_text_multiline('LLAMAR_TELEFONO', 18, 4));
		
		$sql_accion = "select COD_LLAMADO_ACCION, 
							  NOM_LLAMADO_ACCION
						 from LLAMADO_ACCION
					 order by COD_LLAMADO_ACCION";
		$this->add_control(new drop_down_dw('COD_LLAMADO_ACCION', $sql_accion, 168));
		...
		$this->add_control(new edit_text_multiline('GLOSA_REALIZADO', 58, 4));
		...
		$this->add_control(new edit_text('COD_DOC_REALIZADO', 10,10));
	...
	function dw_destinatario() {		
		...
		$sql = "SELECT  COD_LLAMADO_DESTINATARIO  
				 ...
				 where COD_LLAMADO = {KEY1}			
				   and COD_DESTINATARIO not in (select COD_DESTINATARIO from DESTINATARIO_FIJO)					
...
class dw_destinatario_fijo extends datawindow {
	function dw_destinatario_fijo() {
		...
		$sql = "select 'S' SELECCION
				 ...
				 and LD.COD_DESTINATARIO in (select COD_DESTINATARIO from DESTINATARIO_FIJO)
				 ...
				 select 'N' SELECCION
						...
						,NOM_DESTINATARIO NOM_DESTINATARIO_FIJO
				 FROM DESTINATARIO
				 where COD_DESTINATARIO in (select COD_DESTINATARIO from DESTINATARIO_FIJO)
...
class dw_conversacion extends datawindow {
	function dw_conversacion() {		
		...
		$this->add_control(new edit_text_multiline('GLOSA', 57, 1));
...
class wi_llamado extends w_input {
	...
	function new_record() {
        ...
        $this->dws['dw_llamado']->set_item(0, 'ORIGEN', 'Registro manual');
	...
	function load_record() {
		...
		$this->dws['dw_llamado']->set_entrable('COD_USUARIO_REALIZADO', false);
