...
function make_sql_retiro($dia, $cod_usu_vendedor = NULL){
    $sql = "SELECT	F.NRO_FACTURA
						,convert(varchar, F.FECHA_FACTURA, 103) FECHA
						,E.NOM_EMPRESA
						,F.TOTAL_CON_IVA
...
$temp->setVar("PARA_RETIRO", $para_retiro);
...
/***
 * INICIO MAIL PARA REPARTIDOR
 * ***/
$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);

$sql = make_sql_retiro('HOY');
$sql_bitacora_hoy	= $db->build_results($sql);
$count_hoy = count($sql_bitacora_hoy);
...
if($count_hoy <> 0 || $count_ayer <> 0 || $count_manana <> 0){
    ...
    $nom_usuario = 'Leonardo Rojas';//$sql_usuario[0]['NOM_USUARIO'];
    $mail_usuario = 'transdriver10@gmail.com';//$sql_usuario[0]['MAIL'];
    $fecha = $sql_usuario[0]['FECHA'];
    $para_retiro = 'Se ha indicado notificarle de las siguientes bitácoras de cobranza por administracion BIGGI.';
...
$sql = "exec spi_tiene_factura_vendedor";
$result = $db->build_results($sql);
