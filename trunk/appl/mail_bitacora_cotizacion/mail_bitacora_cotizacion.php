...
/*  Esta función arma la consulta con los parámetros día y el código del usuario,
 *  en donde el día puede ser 'HOY', 'MAÑANA' o 'AYER'
 */
function make_sql($dia, $cod_usuario = NULL){
	...
	if($dia <> 'RECHAZADAS')
		...
		/*07-07-2016 MH: Se hace cambio de fecha compromiso a partir de enero del 2016 en adelante (01/01/2016)
		  debido a que los correos pesan mas de 5 MB*/
		$sql .= " AND BC.FECHA_COMPROMISO >= {ts '2018-01-01 00:00:00.000'}
				 ORDER BY BC.FECHA_COMPROMISO ASC";
...
for ($i=0; $i<count($result); $i++) {
	...
	if($cod_usuario <> 6 && $cod_usuario <> 10)
		continue;
	...
	$mail->AddAddress('mherrera@biggi.cl', 'MH');
	$mail->AddAddress('icampos@integrasystem.cl', 'IC');
