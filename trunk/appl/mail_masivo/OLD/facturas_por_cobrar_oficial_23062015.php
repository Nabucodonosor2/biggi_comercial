<?php
	...
	for($i=0;$i < count($sql_cod_vendedor); $i++) {
		...
		//$para = $mail_vendedor . ', '; // atención a la coma
		  $para .= 'ascianca@biggi.cl, sergio.pechoante@biggi.cl, kverdugo@biggi.cl, rescudero@biggi.cl';
		//$para .= 'sergio.pechoante@biggi.cl, kverdugo@biggi.cl, rescudero@biggi.cl';
		//$para .= 'ascianca@biggi.cl';
	...
	/// Inicio MH regulariza el 24/06/2013
		...
		$mail->AddAddress('kverdugo@biggi.cl', 'Karina Verdugo');
		$mail->AddAddress('rescudero@biggi.cl', 'Rafael Escudero');

		$mail->AddCC('cscianca@biggi.cl', 'Claudia Scianca');
		if ($cod_vendedor == 63)	//VICTORIA POBLETE
			$mail->AddCC('cmorales@biggi.cl', 'Claudia Morales');
