...
class dw_ncprov extends dw_help_empresa{
	...
	function dw_ncprov() {
		...
		$sql = "SELECT N.COD_NCPROV
						...
						,COD_TIPO_NCPROV
						,case N.COD_ESTADO_NCPROV
							when ".self::K_ESTADO_NCPROV_ANULADA." then 'ANULADA'
							else ''
		...
		$this->add_control($control = new edit_num('NRO_NCPROV',10, 10, 0, true, false, false));
		$control->set_onChange("ajax_valida_nro_doc();");
...
class wi_ncprov extends w_input {
	...
	function new_record() {
		...
		$this->dws['dw_ncprov']->set_item(0, 'COD_TIPO_NCPROV', 2); //NC_ELECTRONICA
	}
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		if ($boton=='b_add'){
			if ($habilita){
				$temp->setVar("WI_ADD", '<input name="b_add" id="b_add" src="../../../../commonlib/trunk/images/b_add.jpg" type="image" '.
										'onMouseDown="MM_swapImage(\'b_create\',\'\',\'../../../../commonlib/trunk/images/b_add_click.jpg\',1)" '.
				...
				$temp->setVar("WI_ADD", '<img src="../../../../commonlib/trunk/images/b_add_d.jpg"/>');
			}	
		}else
	...
	function habilitar(&$temp, $habilita){
		...
		if($this->get_key() <> '' && $this->modify == false)
	...
	function procesa_event() {		
		if(isset($_POST['b_add_x'])){
			$this->dws['dw_ncprov']->retrieve(0);
			$this->dws['dw_ncprov']->set_entrable('RUT'				, true);
			$this->dws['dw_ncprov']->set_entrable('ALIAS'			, true);
			$this->dws['dw_ncprov']->set_entrable('COD_EMPRESA'		, true);
			$this->dws['dw_ncprov']->set_entrable('NOM_EMPRESA'		, true);
			$this->dws['dw_ncprov']->set_entrable('NRO_NCPROV'		, true);
			$this->dws['dw_ncprov']->set_entrable('FECHA_NCPROV'	, true);
			$this->_new_record();
		}else
