...
class wi_nota_credito_base extends w_cot_nv {
	...
	function enviar_dte(){
		...
		$sql = "SELECT PORC_IVA
					  ,(SELECT F.COD_ESTADO_LIBRE_DTE FROM FACTURA F WHERE F.COD_FACTURA = N.COD_DOC) COD_ESTADO_LIBRE_DTE_FA
				FROM NOTA_CREDITO N
				WHERE COD_NOTA_CREDITO = $cod_nota_credito";
		$result = $db->build_results($sql);
		$COD_ESTADO_LIBRE_DTE_FA = $result[0]['COD_ESTADO_LIBRE_DTE_FA'];
		/*
		 * SE COMENTA LA SIGUIENTE VALIDACION, SOLICITADA POR MH POR NO REALIZAR 
		 * VALIDACION PREVIA DEL ESTADO AL ENVIAR
		 * 16/02/2018
		 * CUALQUIER COSA LINCHENLO A ÉL.
		 */
	/*	if($COD_ESTADO_LIBRE_DTE_FA <> 1 && $COD_ESTADO_LIBRE_DTE_FA <> 2 && $COD_ESTADO_LIBRE_DTE_FA <> 3){
			$this->_load_record();
