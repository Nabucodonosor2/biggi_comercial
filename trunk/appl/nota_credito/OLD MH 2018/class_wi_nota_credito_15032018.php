...
class wi_nota_credito_base extends w_cot_nv {
	...
	function enviar_dte(){
		...
		if($revision_estado <> 'EPR' && $revision_estado <> 'RPR' && $revision_estado <> 'RLV'){
			$this->_load_record();
	...
	function imprimir_dte_xml(){
		$cod_nota_credito = $this->get_key();
		print " <script>window.open('../common_appl/print_dte_xml.php?cod_documento=$cod_nota_credito&DTE_ORIGEN=61')</script>";
		$this->_load_record();
	}
		...
		if($rep_response[1] == '' && $TRACK_ID_DTE == '' && $COD_ESTADO_DOC_SII == self::K_ESTADO_SII_EMITIDA){
			...
			if (($nro_nc_dte <> '') && ($EnvioDTExml <> '')&& ($track_id <> '')){
				...
				$param = "'SAVE_DTE'			--@ve_operacion
					...
					if ($db->EXECUTE_SP($sp, $param)) {
						$db->COMMIT_TRANSACTION();
						print " <script>window.open('print_dte.php?cod_nota_credito=$cod_nota_credito')</script>";
						$this->_load_record();
					}else{
						$db->ROLLBACK_TRANSACTION();
					}
	...
	function xml_dte(){
		$cod_nota_credito = $this->get_key();
		$name_archivo = "XML_DTE_NOTA_CREDITO_".$this->get_key_para_ruta_menu().".xml";
