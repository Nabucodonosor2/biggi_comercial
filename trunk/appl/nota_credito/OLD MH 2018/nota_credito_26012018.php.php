...
class wi_nota_credito_base extends w_cot_nv {
	...
	function enviar_dte(){
		...
		if($result[0]['PORC_IVA'] == 0){
			$this->_load_record();
			print " <script>alert('No se pueden emitir Nota Crédito Excentas. Favor contactarse con Integrasystem');</script>";
			return false;
		}		
		...
		$FolioRef		= $contenido[0]['NRO_FA'];
		$RazonRef		= $contenido[0]['REFERENCIA'];
		$CodRef			= $contenido[0]['COD_TIPO_NOTA_CREDITO'];
		$DescuentoMonto1= $contenido[0]['MONTO_DSCTO1'];
		$DescuentoMonto2= $contenido[0]['MONTO_DSCTO2'];
		...
		$a['Encabezado']['IdDoc']['Folio']			= $nro_nota_credito;
		$a['Encabezado']['Emisor']['RUTEmisor']		= $rutemisor;
		$a['Encabezado']['Emisor']['RznSoc']		= utf8_encode($rznsoc);
		$a['Encabezado']['Emisor']['GiroEmis']		= utf8_encode($giroemis);
		$a['Encabezado']['Emisor']['Acteco']		= self::K_ACTV_ECON;
		$a['Encabezado']['Emisor']['DirOrigen']		= utf8_encode($dirorigen);
		$a['Encabezado']['Emisor']['CmnaOrigen']	= utf8_encode($cmnaorigen);
		...
		$b['Referencia']['TpoDocRef']= '33';//solo prueba
		$b['Referencia']['FolioRef']= $FolioRef;//
		$b['Referencia']['CodRef']= $CodRef;//
		$b['Referencia']['RazonRef']= utf8_encode(substr($RazonRef,0,90));
