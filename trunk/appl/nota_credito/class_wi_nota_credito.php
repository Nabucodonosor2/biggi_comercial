...
class wi_nota_credito_base extends w_cot_nv {
    ...
    const K_AUTORIZA_ENVIAR_DTE = '993545';
    const K_AUTORIZA_IMPRIMIR_DTE = '993560';
    ...
    function load_record() {
        ...
        $cod_tipo_nc = $this->dws['dw_nota_credito']->get_item(0, 'COD_TIPO_NOTA_CREDITO');
        ...
        if($COD_TIPO_NC_INTERNO_SII == 1 || $COD_TIPO_NC_INTERNO_SII == 2) //Anulacion Factura Total
            $this->dws['dw_item_nota_credito']->controls['CANTIDAD']->readonly = true;
                ...
                $ORIGEN_NC	= $this->dws['dw_nota_credito']->get_item(0, 'ORIGEN_NC');
                
                if($ORIGEN_NC == 'CREAR'){
                    unset($this->dws['dw_nota_credito']->controls['PORC_IVA']);
                    $this->dws['dw_nota_credito']->add_control($control = new drop_down_iva());
                    $control->set_onChange("computed(get_num_rec_field(this.id), 'MONTO_IVA');");
                }else if($ORIGEN_NC == 'CREAR_DESDE'){
                    unset($this->dws['dw_nota_credito']->controls['PORC_IVA']);
                ...
                if($this->dws['dw_nota_credito']->get_item(0, 'COD_TIPO_NOTA_CREDITO') == 2){//Correccion de texto
                    $this->dws['dw_nota_credito']->set_entrable('COD_SUCURSAL_FACTURA', true);
                    $this->dws['dw_nota_credito']->set_entrable('COD_PERSONA', true);
                }else{
                    $this->dws['dw_nota_credito']->set_entrable('COD_SUCURSAL_FACTURA', false);
    ...
    function Envia_DTE($name_archivo, $fname){
        ...
        if ($conn_id===false)
            ...
            // iniciar una sesion con nombre de usuario y contrasena
    ...
    function envia_NC_Electronica(){
        if (!$this->lock_record())
            ...
            if ($db->EXECUTE_SP($sp, $param)) {
                ...
                $DP					= $result_dte[0]['DIRECCION'];			//15 Direccion Postal
                ...
                $CR					= $result_dte[0]['COD_TIPO_NOTA_CREDITO'];						//41 Codigo de Referencia
                $RER				= $result_dte[0]['REFERENCIA'];									//42 Razon explicita de la referencia
                ...
                $DP				= substr($DP.$space, 0, 60);				//15 Direccion Postal
                ...
                fwrite($handle, $DP.$this->separador);					//15 Direccion Postal
                ...
                for ($i = 0; $i < 30; $i++){
                    if($i < $count){
                        ...
                        fwrite($handle, $ORDEN.$this->separador);		//31 Numero de Linea
                        fwrite($handle, $MODELO.$this->separador);		//32 Codigo item
                        ...
                        fwrite($handle, $DESCRIPCION.$this->separador);	//37 personalizados Zona Detalles(Modelo item)
                ...
                for($i = 0; $i < 1; $i++){
                    ...
                    fwrite($handle, $CR.$this->separador);			//41 Codigo de Referencia
                    fwrite($handle, $RER.$this->separador);			//42 Razon explicita de la referencia
                    fwrite($handle, "\r\n");
                }
                /*fclose($handle);
                ...
                if (!$upload) {
                    ...
                    $this->_load_record();
                    $this->alert('Gestión Realizada con éxito. Nota Credito Electronica N° '.$NRO_NOTA_CREDITO.'.');
                }
                unlink($fname);
            }else{
                $db->ROLLBACK_TRANSACTION();
    ...
    function procesa_event() {
        if(isset($_POST['b_save_x'])) {
            ...
            if ($this->_save_record()) {
                if ($_POST['wi_hidden']=='save_enviar_dte')		// Es es el save enviar_dte
    ...
    function enviar_dte($reenviar = false){
        if (!$this->lock_record())
            ...
            $sql = "SELECT NRO_NOTA_CREDITO
					  ,TOTAL_CON_IVA TOTAL_CON_IVA_NC
					  ,(SELECT TOTAL_CON_IVA FROM FACTURA WHERE COD_FACTURA = NC.COD_DOC) TOTAL_CON_IVA_FA
            ...
            if($result[0]['NRO_NOTA_CREDITO'] <> '' && $reenviar == false)
                ...
                if($result[0]['TOTAL_CON_IVA_NC'] > $result[0]['TOTAL_CON_IVA_FA']){
                    $this->_load_record();
                    print " <script>alert('Error al Emitir Dte, el total de la Nota de Crédito supera al monto de la Factura.');</script>";
                    return false;
                }
                ...
                if($reenviar){
                    ...
                    $sql_folio = "SELECT dbo.f_get_nro_dte('NC') NRO_NOTA_CREDITO";
                    
                    $result_folio = $db->build_results($sql_folio);
                    $nro_nota_credito = $result_folio[0]['NRO_NOTA_CREDITO'];
                }
                ...
                if($COD_FACTURA == ""){
                    $this->_load_record();
                    print " <script>alert('Error al Emitir Dte, esta Nota de Crédito tiene que estar asociado a una Factura');</script>";
                    return;
                }
				...
				WHERE F.COD_FACTURA = $COD_FACTURA";
						...
						//MH 01062022 SE MODIFICA PARA QUE VALIDE SOLO DESDE EL COD_FACTURA 59202
						//MH 01062022 DESDE EL 31052022 AL CERRAR BIGGI SE DEJA DE USAR LIBRE DTE INTEGRASYSTEM Y SE PASA A LIBRE DTE BY MH
                        if($revision_estado <> 'EPR' && $revision_estado <> 'RPR' && $revision_estado <> 'RLV' && $COD_FACTURA > 59202){//ultima factura signature cod_factura 36308
                            $this->_load_record();
                        ...
                        if($result_folio[0]['RANGO'] == 'ERROR'){
                            $this->_load_record();
                            print " <script>alert('Error al Emitir Dte, el Nro de folio no figura dentro de los rangos permitidos.');</script>";
                            return false;
                        }
				...
				AND C.COD_COMUNA = N.COD_COMUNA";
                        ...
                        if($GiroRecep == ''){
                            $this->_load_record();
                            print " <script>alert('Error al Emitir Dte, la empresa de la Nota Credito no tiene Giro.');</script>";
                            return;
                        }
                        ...
                        $sql = "select DATEDIFF (month, FECHA_FACTURA, getdate()) +
                                        CASE
                                            WHEN DATEPART(day, FECHA_FACTURA) < DATEPART(day, getdate()) THEN 1 
                                            ELSE 0
                                        END DIF_FECHA
                                from FACTURA
                        ...
                        if($CodRef == 1 && $DIF_FECHA > 6){
                            $a['Encabezado']['IdDoc']['IndNoRebaja']	= 1;
                        }
                        ...
                        if($DescuentoMonto1 <> 0){
                            ...
                            $b['DscRcgGlobal'][0]['TpoValor']= '$';//Indica si es Porcentaje o Monto
                        ...
                        if($DescuentoMonto2 <> 0){
                            ...
                            $b['DscRcgGlobal'][1]['TpoValor']= '$';//Indica si es Porcentaje o Monto
						...
						$Sql= "select COD_LOG_CAMBIO
				                from LOG_CAMBIO 
				                where KEY_TABLA='$cod_nota_credito'
				                AND TIPO_CAMBIO='E'
				                and NOM_TABLA='NOTA_CREDITO'";
                        ...
                        //Guarda el response de la funcion emitir_dte.
    ...
    function imprimir_dte($desde_output=false){
        ...
        if($cod_nota_credito > 2962)
            print " <script>window.open('../common_appl/print_dte.php?cod_documento=$cod_nota_credito&DTE_ORIGEN=61')</script>";
                ...
                if(file_exists("../../../../PDF/PDFCOMERCIALBIGGI/$year/61_$nro_nota_credito.pdf"))
                    print " <script>window.open('../../../../PDF/PDFCOMERCIALBIGGI/$year/61_$nro_nota_credito.pdf')</script>";
                    else
