...
class wo_nota_credito_base extends w_output_biggi {
	...
	function wo_nota_credito_base(){
		...
		// MH DESDE 20032022 SE RESTRINGE VER LAS NC DE OTROS VENDEDORES PARA ARODRIGUEZ, CURTUBIA, EOLMEDO
		// Llama a w_base para que $this->cod_usuario contenga al usuario actual MH 20032022
		parent::w_base('nota_credito', $_REQUEST['cod_item_menu']);		
		...
		$sql = "select NC.COD_NOTA_CREDITO
				...
				where	NC.COD_ESTADO_DOC_SII = EDS.COD_ESTADO_DOC_SII
						and dbo.f_get_tiene_acceso (".$this->cod_usuario.", 'NOTA_CREDITO',F.COD_DOC, NULL) = 1 
		...
		$priv = $this->get_privilegio_opcion_usuario('993560', $this->cod_usuario); //print
		if($priv=='E')
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		if ($boton=='create' && $habilita){
			...
			$temp->setVar("DISPLAY_CREAR_DESDE", '<input name="b_'.$boton.'" id="b_'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
							...
							'onClick="request_nota_credito(\'Ingrese Nº de Documento\',\'\');" />');
	...
	function redraw(&$temp) {
  	...
  	function crear_nc_from($valor_devuelto) 
  	{
		...
		if ($opcion=='desde_fa' || $opcion=='desde_fa_adm')
		{
				...
				$sql = "select COD_FACTURA, NRO_FACTURA, COD_ESTADO_DOC_SII from FACTURA where NRO_FACTURA = $nro_factura AND COD_ESTADO_DOC_SII=".self::K_ESTADO_SII_ENVIADA;
	...
	function redraw_item(&$temp, $ind, $record){
		...
		if($COD_ESTADO_DOC_SII == 3 && $this->autoriza_xml == true && $COD_NOTA_CREDITO > 2962){
