...
$sql_fecha= "select TOP 1 DATEDIFF(DAY,FECHA_WP_TRANSACCION, GetDate()) FECHA_TRANSACCION
			from WP_TRANSACCION 
...
if(($fecha_transaccion >=1)||($link_visible == 'N')){
	//$vl_error = $vl_error + 16;
	$vl_error = $vl_error.'|16|';
	$error = 'S';
}
...
// FIN BORRAR DESPUES DE PRUEBAS
if(($error == 'S')&&($fecha_error == '')){
	$sql = "UPDATE WP_TRANSACCION
			SET COD_ERROR_WEBPAY =$vl_error
			WHERE COD_WP_TRANSACCION =$cod_wp_transaccion";
	$db->query($sql);
}


if(($link_visible != 'S') or ($fecha_transaccion >=1) or ($exito=='S') or ($error == 'S')){
