		...
		for ($i = 0; $i < $this->row_count(); $i++){
			...
			if ($statuts == K_ROW_NEW_MODIFIED) {
				...
				if($t > (($this->get_parametro(67) * 1024) * 1024)){
					$this->alert('El archivo que intenta subir excede el peso m�ximo en disco ('.$this->get_parametro(67).' MB), pero su archivo pesa = '.number_format($size_x_bytes, 2, ',', '.').' MB');
			 		return true;
				}
...
class dw_item_nota_venta extends dw_item {
	function dw_item_nota_venta () {		
		...
		//todos los campos que se agreguen en el select se deben agregar en funci�n "creada_desde"
	...
	function update($db, &$dw_preorden)	{
		...
		for ($i = 0; $i < $this->row_count(); $i++){
			...
			//se agrega $COD_ITEM_NOTA_VENTA == 'null', cuando la NV es creada desde cotizaci�n, ya que $statuts == K_ROW_NEW_MODIFIED
				$operacion = 'INSERT';
			elseif ($statuts == K_ROW_MODIFIED)
				$operacion = 'UPDATE';
			...
			if (!$db->EXECUTE_SP($sp, $param)) 
				...
				if ($statuts == K_ROW_NEW_MODIFIED) {
					...
					//reemplaza utilizaci�n de redirect 
				...
				//si el equipo es TE y tiene autorizaci�n, busca si es nueva para crear registro AUTORIZA_TE
				if ($COD_PRODUCTO == 'TE' && $MOTIVO_AUTORIZA != ''){
		        	...
		        	if ($count_autoriza_te == 0){// no tiene autorizaci�n anterior de TE
...
class dw_lista_factura extends datawindow {
	...
	function retrieve($COD_NOTA_VENTA){
		//MH 09/08/2016 = CASO FERROVIAL solicitado por administraci�n BIGGI
...
//			$this->set_entrable('BTN_CONSULTA_STK', false);
		...
		if ($boton=='envia_mail_pago') {
			if ($habilita)
				$temp->setVar("WI_ENVIA_MAIL_PAGO", '<input name="b_create" id="b_create" src="../../../../commonlib/trunk/images/b_create.jpg" type="image" '.
											...
											'onClick="return request_factura(\'Ingrese N� de la Nota de Venta\',\'\');"'.
											'/>');
	...
	function load_record() {
		...
		if ($COD_ESTADO_NOTA_VENTA == self::K_ESTADO_EMITIDA){	
			...
			/** SEMAFORO **/
			$exito = true;
			/** Si la NV ya fue autorizada por SP, RE, o JT, entonces NO requiere autorizaci�n. **/
            ...
            if($autoriza_procesar_venta <> 'S'){
            	/** Si el RUT de la NV pertenece a un Centro de Costo (Sodexo, Compass, CDR), entonces NO requiere autorizaci�n.. **/
	...
	function save_record($db) {
		...
		/*MH 14:59 25/06/2018 = Cuando estos rut est�n con el vendedor1: CARLOS URTUBIA, el % siempre el procentaje del vendedor sera del 15%*/
		if($COD_USUARIO_VENDEDOR1 == 6){
			...
			if($RUT == '76178360' || $RUT == '88279900' || $RUT == '96945020' || $RUT == '76178390' || $RUT == '76117696')
				$PORC_VENDEDOR1 = 15;
		/*MH 11:49 24/07/2018 = Cuando estos rut est�n con el vendedor1: RODRIGO BARRAZA, el procentaje del vendedor sera del 10%*/
		}
	...
	function creada_desde($ve_result){
		...
		if(session::is_set('CREADA_DESDE_COTIZACION_COD_RECHAZADA')){
			$this->alert('La Cotizaci�n N� '.$cod_cotizacion.' se encuentra en estado rechazada.\nAl crear una Nota de Venta desde la Cotizaci�n N� '.$cod_cotizacion.', esta quedar� en estado RE-ABIERTA.\nFavor considere esta situaci�n.');
		...
		// crea �tems, excepto los T
		//todos los campos que se agreguen en el select se deben agregar en funci�n "dw_item_nota_venta"
	...
	function procesa_event() {
		...
		elseif(isset($_POST['b_envia_mail_pago_x'])){
			...
			if($this->genera_oc()){
				...
				$this->alert('Se ha creado la OC N� '.$cod_orden_compra.', ahora puede revisar esta OC y modificarla seg�n sea la necesidad.');
			}else
	...
	function actualiza_webpay($cod_nota_venta,$monto_webpay){
 	...
 	function f_envia_mail($estado_nota_venta){
		...
		*/
		...
		{
	        $asunto = 'Confimaci�n de Nota de Venta N� '.$COD_NOTA_VENTA;
		...
		{
			$asunto = 'Creaci�n Nueva Nota de Venta N� '.$COD_NOTA_VENTA;
	...
	function envia_mail_pago($link_pago,$cod_nota_venta,$email,$nombre_correo){
		...
		$asunto 			= "'Autorizaci�n  Pago Web Pay N� Nota Venta: $cod_nota_venta'";
