		...
		for ($i = 0; $i < $this->row_count(); $i++){
			...
			if ($statuts == K_ROW_NEW_MODIFIED) {
				...
				if($t > (($this->get_parametro(67) * 1024) * 1024)){
					$this->alert('La capacidad máxima para subir es de '.$this->get_parametro(67).' MB');
			 		return true;
				}
				...
				$archivo = new archivo($nom_archivo, $ruta_archivo, $e,$t,$tmp);
			 	...
			 	if(!file_exists($ruta_archivo.$nom_archivo)){
			 		$this->alert('Error en la subida de archivo, favor contacte a IntegraSystem.');
			 		return true;
			 	}
...
class wi_nota_venta extends w_cot_nv {
	...
	order by INV.ORDEN asc";
		...
		$this->dws['dw_nota_venta']->set_computed('RESULTADO', '[TOTAL_NETO] - [SUM_OC_TOTAL] - [MONTO_GASTO_FIJO] - [MONTO_DSCTO_CORPORATIVO] - [SUMA_NC]');
		$this->dws['dw_nota_venta']->set_computed('STATIC_RESULTADO', '[TOTAL_NETO] - [SUM_OC_TOTAL] - [MONTO_GASTO_FIJO] - [MONTO_DSCTO_CORPORATIVO] - [SUMA_NC]');	
...
//			$this->set_entrable('BTN_CONSULTA_STK', false);
	...
	function habilita_boton(&$temp, $boton, $habilita) {
		if ($boton=='b_envia_mail_pago'){
			if ($habilita){
				$ruta_over = "'../../images_appl/boton_enviar_over.png'";
				$ruta_out = "'../../images_appl/boton_enviar.png'";
				$ruta_click = "'../../images_appl/boton_enviar_click.png'";
				$temp->setVar("WI_ENVIA_MAIL_PAGO", '<input name="'.$boton.'" id="'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
								'style="cursor:pointer;height:30px;width:200px;border: 0;background-image:url(../../images_appl/boton_enviar.png);background-repeat:no-repeat;background-position:center;border-radius: 15px;"'.
	...
	function habilitar(&$temp, $habilita){
		...
		if ($cod_estado_nota_venta == self::K_ESTADO_ANULADA || $this->is_new_record())
		...
		if ($COD_ESTADO_NOTA_VENTA == self::K_ESTADO_EMITIDA){	
			...
			for($j=0 ; $j < count($result) ; $j++){ 
				...
				if($cod_orden_compra <> ''){
					...
					$result_par_ws = $db->build_results($sql);
					
					$user_ws		= $result_par_ws[0]['USER_WS'];
					$passwrod_ws	= $result_par_ws[0]['PASSWROD_WS'];
					$url_ws			= $result_par_ws[0]['URL_WS'];
		   		 
		   			$biggi = new client_biggi("$user_ws", "$passwrod_ws", "$url_ws");
					$result_ws = $biggi->cli_orden_despacho_to_nv($cod_orden_compra);
					
					for($i=0 ; $i < count($result_ws['ORDEN_DESPACHO']) ; $i++){
						$COD_ORDEN_DESPACHO			= $result_ws['ORDEN_DESPACHO'][$i]['COD_ORDEN_DESPACHO'];
						$COD_ITEM_ORDEN_DESPACHO	= $result_ws['ORDEN_DESPACHO'][$i]['COD_ITEM_ORDEN_DESPACHO'];
						$COD_PRODUCTO				= $result_ws['ORDEN_DESPACHO'][$i]['COD_PRODUCTO'];
						$NOM_PRODUCTO				= $result_ws['ORDEN_DESPACHO'][$i]['NOM_PRODUCTO'];
						$CANTIDAD					= $result_ws['ORDEN_DESPACHO'][$i]['CANTIDAD'];
						$NRO_FACTURA				= $result_ws['ORDEN_DESPACHO'][$i]['NRO_FACTURA'];
						$FECHA_FACTURA				= $result_ws['ORDEN_DESPACHO'][$i]['FECHA_FACTURA'];
						$FECHA_ORDEN_DESPACHO		= $result_ws['ORDEN_DESPACHO'][$i]['FECHA_ORDEN_DESPACHO'];
						$FECHA_ORDEN_DESPACHO_HORA	= $result_ws['ORDEN_DESPACHO'][$i]['FECHA_ORDEN_DESPACHO_HORA'];
						$NOM_USUARIO				= $result_ws['ORDEN_DESPACHO'][$i]['NOM_USUARIO'];
...
function envia_mail_pago($link_pago,$cod_nota_venta,$email){
		...
		$result_param = $db2->build_results($sql_param);
	...
	//	$mail->AddBCC("jcatalan@biggi.cl");
	    ...
	    //descomentar para ofiacial
		//$mail->AddBCC("$email_vendedor");
		
		$mail->From			= "$email_vendedor";		

		$mail->FromName		= "$nombre_vendedor";
		$mail->Timeout		= 30;
