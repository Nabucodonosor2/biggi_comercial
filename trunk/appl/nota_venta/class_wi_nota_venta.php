...
class static_link_rojo extends static_link {
    var	$href;
    ...
    function draw_no_entrable($dato, $record) {
        ...
        $ctrl = '<a id="'.$field.'" style="color: red;" href="'.$href.'" '.$this->make_java_script().'>'.$valor1.'</a>';
        return $ctrl;
    }
...
class dw_item_nota_venta extends dw_item {
	function dw_item_nota_venta () {		
		...
		//todos los campos que se agreguen en el select se deben agregar en funcion "creada_desde"
	...
	function update($db, &$dw_preorden)	{
		...
		for ($i = 0; $i < $this->row_count(); $i++){
			...
			//se agrega $COD_ITEM_NOTA_VENTA == 'null', cuando la NV es creada desde cotizacion, ya que $statuts == K_ROW_NEW_MODIFIED
				$operacion = 'INSERT';
			elseif ($statuts == K_ROW_MODIFIED)
				$operacion = 'UPDATE';
			...
			if (!$db->EXECUTE_SP($sp, $param)) 
				...
				if ($statuts == K_ROW_NEW_MODIFIED) {
					...
					//reemplaza utilizacion de redirect 
				...
				//si el equipo es TE y tiene autorizacion, busca si es nueva para crear registro AUTORIZA_TE
				if ($COD_PRODUCTO == 'TE' && $MOTIVO_AUTORIZA != ''){
		        	...
		        	if ($count_autoriza_te == 0){// no tiene autorizacion anterior de TE
...
class dw_lista_guia_despacho extends datawindow {
	...
	const K_TIPOGD_GARANTIA = 3;
	const K_ESTADO_SII_IMPRESA = 2;
	const K_ESTADO_SII_ENVIADA = 3;
	const K_ITEM_MENU_GUIA_DESPACHO = '1525';
	/*
	 * 	MH 05122019 SOLICITA QUE SE CONSIDEREN LAS GD COD_TIPO_GUIA_DESPACHO = (1, 4, 3).
	   NO ESTABA CONSIDERANDO LAS GD TIPO 3 ANTES DE ESTE CAMBIO. NO SE RECUERDA EL MOTIVO, PERO SE CORRIGE SEGUN ADM BIGGI.
	   (RE+MH CASO NV 91181)
	 * */
	function dw_lista_guia_despacho() {
		$sql = "select convert(varchar, NRO_GUIA_DESPACHO)+'|'+convert(varchar, COD_GUIA_DESPACHO) NRO_GUIA_DESPACHO
				from   GUIA_DESPACHO
				where  COD_DOC = {KEY1}
	  			and  COD_TIPO_GUIA_DESPACHO in (".self::K_TIPOGD_VENTA.", ".self::K_TIPOGD_TRASLADO.", ".self::K_TIPOGD_GARANTIA.")
...
class dw_lista_guia_despacho_demo extends datawindow {
    const K_TIPOGD_DEMO = 2;
    const K_TIPOGD_TRASLADO = 4;
    const K_TIPOGD_GARANTIA = 3;
    const K_ESTADO_SII_IMPRESA = 2;
    const K_ESTADO_SII_ENVIADA = 3;
    ...
    function dw_lista_guia_despacho_demo() {
        $sql = "select convert(varchar, NRO_GUIA_DESPACHO)+'|'+convert(varchar, COD_GUIA_DESPACHO) DEMO_NRO_GUIA_DESPACHO
				from   GUIA_DESPACHO
				where  COD_DOC = {KEY1}
	  			and  COD_TIPO_GUIA_DESPACHO in (".self::K_TIPOGD_DEMO.")
	  			and  COD_ESTADO_DOC_SII in (".self::K_ESTADO_SII_IMPRESA.", ".self::K_ESTADO_SII_ENVIADA.")";
        parent::datawindow($sql, 'GD_RELACIONADA_DEMO');
        
        $this->add_control(new static_link_rojo('DEMO_NRO_GUIA_DESPACHO', '../../../../commonlib/trunk/php/link_wi.php?modulo_origen=nota_venta&modulo_destino=guia_despacho&cod_modulo_destino=[DEMO_NRO_GUIA_DESPACHO]&cod_item_menu=1525'));
    }
    function fill_record(&$temp, $record) {
        parent::fill_record($temp, $record);
        if ($record < $this->row_count() - 1)
            $temp->setVar($this->label_record.'.GD_SEPARADOR_DEMO', '-');
    }
}

class dw_lista_guia_recepcion extends datawindow {
	const K_TIPO_GR	= 3;
	const K_TIPO_GR_PRESUPUESTO	= 5;
	...
	function dw_lista_guia_recepcion() {
		$sql = "select convert(varchar, COD_GUIA_RECEPCION) NRO_GUIA_RECEPCION
				from   GUIA_RECEPCION
 				where  COD_TIPO_GUIA_RECEPCION NOT IN(".self::K_TIPO_GR.")
...
class dw_lista_factura extends datawindow {
	...
	function retrieve($COD_NOTA_VENTA){
		//MH 09/08/2016 = CASO FERROVIAL solicitado por administraciï¿½n BIGGI
...
class wi_nota_venta extends w_cot_nv {
    ...
    $this->dws['dw_nota_venta']->add_control(new static_link('COD_COTIZACION', '../../../../commonlib/trunk/php/link_wi.php?modulo_origen=nota_venta&modulo_destino=cotizacion&cod_modulo_destino=[COD_COTIZACION_AUX]&cod_item_menu=1505'));
		...
		$this->dws['dw_lista_guia_despacho_demo'] = new dw_lista_guia_despacho_demo();
...
//			$this->set_entrable('BTN_CONSULTA_STK', false);
		...
		if ($boton=='envia_mail_pago') {
			if ($habilita)
				$temp->setVar("WI_ENVIA_MAIL_PAGO", '<input name="b_create" id="b_create" src="../../../../commonlib/trunk/images/b_create.jpg" type="image" '.
											...
											'onClick="return request_factura(\'Ingrese Nï¿½ de la Nota de Venta\',\'\');"'.
											'/>');
	...
	function load_record() {
		...
		$this->dws['dw_lista_guia_despacho_demo']->retrieve($COD_NOTA_VENTA);
		...
		if ($COD_ESTADO_NOTA_VENTA == self::K_ESTADO_EMITIDA){	
			...
			/** Si la NV ya fue autorizada por SP, RE, o JT, entonces NO requiere autorizaciï¿½n. **/
            ...
            if($autoriza_procesar_venta <> 'S'){
            	...
            	/** Si el RUT de la NV pertenece a un Centro de Costo (Sodexo, Compass, CDR), entonces NO requiere AUTORIZACION.. **/
            	...
            	if($result_val2[0]['COUNT'] == 0){/** Si la forma de pago de la NV es: CCF - 60 DIAS
					...
					// SE COMENTA EL SIGUIENTE IF POR QUE EL 21012020 SE IMPLEMENTO CAMPO CRITERIO_DESPACHO_FACTURA POR LO TANTO ESTA LINEA DEBIO QUEDAR COMENTADA
					// MH 28012021		
	   ...
	   $sql = "SELECT LL.COD_LLAMADO LL_COD_LLAMADO
		...
		if ($COD_NOTA_VENTA==88860 ||
			...
			$this->dws['dw_nota_venta']->calc_computed();
		}		
		/**
		 * SI ESTA CONFIRMADA Y EL Vendedor 1 ES = A Emisor PERMITE EDITAR REFERENCIA, NRO CC CLIENTE Y TODA PESTAÑA DOCUMENTOS
		 * **/
		$COD_USUARIO_VENDEDOR = $this->dws['dw_nota_venta']->get_item(0, 'COD_USUARIO_VENDEDOR1');
		$COD_USUARIO = $this->dws['dw_nota_venta']->get_item(0, 'COD_USUARIO');
		if($COD_ESTADO_NOTA_VENTA == self::K_ESTADO_CONFIRMADA && $COD_USUARIO_VENDEDOR == $COD_USUARIO){
		    $this->dws['dw_nota_venta']->set_entrable('REFERENCIA'       		, true); 
		    $this->dws['dw_nota_venta']->set_entrable('CENTRO_COSTO_CLIENTE'       		, true);
		    $this->dws['dw_nota_venta']->set_entrable('NRO_ORDEN_COMPRA'       		, true);
		    $this->dws['dw_nota_venta']->set_entrable('FECHA_ORDEN_COMPRA_CLIENTE'       		, true);
		...
		if($COD_ESTADO_NOTA_VENTA == self::K_ESTADO_CONFIRMADA || $COD_ESTADO_NOTA_VENTA == self::K_ESTADO_CERRADA){
    		if($this->cod_usuario == 4 || $this->cod_usuario == 8 || $this->cod_usuario == 1 || $this->cod_usuario == 71)
    		    $this->dws['dw_nota_venta']->set_entrable('AUTORIZA_PROCESAR_VENTA', true);
    	}else
		    $this->dws['dw_nota_venta']->set_entrable('AUTORIZA_PROCESAR_VENTA', false);
	...
	function save_record($db) {
		...
		$CORREO_FA = $this->dws['dw_nota_venta']->get_item(0, 'CORREO_FA');
		...
		/*MH 14:59 25/06/2018 = Cuando estos rut estï¿½n con el vendedor1: CARLOS URTUBIA, el % siempre el procentaje del vendedor sera del 15%*/
		if($COD_USUARIO_VENDEDOR1 == 6){
			...
			if($RUT == '76178360' || $RUT == '88279900' || $RUT == '96945020' || $RUT == '76178390' || $RUT == '76117696')
				$PORC_VENDEDOR1 = 15;
		/*MH 11:49 24/07/2018 = Cuando estos rut estï¿½n con el vendedor1: RODRIGO BARRAZA, el procentaje del vendedor sera del 10%*/
		}
		...
		$CORREO_FA = ($CORREO_FA =='') ? "null" : "'$CORREO_FA'";
		
		$NOTAS_INTERNAS = $this->dws['dw_nota_venta']->get_item(0, 'NOTAS_INTERNAS');
		$NOTAS_INTERNAS = str_replace("'", "''", $NOTAS_INTERNAS);
		$NOTAS_INTERNAS = ($NOTAS_INTERNAS =='') ? "null" : "'$NOTAS_INTERNAS'";
	...
	function creada_desde($ve_result){
		...
		if(session::is_set('CREADA_DESDE_COTIZACION_COD_RECHAZADA')){
			$this->alert('La Cotización N° '.$cod_cotizacion.' se encuentra en estado rechazada.\nAl crear una Nota de Venta desde la Cotización N° '.$cod_cotizacion.', esta quedará en estado RE-ABIERTA.\nFavor considere esta situación.');
		...
		// crea ï¿½tems, excepto los T
		//todos los campos que se agreguen en el select se deben agregar en funciï¿½n "dw_item_nota_venta"
	...
	//SE AGREGA BOTON PARA IMPRIMIR RESUMEN DE ITEMS DE NOTA VENTA MH 07072021
    function print_resumen_item($con_logo, $print_especial){
        ...
        $sql = "SELECT COD_PRODUCTO
					   ...
					   ,PRECIO * SUM(CANTIDAD) TOTAL,
						...
						E.RUT,
						E.DIG_VERIF,
						dbo.f_format_date(C.FECHA_NOTA_VENTA, 3) FECHA_NOTA_VENTA,
				...
				FROM ITEM_NOTA_VENTA IC, NOTA_VENTA C, EMPRESA E, SUCURSAL S, PERSONA P
					,FORMA_PAGO FP, MONEDA M, USUARIO U
				WHERE C.COD_NOTA_VENTA = $cod_nota_venta
				AND C.COD_NOTA_VENTA = IC.COD_NOTA_VENTA
				...
				RUT, DIG_VERIF, C.COD_NOTA_VENTA, FECHA_NOTA_VENTA,
        ...
        $labels = array();
        $labels['strCOD_NOTA_VENTA_AUX1'] = $COD_NOTA_VENTA_AUX1;
        
        ///if($print_especial == 'S')
            $xml = $this->root_dir.'appl/nota_venta/nv_resumen_item_esp.xml';
            //else
                //$xml = $this->root_dir.'appl/cotizacion/cot_resumen_item.xml';
                
                $rpt= new reporte($sql, $xml, $labels, "RESUMEN ITEMS NOTA VENTA ".$COD_NOTA_VENTA_AUX1.".pdf", $con_logo);
    }
	...
	function printnv_resumen_pdf($con_logo, $con_precio) {
		...
		$sql= "SELECT NV.COD_NOTA_VENTA,
				...
				dbo.f_format_date (GETDATE(),2) F_PRINT_NV
	...
	function procesa_event() {
		...
		elseif(isset($_POST['b_envia_mail_pago_x'])){
			...
			if($this->genera_oc()){
				...
				$this->alert('Se ha creado la OC N° '.$cod_orden_compra.', ahora puede revisar esta OC y modificarla según sea la necesidad.');
			}else
		...
		else if(isset($_POST['b_print_resumen_it'])){
	...
	function actualiza_webpay($cod_nota_venta,$monto_webpay){
 	...
 	function f_envia_mail($estado_nota_venta){
		...
		*/
		...
		{
	        $asunto = 'Confimación de Nota de Venta N° '.$COD_NOTA_VENTA;
	        $mensaje = 'Se ha <b>CONFIRMADO</b> la <b>NOTA DE VENTA N° '.$COD_NOTA_VENTA.'</b> por el usuario <b><i>'.$remitente.'<i><b>';
		}        
 		if($estado_nota_venta == 'EMITIDO')
		{
			$asunto = 'Creación Nueva Nota de Venta N° '.$COD_NOTA_VENTA;
	        $mensaje = 'Se ha <b>CREADO</b> una nueva <b>NOTA DE VENTA N° '.$COD_NOTA_VENTA.'</b> por el usuario <b><i>'.$remitente.'<i><b>';
	  	}
 		if($estado_nota_venta == 'ANULADA')
		{
	        $asunto = 'Anulacion de Nota de Venta N° '.$COD_NOTA_VENTA;
	        $mensaje = 'Se ha <b>ANULADO</b> la <b>NOTA DE VENTA N° '.$COD_NOTA_VENTA.'</b> por el usuario <b><i>'.$remitente.'<i><b>';
		}
	...
	function envia_mail_pago($link_pago,$cod_nota_venta,$email,$nombre_correo){
		...
		//CREACION DE EMAIL
		$autorizacion ="AUTORIZACION PARA PAGO VIA WEB PAY NOTA DE VENTA N°";
