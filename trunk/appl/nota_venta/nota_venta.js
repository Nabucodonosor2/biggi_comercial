...
function confirm_suma_nv(){
	...
	if(vl_es_no_suma_nv_informe == 'S')
		str_confirm = "¿Está seguro que deséa que esta NV se considere como ventas en los informes?";
	else
		str_confirm = "¿Está seguro que deséa que esta NV NO se considere como ventas en los informes?";
...
function validate() {
	...
	//MH dice que si la NV está confirmada o cerrada ya es tarde para validar el centro costo cliente.
	...
	/*MH 26/04/2018 10:38: Se solicitó comentar esta validación, ya que esta pestaña se ocultó y no tiene sentido
	 validar porque el usuario no tendrá como llenar el dato dentro de la pestaña que se llenaba ahí
...
function valida_monto_doc(ve_monto_doc) {
	var cant_docs = document.getElementById('COD_FORMA_PAGO_0').options[document.getElementById('COD_FORMA_PAGO_0').selectedIndex].dataset.dropdown; //en el label está la cantidad de documentos.
	var total_con_iva = document.getElementById('TOTAL_CON_IVA_H_0').value;
