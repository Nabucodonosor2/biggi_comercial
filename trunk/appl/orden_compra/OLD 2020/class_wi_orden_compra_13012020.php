...
class dw_orden_compra extends dw_help_empresa {
	...
	const K_PARAM_PORC_DSCTOS_MAX =74;
	...
	function dw_orden_compra() {	
		...
		$this->add_control(new edit_text('PORC_DSCTOS_MAX',10,10, 'hidden'));
...
class drop_down_iva_oc_negativo_no_zero extends drop_down_iva  {
	...
	function drop_down_iva_oc_negativo_no_zero() {
		// No se llama al ancestro directo drop_down_iva
		$porc_iva = $this->get_parametro(self::K_PARAM_IVA);
		$porc_iva = number_format($porc_iva, 1, ',', '.');
		...
		$porc_bh = $this->get_parametro(self::K_PARAM_BH);
		$porc_bh = number_format( - $porc_bh, 1, ',', '.');
		parent::drop_down_list('PORC_IVA',array($porc_iva,$porc_bh),array($porc_iva,$porc_bh),52);
	}
}
...
class drop_down_iva_oc_no_zero extends drop_down_iva  {
	const K_PARAM_IVA = 1;
	const K_PARAM_BH = 2;
	
	function drop_down_iva_oc_no_zero() {
		// No se llama al ancestro directo drop_down_iva
		$porc_iva = $this->get_parametro(self::K_PARAM_IVA);
		$porc_iva = number_format($porc_iva, 1, ',', '.');

		parent::drop_down_list('PORC_IVA',array($porc_iva),array($porc_iva),52);
	}
}
class wi_orden_compra_base extends w_cot_nv {
	...
	const K_AUTORIZA_SIN_IVA       	 = '991575';
	...
	function add_controls_cot_nv() {
		...
		$priv_ZERO = $this->get_privilegio_opcion_usuario(self::K_AUTORIZA_SIN_IVA, $this->cod_usuario);
		if ($priv=='E' && $priv_ZERO == 'E') {
			$this->dws[$this->dw_tabla]->add_control(new drop_down_iva_oc_negativo());
		}else if($priv=='E' && $priv_ZERO != 'E'){
		    $this->dws[$this->dw_tabla]->add_control(new drop_down_iva_oc_negativo_no_zero());
		}else if($priv !='E' && $priv_ZERO == 'E'){
			$this->dws[$this->dw_tabla]->add_control(new drop_down_iva_oc());
      	}else if($priv !='E' && $priv_ZERO != 'E'){
			$this->dws[$this->dw_tabla]->add_control(new drop_down_iva_oc_no_zero());
      	}
	...
	function new_record() {
		...
		$priv_desc = $this->get_privilegio_opcion_usuario('991570', $this->cod_usuario); /** DESCUENTOS 1 Y 2 **/
	...
	function load_record() {
		...
		if ($priv=='E') {
			...
			/**SE COMENTA HASTA NUEVA DEFINICION DE MH 18/03/2019**/
		...
		$cod_usu_original = $this->dws['dw_orden_compra']->get_item(0, 'COD_USUARIO');
		
		
		
		if($cod_usu_original =! $this->cod_usuario || $cod_empresa == 1138 || $cod_empresa == 1302)
		    $usuario_permitido = 'N';
		if($this->cod_usuario == 1 ||$this->cod_usuario == 2 ||$this->cod_usuario == 4 || $this->cod_usuario == 8 || $this->cod_usuario == 71)
		    $usuario_permitido = 'S';
	...
	function procesa_event() {		
		if(isset($_POST['b_autoriza_monto_oc'])){
			$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
			$sp = 'spu_orden_compra';
			
			$param	= "'AUTORIZA_MONTO_OC'
						,".$this->get_key()."
						,".$this->cod_usuario;
	...
	function navegacion(&$temp){
		...
		if(!$this->is_new_record()){
			...
			if($COD_ESTADO_ORDEN_COMPRA == 3){
				$priv = $this->get_privilegio_opcion_usuario('991550', $this->cod_usuario);
				if($priv=='E')
					$temp->setVar('BTN_AUTORIZA_MONTO_OC', '<input type="submit" value="Autoriza Monto OC" class="Button" id="b_autoriza_monto_oc" name="b_autoriza_monto_oc" onclick="return confirm_autoriza();">');
		      	else
					$temp->setVar('BTN_AUTORIZA_MONTO_OC', '');
