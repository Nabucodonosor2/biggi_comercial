...
class OC_PDF extends PDF {
	...
	function OC_PDF($t,$d,$rd, $con_logo, $orientation,$unit,$format,$sql,$centroCosto){
		$this->sql_print = $sql;
	...
	function draw_barcode(){
		...
		$imprime_centro_costo = false;
		
		if($this->centroCosto == 'S'){
		    $imprime_centro_costo = true;
		}
		
		if(K_CLIENTE == 'COMERCIAL'){
			$cadena = $cod_orden_compra.'-91462001X';
			if($imprime_centro_costo){
...
class print_reporte extends reporte{
    var $centroCosto;
	function print_reporte($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false,$centroCosto) {
        $this->centroCosto = $centroCosto;
	...
	function make_reporte() {
		...
		$pdf = new OC_PDF(array($p), array($this->labels), array($rdata), $this->con_logo,$this->orientation,$this->unit,$this->format,$this->sql,$this->centroCosto);
