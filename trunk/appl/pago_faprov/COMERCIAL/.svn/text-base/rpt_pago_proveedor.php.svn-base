...
class rpt_pago_proveedor extends reporte {
	...
	function modifica_pdf(&$pdf) {
		...
		/*$sql = 'SELECT COD_USUARIO FROM USUARIO WHERE NOM_USUARIO = '.$usuario_cambio;
		$rst_usuario = $db->build_results($sql);
		$COD_USUARIO = $rst_usuario[0]['COD_USUARIO'];*/
