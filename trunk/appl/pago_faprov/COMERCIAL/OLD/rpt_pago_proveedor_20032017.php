...
class rpt_pago_proveedor extends reporte {
	...
	function print_normnal(&$pdf, $plataforma) {
	    ...
	    case "S":
				...
				//llenado del cheque
				$pdf->Text($x_ini+280, $y_ini+10, ' ****'.$monto_documento.' .-');
				$pdf->Text($x_ini+251, $y_ini+35, 'Santiago,  '.$fecha_cheque);
				$pdf->SetFont('Arial','',11);
				$pdf->Text($x_ini+70, $y_ini+64, $pagese_a);
				$pdf->SetXY($x_ini+75, $y_ini+70);
				$pdf->MultiCell(300, 12,"$total_palabras_linea");
				...
				if($tipo_cruzado == 'N-S' ){
					$pdf->SetFont('Arial','',18);
					$pdf->Text($x_ini+15, $y_ini+65, $linea_nominativa);
					//linea vertical
					$pdf->SetLineWidth(1);
					$pdf->SetDrawColor(0,0,0);
					$pdf->Line($x_ini+40, $y_ini-15, $x_ini+40, $y_ini+165-30);
					...
					$pdf->SetFont('Arial','',18);
					$pdf->Text($x_ini+15, $y_ini+65, $linea_nominativa);
					$pdf->Text($x_ini+380, $y_ini+70+12, $linea_nominativa);
					//linea vertical
					$pdf->SetLineWidth(1);
					$pdf->SetDrawColor(0,0,0);
					$pdf->Line($x_ini+40, $y_ini-15, $x_ini+40, $y_ini+165-30);
