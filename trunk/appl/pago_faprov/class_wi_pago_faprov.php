...
class dw_pago_faprov extends dw_help_empresa{
	...
	function dw_pago_faprov(){
		$sql = "SELECT PF.COD_PAGO_FAPROV
						...
						,0 TOTAL_MONTO_NC_H
						,case PF.COD_ESTADO_PAGO_FAPROV
							when ".self::K_ESTADO_PAGO_FAPROV_ANULADA." then 'ANULADA'
							else ''
...
class wi_pago_faprov_base extends w_input {
	...
	function procesa_event() {
		if(isset($_POST['b_print_anexo_faprov_x'])){
			$this->print_anexo_faprov();
			$this->redraw();
		}else if(isset($_POST['b_envio_mail_x']))
			$this->envio_mail();
		else if(isset($_POST['b_add_x'])){
			$this->agregar_pago_faprov($_POST['wi_hidden']);
		}
		else	
	...
	function envio_mail(){
		$temp = new Template_appl('envio_mail_pp.htm');
		
		$COD_PAGO_FAPROV = $this->get_key();
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		
		$sql = "SELECT E.MAIL_NOTIFICACION_PAGO
					  ,NOM_EMPRESA
					  ,CONVERT(VARCHAR, FECHA_PAGO_FAPROV, 103) FECHA_PAGO_FAPROV
					  ,dbo.number_format(E.RUT, 0, ',', '.')+'-'+E.DIG_VERIF RUT
					  ,dbo.number_format(MONTO_DOCUMENTO, 0, ',', '.') MONTO_DOCUMENTO
					  ,NRO_DOCUMENTO
				FROM PAGO_FAPROV PF
					,EMPRESA E
				WHERE PF.COD_PAGO_FAPROV = $COD_PAGO_FAPROV
				AND PF.COD_EMPRESA = E.COD_EMPRESA";
		
		$result	= $db->build_results($sql);
		$lista_mail_to		= $result[0]['MAIL_NOTIFICACION_PAGO'];
		
		if($lista_mail_to == ""){
			$this->redraw();
			$this->alert("Ha ocurrido un error. La empresa asociada debe tener un mail de notificación de pago.");
			return;
		}
		
		$temp->setVar("NOM_EMPRESA",		$result[0]['NOM_EMPRESA']);
		$temp->setVar("RUT", 				$result[0]['RUT']);
		$temp->setVar("FECHA_PAGO_FAPROV",	$result[0]['FECHA_PAGO_FAPROV']);
		$temp->setVar("NRO_CHEQUE",			$result[0]['NRO_DOCUMENTO']);
		$temp->setVar("MONTO_CHEQUE",		$result[0]['MONTO_DOCUMENTO']);
		
		$sql = "SELECT 'Factura ' + CONVERT(VARCHAR, NRO_FAPROV) DOCUMENTO
					  ,CONVERT(VARCHAR, FECHA_FAPROV, 103) FECHA_DOC
					  ,dbo.f_get_oc_from_faprov(F.COD_FAPROV) NRO_ORDEN_COMPRA
				...
				AND F.COD_FAPROV = PFF.COD_FAPROV
				UNION				
				select 'Nota de Credito ' + CONVERT(VARCHAR, NRO_NCPROV) DOCUMENTO
					  ,CONVERT(VARCHAR, FECHA_NCPROV, 103) FECHA_DOC
					  ,'-' NRO_ORDEN_COMPRA
					  ,dbo.number_format(TOTAL_CON_IVA, 0, ',', '.') TOTAL_CON_IVA
				from PAGO_FAPROV PF
					 ,NCPROV_PAGO_FAPROV NPF
					 ,NCPROV N
				where PF.COD_PAGO_FAPROV = $COD_PAGO_FAPROV
				AND PF.COD_PAGO_FAPROV = NPF.COD_PAGO_FAPROV
				AND N.COD_NCPROV = NPF.COD_NCPROV";
		
		$dw_it_pagofaprov = new datawindow($sql, "ITEM_PAGO_FAPROV");
		$dw_it_pagofaprov->retrieve();
		$dw_it_pagofaprov->habilitar($temp, false);
		
		$html = $temp->toString();
		
		$lista_mail_to		= "'$lista_mail_to'";
		$lista_mail_to_name	= "NULL";
		//$lista_mail_to		= "'icampos@integrasystem.cl'";
		
		/*$lista_mail_cc = "'sergio.pechoante@biggi.cl;rescudero@biggi.cl;jcatalan@biggi.cl;'";
		$lista_mail_cc_name = "'SERGIO PECHOANTE;RAFAEL ESCUDERO;JOSE CATALAN;'";*/
		
		$lista_mail_cc = "NULL";
		$lista_mail_cc_name = "NULL";
		
		$lista_mail_bcc = "evergara@integrasystem.cl";
		$lista_mail_bcc_name = "Erick Vergara";
		...
		$param = "'INSERT'
				...
				,null
				,null
			 	,'modulo_alertas@biggi.cl' --sera esta?
			 	,'Módulo Alertas Sistemas Web BIGGI' --sera esta?
			 	,$lista_mail_cc
			 	,$lista_mail_cc_name
			 	,$lista_mail_bcc
			 	,$lista_mail_bcc_name
			 	,$lista_mail_to
			 	,$lista_mail_to_name
			 	,'Prueba (COMERCIAL BIGGI)'	--titulo provisional
			 	,'".str_replace("'","''",$html)."'
			 	,NULL
			 	,'MAIL_PAGO_PROVEEDOR'
			 	,0";
		...
		if($db->EXECUTE_SP($sp, $param))
			$this->alert("Se ha enviado un correo.");
		else
	...
	function print_anexo_faprov() {
		$cod_pago_faprov = $this->get_key();
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);

		$sql = "SELECT F.NRO_FAPROV
						,dbo.f_format_date(FECHA_PAGO_FAPROV, 3) FECHA_FAPROV_STR
						,CONVERT(VARCHAR, F.FECHA_FAPROV, 103) FECHA_FAPROV
						,F.TOTAL_CON_IVA
						,dbo.f_pago_faprov_get_pago_ant(F.COD_FAPROV)+dbo.f_pago_faprov_get_monto_ncprov(F.COD_FAPROV) PAGO_ANTERIOR
						,dbo.f_pago_faprov_get_monto_ncprov(F.COD_FAPROV) MONTO_NC
						,PFF.MONTO_ASIGNADO MONTO_PAGADO
						,PFF.COD_PAGO_FAPROV
						,NOM_EMPRESA
						...
						,PF.COD_TIPO_PAGO_FAPROV
						,UPPER(NOM_TIPO_PAGO_FAPROV) NOM_TIPO_PAGO_FAPROV
						,XC.NOM_CUENTA_CORRIENTE
				FROM 	PAGO_FAPROV_FAPROV PFF
						, FAPROV F left outer join CUENTA_COMPRA c on c.COD_CUENTA_COMPRA = f.COD_CUENTA_COMPRA
								   left outer join CUENTA_CORRIENTE cc on c.COD_CUENTA_CORRIENTE = cc.COD_CUENTA_CORRIENTE
						, PAGO_FAPROV PF
				...
				WHERE 	PFF.COD_PAGO_FAPROV = $cod_pago_faprov AND
						...
						E.COD_EMPRESA = PF.COD_EMPRESA AND
						TPF.COD_TIPO_PAGO_FAPROV = PF.COD_TIPO_PAGO_FAPROV
						AND XC.COD_CUENTA_CORRIENTE = PF.COD_CUENTA_CORRIENTE
				ORDER BY F.NRO_FAPROV";
		$result = $db->build_results($sql);
		...
		for($i=0 ; $i < count($result) ; $i++){
			$total_pagado		= $total_pagado + $result[$i]['MONTO_PAGADO'];
			$total_nc_pagado	= $total_nc_pagado + $result[$i]['MONTO_NC'];
		}	
		
		$labels = array();
		$labels['strCOD_PAGO_FAPROV'] = $cod_pago_faprov;
		$labels['strNOM_EMPRESA'] = $result[0]['NOM_EMPRESA'];
		$labels['strRUT'] = "[".$result[0]['RUT']."]";
		$labels['strMONTO_PAGADO'] = number_format($total_pagado, 0, '', '.');
		$labels['strFECHA_FAPROV'] = $result[0]['FECHA_FAPROV_STR'];
		$labels['strMONTO_TOTAL'] = number_format($total_pagado+$total_nc_pagado, 0, '', '.');
		$labels['strEMISOR'] = $result[0]['EMISOR'];
		$labels['strNOM_TIPO_PAGO_FAPROV'] = $result[0]['NOM_TIPO_PAGO_FAPROV'];
		$labels['strNCC'] = $result[0]['NOM_CUENTA_CORRIENTE'];
		$rpt = new rpt_anexo($sql, $this->root_dir.'appl/pago_faprov/pago_faprov_anexo.xml', $labels, "Anexo ".$cod_pago_faprov.".pdf", 1);			
	}
	
	function habilitar(&$temp, $habilita){
		parent::habilitar($temp, $habilita);
		$this->habilita_boton($temp, 'b_print_anexo_faprov', true);
		
	}
	
	function habilita_boton(&$temp, $boton, $habilita) {
		if($boton == 'b_print_anexo_faprov'){
			if($habilita){
				$control = '<input name="b_print_anexo_faprov" id="b_print_anexo_faprov" src="../../images_appl/b_print_anexo.jpg" type="image" '.
							 'onMouseDown="MM_swapImage(\'b_print_anexo_faprov\',\'\',\'../../images_appl/b_print_anexo_click.jpg\',1)" '.
							 'onMouseUp="MM_swapImgRestore()" onMouseOut="MM_swapImgRestore()" '.
							 'onMouseOver="MM_swapImage(\'b_print_anexo_faprov\',\'\',\'../../images_appl/b_print_anexo_over.jpg\',1)" 
							 onClick="return true;"/>';
			}else{
				$control = '<img src="../../images_appl/b_print_anexo_d.jpg">';
			}
			
			$temp->setVar("WI_PRINT_ANEXO_FAPROV", $control);
		}else if($boton == 'envio_mail'){
			if($habilita){
			...
			if($habilita){
				...
				$control = '<input name="b_'.$boton.'" id="b_'.$boton.'" type="button" onmouseover="entrada(this, '.$ruta_over.')" onmouseout="salida(this, '.$ruta_out.')" onmousedown="down(this, '.$ruta_click.')"'.
							...
							'onClick="dlg_empresa_cta_corriente2();" />';
			}else{
				$control = '<img src="../../../../commonlib/trunk/images/b_add_d.jpg">';
			}
...
class rpt_anexo extends reporte {
	function rpt_anexo($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false) {
	...
	function modifica_pdf(&$pdf){
		...
		//		**************NC*************************
		$sql = "SELECT NRO_NCPROV
					  ,CONVERT(VARCHAR, FECHA_NCPROV, 103) FECHA_NCPROV
					  ,MONTO_ASIGNADO
				FROM NCPROV_PAGO_FAPROV NPF
					,NCPROV N
				WHERE COD_PAGO_FAPROV = $cod_pago_faprov
				AND N.COD_NCPROV = NPF.COD_NCPROV
		...
		if($row > 0){
			...
			$pdf->SetFillColor(194, 214, 154);
			$pdf->SetXY(28.4,$y_ini);
			$pdf->Cell(138.7,15,'DOCUMENTO', 'LTRB', '','L', true);
			$pdf->Cell(138.7,15,'NRO DOC', 'LTRB', '','R', true);
			$pdf->Cell(138.9,15,'FECHA DOC', 'LTRB', '','R', true);
			$pdf->Cell(138.9,15,'MONTO', 'LTRB', '','R', true);
			...
			for($i=0 ; $i < $row ; $i++){
				...
				$pdf->SetXY(28.4,$y_ini);
				$pdf->Cell(138.7,15,'NOTA CREDITO', 'LTRB', '','L');
				$pdf->Cell(138.7,15, $result[$i]['NRO_NCPROV'], 'LTRB', '','R');
				$pdf->Cell(138.9,15, $result[$i]['FECHA_NCPROV'], 'LTRB', '','R');
				$pdf->Cell(138.9,15, number_format($result[$i]['MONTO_ASIGNADO'], 0, ',', '.'), 'LTRB', '','R');
				
				$monto_asig_nc_tot = $monto_asig_nc_tot + $result[$i]['MONTO_ASIGNADO'];
			}
			
			$pdf->SetFont('Arial','B',10);
			$pdf->SetXY(305.8, $y_ini+15.3);
			$pdf->Cell(138.9,15, "Total Nota Crédito:", 'LTRB', '','R');
			$pdf->Cell(138.9,15, number_format($monto_asig_nc_tot, 0, ',', '.'), 'LTRB', '','R');
		}
		////**************factura*****************
		$y_ini = $y_ini+45.9;			
		$pdf->SetFillColor(194, 214, 154);
		...
		$pdf->SetXY(28.4,$y_ini+15.3);
		$pdf->SetFont('Arial','',10);
		
		if($cod_tipo_pago_faprov == 1)
			$pdf->Cell(138.7,15,'CHEQUE', 'LTRB', '','L');
		else if($cod_tipo_pago_faprov == 2)
			$pdf->Cell(138.7,15,'TRANSFERENCIA', 'LTRB', '','L');	
			
		$pdf->Cell(138.7,15, $result[0]['NRO_DOCUMENTO'], 'LTRB', '','R');
		$pdf->Cell(138.9,15, $result[0]['FECHA_DOCUMENTO'], 'LTRB', '','R');
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(138.9,15, number_format($result[0]['MONTO_DOCUMENTO'], 0, ',', '.'), 'LTRB', '','R');
