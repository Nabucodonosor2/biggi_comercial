...
function dlg_tipo_op(ve_cod_usuario) {
    ...
    if (ve_cod_usuario.options[ve_cod_usuario.selectedIndex].label=='S') 
...
function asignacion_monto(ve_seleccion) {	
	...
	if (vl_tipo_documento!='SUELDO') {
		...
		//calcula total iva
		var monto_iva = roundNumber(parseInt(new_sum_total_neto) * parseFloat(document.getElementById('PORC_IVA_0').innerHTML) / 100, 0);
