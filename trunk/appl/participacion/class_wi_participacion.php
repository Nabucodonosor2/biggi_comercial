...
class wi_participacion extends w_input {
	...
	function wi_participacion($cod_item_menu){
		...
		$sql = "SELECT COD_PARTICIPACION
					  ...
					  ,case P.COD_ESTADO_PARTICIPACION
							when ".self::K_ESTADO_PARTICIPACION_ANULADA." then 'ANULADA'
							else ''
						end TITULO_ESTADO_PARTICIPACION
					  ,MOTIVO_ANULA
		...
		$this->dws['dw_participacion']->add_control(new static_num('PORC_IVA', 2));
	...
	function print_record() {	
		...
		$rpt = new rpt_print_participacion($sql, $this->root_dir.'appl/participacion/participacion.xml', $labels, "Participacion ".$cod_participacion.".pdf", 0);
		$this->_load_record();
		return true;
	}
}
