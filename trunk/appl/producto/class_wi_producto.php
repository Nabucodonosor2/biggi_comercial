...
class dw_producto_proveedor extends dw_help_empresa{
    function dw_producto_proveedor(){
        ...
        // se ajustan tamanos para la ventana de productos
        ...
        $this->add_control(new edit_text_hidden('COD_PRODUCTO_PROVEEDOR'));
        ...
        $this->set_mandatory('COD_EMPRESA', 'Código del proveedor');
...
class dw_producto_compuesto_base extends datawindow{
    function dw_producto_compuesto_base(){
        $sql = "	SELECT		COD_PRODUCTO_COMPUESTO
							...
							-- CORRIGE EL CALCULO DEL COSTO BASE DE LOS PRODUCTOS COMPUESTOS LA FUNCION ANTERIOR SIEMPRE REGRESA VALOR 1 MH 21072021
							,(SELECT TOP(1)
                                        dbo.f_prod_get_precio_costo(PP.COD_PRODUCTO,PP.COD_EMPRESA,GETDATE()) PRECIO
                            FROM      	PRODUCTO_PROVEEDOR PP
                                        ,EMPRESA E
                            WHERE     	PP.COD_EMPRESA = E.COD_EMPRESA
                                        AND PP.COD_PRODUCTO = COD_PRODUCTO_HIJO
                                        AND PP.ELIMINADO = 'N'
                            ) COSTO_BASE_PC
...
class dw_producto extends datawindow{
...
class wi_producto_base extends w_input{
    ...
    function wi_producto_base($cod_item_menu){
        ...
        $this->dws['dw_producto'] = new dw_producto($sql);
        ...
        $control->set_onChange("validar_descont(); actualiza_otros_tabs(); cambia_factor_publico();");
        ...
        // asigna los mandatorys
        $this->dws['dw_producto']->set_mandatory('COD_PRODUCTO_PRINCIPAL', 'Código del producto');
        $this->dws['dw_producto']->set_mandatory('NOM_PRODUCTO_PRINCIPAL', 'Descipción del producto');
        ...
        $this->dws['dw_producto']->set_mandatory('FACTOR_VENTA_PUBLICO', 'Factor Venta Público');
        $this->dws['dw_producto']->set_mandatory('PRECIO_VENTA_PUBLICO', 'Precio Venta Público');
        ...
        $this->dws['dw_producto']->set_mandatory('USA_VENTILACION', 'Usa Ventilación');
        ...
        $this->dws['dw_producto']->add_control(new edit_num('CONSUMO_GAS', 16, 16, 2));
        ...
        $this->add_auditoria_relacionada('PRODUCTO_PROVEEDOR', 'COD_PRODUCTO_PROVEEDOR');
        ...
        if (count($result)> 0)
            $precio_prov_1 = $result[0]['PRECIO'];
                ...
                if ($es_compuesto == 'S'){
                    ...
                    $this->dws['dw_producto']->set_item(0, 'TABLE_PRODUCTO_COMPUESTO', 'none');
                    $this->dws['dw_producto']->set_item(0, 'ULTIMO_REG_INGRESO', 'none'); // NO LO DEBE MOSTRAR EN SUS 
    ...
    function print_folleto($tipo_print){
        ...
        print " <script>window.open('../producto_web/print_especial.php?COD_PRODUCTO=$cod_producto&TIPO_PRINT=$tipo_print')</script>";
        $this->_load_record();
    }
    ...
    function save_record($db){
        ...
        /*marca en campo SISTEMA_VALIDO para que sistema es valido el equipo
         * solo en el insert del equipo se asignara valor, por lo tanto no tiene update
         */
        $sistema = $this->get_parametro(self::K_PARAM_SISTEMA);
    ...
    function print_record() {
        ...
        $rpt = new print_producto($sql, $file_name, $labels, "Producto ".$cod_producto.".pdf", 0);
        $this->_load_record();
        return true;
    }
...
class print_producto extends reporte {
    ...
    function modifica_pdf(&$pdf) {
        ...
        //MH ACTUALIZA IMPRESION OFERTA 23052021
		$pdf->SetLineWidth(2);
		//$pdf->Line(120, 33, 480, 33);
		//$pdf->Image(dirname(__FILE__).'/../../images_appl/logobiggipo.jpg', 113, 13,90,36,'JPG');
		$pdf->Image(dirname(__FILE__).'/../../images_appl/fondoimpresionoferta.jpg', 120, 13,370,281,'JPG');
		//fondoimpresionoferta
        $nom_producto = substr($nom_producto, 0, 65);
        ...
        $pdf->SetXY(230,205);
        //$pdf->MultiCell(230,15, $precio_venta_publico.' + IVA.',0,'L'); 29072021 AS SOLICITA VIA PVIDAL QUE LOS CARTELES DE SALA VENTA NO INCLUYAN EL PRECIO

		// QR FUE DESACTIVADO EL 11082021 POR MH PARA ACTUALIZAR LA LIBRERIA.
		/* if ($cod_qr == $cod_producto){
            ...
            $pdf->Image($fname,145,150,80,80);
            unlink($fname);
        } */

		// NUEVO QR FUE ACTUALIZADO EL 11082021 POR MH PARA ACTUALIZAR LA LIBRERIA.
		if ($cod_qr == $cod_producto){
			...
			$pdf->Image($filename,145,150,80,80);
			unlink($filename);
		}

		//$pdf->Image(dirname(__FILE__).'/../../images_appl/logobiggipo.jpg', 395, 250,90,36,'JPG');
		//$pdf->Image(dirname(__FILE__).'/../../images_appl/separapng1.png', 308, 255,90,27,'PNG');
