	...
	function folleto($cod_producto, $pdf){
		...
		if(file_exists($this->folder_producto($cod_producto).'/'.$this->cod_producto_char($cod_producto).'_CAT1.jpg')){
			...
			if($USA_FOTO_ANTIGUA == 'S'){
				...
				$factor = 0.45; 
				$pdf->Image($this->folder_producto($cod_producto).'/'.$this->cod_producto_char($cod_producto).'_CAT1.jpg', 33+$POS_X, 102+$POS_Y, 800*$factor, 800*$factor);
			}
		}else
		...
		if(count($result_atrr_dest) == 3)
			...
			$pdf->Image(dirname(__FILE__).'/../../images_appl/FICHA_FOLLETO_SA.PNG', 24, 26,612*$factor,768*$factor);
