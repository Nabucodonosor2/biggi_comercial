...
class print_folleto_producto{
	...
	function folleto($cod_producto, $pdf){
		...
		$factor = 0.94;
		$pdf->Image('../../images_appl/ficha_folleto.jpg', 24, 26,612*$factor,768*$factor);
		...
		if(file_exists($this->folder_producto($cod_producto).'/FOLLETO_FOTO1.jpg')){
			...
			$factor = 0.95;
			$pdf->Image($this->folder_producto($cod_producto).'/FOLLETO_FOTO1.jpg' , 115+$POS_X, 132+$POS_Y, 199*$factor, 292*$factor);
		}else
		...
		for($k=0 ; $k < count($result) ; $k++){
			...
			$pdf->MultiCell(270,14,$result[$k]['NOM_ATRIBUTO_PRODUCTO'], 0, '', 'L');
		}
