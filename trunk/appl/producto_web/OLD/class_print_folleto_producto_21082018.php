...
class print_folleto_producto{
	...
	function folleto($cod_producto, $pdf){
		...
		if(file_exists($this->folder_producto($cod_producto).'/FOLLETO_FOTO1.jpg')){
			...
			//$factor = 0.45;
			$factor = 0;
			$pdf->Image($this->folder_producto($cod_producto).'/FOLLETO_FOTO1.jpg', 33+$POS_X, 102+$POS_Y, 400*$factor, 300*$factor);
		}else
		...
		$pdf->SetXY(392, 727);
		$pdf->Cell(202,17,'Tel. (56-2) 2412 6200 - www.biggi.cl - info@biggi.cl', 0, '', 'C');
    ...
    function ficha_tecnica($cod_producto, $pdf){
		...
		if(file_exists($this->folder_producto($cod_producto).'/FICHA_FOTO1.jpg')){
			...
			$factor = 0;
			$pdf->Image($this->folder_producto($cod_producto).'/FICHA_FOTO1.jpg' , 28+$POS_X, 140+$POS_Y, 700*$factor, 740*$factor);
		}else
		...
		if(file_exists($this->folder_producto($cod_producto).'/FICHA_FOTO2.jpg')){
			...
			$factor = 0.52;
			$pdf->Image($this->folder_producto($cod_producto).'/FICHA_FOTO2.jpg' , 323+$POS_X, 405+$POS_Y, 500*$factor, 500*$factor);
		}else
