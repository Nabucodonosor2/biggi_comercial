...
class dw_traspaso_bodega extends datawindow {
	...
	function dw_traspaso_bodega() {
		...
		$this->add_control($control = new drop_down_dw('COD_BODEGA_ORIGEN', $sql));
		  $control->set_onChange("limpiaTabla();");
		$this->add_control(new drop_down_dw('COD_BODEGA_DESTINO', $sql));
		$this->add_control(new edit_text_upper('REFERENCIA', 80,100));
		$this->add_control(new static_text('FECHA_TRASPASO_BODEGA'));
		$this->add_control(new static_text('FECHA_REGISTRO'));
		$this->add_control(new edit_text_hidden('COD_TRASPASO_BODEGA_H'));
		
		$sql = "SELECT COD_ESTADO_TRASPASO,NOM_ESTADO 
                FROM ESTADO_TRASPASO_BODEGA";
		
		$this->add_control(new drop_down_dw('COD_ESTADO_TRASPASO', $sql));
		$this->add_control(new edit_text_multiline('OBS', 80, 3));
		
		$this->set_mandatory('COD_BODEGA_ORIGEN', 'Bodega Origen');
		$this->set_mandatory('COD_BODEGA_DESTINO', 'Bodega Destino');
		$this->set_mandatory('FECHA_TRASPASO_BODEGA', 'Fecha traspaso');
		$this->set_mandatory('REFERENCIA', 'Refencia');
	}
}
