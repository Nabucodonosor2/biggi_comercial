...
class server_biggi{
	...
	function svr_print_adicional($empresa, $nro_orden_compra){
		...
		return utf8_encode($result[0]['RESULTADO']);
	}

	function svr_print_etiqueta($nro_orden_compra){
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$sql = "SELECT U.INI_USUARIO+'|'+CONVERT(VARCHAR, OC.COD_ORDEN_COMPRA)+'|'+NOM_EMPRESA+'|'+CONVERT(VARCHAR, NV.COD_NOTA_VENTA) RESULTADO
				FROM ORDEN_COMPRA OC
					,NOTA_VENTA NV
					,USUARIO U
					,EMPRESA E
	...
	function svr_add_faprov_serv($array_datos, $sistema){
		...
		if($sistema == 'COMERCIAL'){
			...
			if($result_cce[0]['COD_PROYECTO_COMPRA'] == '' || count($result_cce) == 0){
				$cod_cuenta_compra = "6"; //(1)COMERCIAL--(3002001) EQUIPOS 
			}else{
	...
	function svr_update_costo_producto($empresa,$cod_producto,$precio){
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$result_ws = $db->build_results("exec sp_update_costo_producto '$empresa','$cod_producto',$precio");
	...
	function svr_oc_por_facturar_serv($origen, $cod_usuario){
		...
		if($origen == 'BODEGA')
			...
			$sql = "SELECT CONVERT(VARCHAR, GETDATE(), 103) FECHA_INF_OC_POR_FACTURAR_SERV
                     ...
                     and O.COD_ESTADO_ORDEN_COMPRA in (1,3,4) -- incluido 3 y 4 solicitado po MH 28/02/2017
                     and dbo.f_oc_get_saldo_sin_faprov(O.COD_ORDEN_COMPRA) > 0
                     AND i.FACTURADO_SIN_WS = 'N'
                     and  FECHA_ORDEN_COMPRA  > {ts '2016-11-01 00:00:00.000'}  --solicitado po MH 28/02/2017
                     and TIPO_ORDEN_COMPRA != 'GASTO_FIJO'  --solicitado po MH 28/02/2017";
	...
	function svr_wo_pago_faprov($sistema_pf, $sistema){
		...
		if($sistema_pf == 'COMERCIAL'){
			...
			if($sistema == 'TODOINOX'){
				...
				$lista_cod_pago_faprov = "11145, 11138, 11135, 11121, 11169, 11160, 11158, 11263, 11301, 11594, 11526, 11641";
			...
			if($sistema == 'TODOINOX'){
				...
				$limite_cod_pf = 439;
				//pagos faprov que estan hechas a mano
				$lista_cod_pago_faprov = "467, 487, 501";
			}else if($sistema == 'COMERCIAL'){
